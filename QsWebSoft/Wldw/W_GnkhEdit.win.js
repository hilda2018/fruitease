...
function W_GnkhEdit() {
	 ...
	 var dw_tybm_list= new DataWindow( requestor );
	 var dw_tybm_edit= new DataWindow( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(dw_tybm_list == null ) dw_tybm_list= new DataWindow( requestor );
		 dw_tybm_list.Attach( win.Control( "dw_tybm_list") ); 
		 if(dw_tybm_edit == null ) dw_tybm_edit= new DataWindow( requestor );
		 dw_tybm_edit.Attach( win.Control( "dw_tybm_edit") ); 
		 ...
		 if(typeof(self.dw_tybm_list_GetFocus)=="function") 
			 win.AttachEvent("dw_tybm_list","GetFocus",self.dw_tybm_list_GetFocus); 
		 ...
		 if(typeof(dw_gnkh_lxr_Clicked)=="function") 
			 win.AttachEvent("dw_gnkh_lxr","Clicked",dw_gnkh_lxr_Clicked); 
		 ...
		 if(typeof(self.Tybmgg)=="function") 
			 win.AttachEvent("btn_tybmgg","Clicked",self.Tybmgg); 
	 ...
	 if(typeof(baseUnBind)=="function") baseUnBind(); 
		 ...
		 dw_tybm_list.Detach(); dw_tybm_list=null; 
		 dw_tybm_edit.Detach(); dw_tybm_edit=null; 
    ...
    this.SetEditMode = function (newMode) {
        ...
        if (bEditFlag) {
            ...
            dw_tybm_list.Modify("DataWindow.Readonly=no");
            dw_tybm_edit.Modify("DataWindow.Readonly=no");
            ...
            dw_tybm_list.Modify("DataWindow.Readonly=yes");
            dw_tybm_edit.Modify("DataWindow.Readonly=yes");
    ...
    this.PostOpen = function () {
        dw_master.SetFocus();
        dw_tybm_list.ShareData(dw_tybm_edit);
        ...
        //#region 获得数据窗口的焦点
        var lastdw = new DataWindow();
        this.dw_tybm_list_GetFocus = function (sender) {
            lastdw.Attach(sender);
        };
        //#endregion
        ...
        this.Save = function () {
            ...
            if ((dw_master.ModifiedCount() + dw_master.DeletedCount() + dw_tybm_list.ModifiedCount() + dw_tybm_list.DeletedCount() + dw_gnkh_lxr.ModifiedCount() + dw_gnkh_lxr.DeletedCount() + dw_gnkh_list.ModifiedCount() + dw_gnkh_list.DeletedCount() + dw_gnkh_shdz.ModifiedCount() + dw_gnkh_shdz.DeletedCount()) <= 0) {
                alert("没有数据需要保存!");
                return;
            }

            var tybm = dw_tybm_list.GetItemString(1, "tybm")

            //更新行号
            for (row = dw_tybm_list.RowCount(); row > 0; row--) {
                var pdcs = dw_tybm_list.GetItemNumber(row, "pdcs")
                if (pdcs = dw_tybm_list.RowCount()) {
                    dw_tybm_list.SetItem(row, "sfzx", "Y");
                } else {
                    dw_tybm_list.SetItem(row, "sfzx", "N");
                }
                var tymc = dw_tybm_list.GetItemString(row, "tymc_jsl")
                dw_tybm_list.SetItem(row, "tymc", tymc)
                dw_tybm_list.SetItem(row, "tybm", tybm)
            }
            ...
            var dw_master_data = dw_tybm_list.GetChanges();
            ...
            if (dw_master_data == "" && dw_gnkh_lxr_data == "" && dw_gnkh_list_data == "" && dw_gnkh_shdz_data == "")
                return;
            ...
            webReq.Invoke("Save", "dw_Master=" + dw_master_data + "&dw_gnkh_lxr=" + dw_gnkh_lxr_data + "&dw_gnkh_list=" + dw_gnkh_list_data + "&dw_gnkh_shdz=" + dw_gnkh_shdz_data + "&tybm=" + tybm + "&operation=" + editMode);
    ...
    //#endregion
        ...
        this.dw_master_ButtonClicked = function (Row, dwoName) {
            if (editMode == "show")
                return;
            if (dwoName == "b_ty") {
                var state = dw_master.GetItemString(Row, "state")
                if (state == "停用") {
                    dw_gnkh_list.SetItem(Row, "state", "新制")
                    dw_master.SetItem(Row, "state", "新制")

                } else {
                    dw_gnkh_list.SetItem(Row, "state", "停用")
                    dw_master.SetItem(Row, "state", "停用")
                }
            }
        ...
        this.dw_master_ItemChanged = function (Row, dwoName, data) {
            if (editMode == "show")
                return;
            if (dwoName == "sfqytybm") {
                dw_gnkh_list.SetItem(Row, "sfqytybm", data)
            }
            if (dwoName == "gzry") {
                dw_gnkh_list.SetItem(Row, "gzry", data)
            }
            if (dwoName == "czry") {
                dw_gnkh_list.SetItem(Row, "czry", data)
            }
        };
        //#endregion
        ...
        //#region 添加明细
            this.AddRow = function () {
              if (lastdw.GetName() == "dw_tybm_list") {
                var row = dw_tybm_list.InsertRow(0);
                var userid = requestor.GetParm("userid");
                dw_tybm_list.SetItem(row, "zbr", userid);
                var pdcs_max = dw_tybm_list.GetItemNumber(row, "pdcs_max");
                dw_tybm_list.SetItem(row, "pdcs", pdcs_max);             
                 
                dw_tybm_list.ScrollToRow(row);
                dw_tybm_list.SetFocus();
        ...
        this.RowCopy = function () {
            ...
            if (lastdw.GetName() == 'dw_tybm_list') {
                var row = dw_tybm_list.GetRow();
                if (row <= 0)
                    return;
                dw_tybm_list.RowsCopy(row, row, DWBUFFER.Primary, dw_tybm_list, row + 1, DWBUFFER.Primary)
                var userid = requestor.GetParm("userid");
                dw_tybm_list.SetItem(row + 1, "zbr", userid);
                var pdcs_max = dw_tybm_list.GetItemNumber(row + 1, "pdcs_max"); 
                dw_tybm_list.SetItem(row + 1, "pdcs", pdcs_max);
                dw_tybm_list.SetFocus();
                dw_tybm_list.ScrollToRow(row + 1);
            }
            if (lastdw.GetName() == 'dw_gnkh_shdz') {
        ...
        this.InsertRow = function () {
            ...
            if (lastdw.GetName() == "dw_tybm_list") {
                var row = dw_tybm_list.InsertRow(dw_tybm_list.GetRow());
                var userid = requestor.GetParm("userid");
                dw_tybm_list.SetItem(row, "zbr", userid);
                var pdcs_max = dw_tybm_list.GetItemNumber(row, "pdcs_max");
                dw_tybm_list.SetItem(row, "pdcs",pdcs_max);
        ...
        this.Tybmgg = function () {
            ...
            var tybm = dw_tybm_list.GetItemString(1, "tybm");
            iw_Wldw_Select.SetDataWindow(dw_tybm_list);
