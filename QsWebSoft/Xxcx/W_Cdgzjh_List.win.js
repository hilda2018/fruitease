...
function W_Cdgzjh_List() {
	 ...
	 var dp_date= new DatePicker( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(dp_date == null ) dp_date= new DatePicker( requestor );
		 dp_date.Attach( win.Control( "dp_date") ); 
		 ...
		 if(typeof(self.dp_date_Clicked)=="function") 
			 win.AttachEvent("dp_date","ValueChanged",self.dp_date_Clicked); 

		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("btn_10","Clicked",self.Retrieve); 
...
//ExtPB.Net JavaScript End  设计器查找标志,请不要删除或修改
    ...
    var iW_Cdgzjh_List_fgqth_cmd = null;
    var iW_Cdgzjh_List_kchx_cmd = null;
    var iW_Cdgzjh_List_schx_cmd = null;
    var iW_Cdgzjh_List_thmx_cmd = null;
...
//        self.dp_date_Clicked();
        //        var myDate = dp_date.GetValue();
        var myDate = new Date(dp_date.GetValue());
        myDate.setDate(1);
        myDate.setHours(0);
        myDate.setMinutes(0);
        myDate.setSeconds(0); 
        dw_list.Retrieve(myDate.getVarDate());
       
    }
    //#endregion

    this.dp_date_Clicked = function () {
        ...
        var myDate = y + '-' + m + '-' + d + ' ' + h + ':' + minute;

        dp_date.SetValue(myDate);
    }
    ...
    this.dw_List_DoubleClicked = function (xPos, yPos, Row, dwoName, Data) {
        if (dwoName != 'sscd') {
            ...
            if (iW_Cdgzlx_Select == null)
                iW_Cdgzlx_Select = new W_Cdgzlx_Select();

            var sscd = escape(dw_list.GetItemString(Row, "sscd").toString());

            iW_Cdgzlx_Select.SetName(dwoName);
            var myDate = new Date(dp_date.GetValue());
            var sjrq = sj(dwoName);
            
            var date = GMTToStr(myDate, sjrq);
            
             
            iW_Cdgzlx_Select.SetDataWindow(dw_list);
            iW_Cdgzlx_Select.SetRow(Row);
            requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Cdgzlx_Select&sscd=" + sscd + "&date=" + date, iW_Cdgzlx_Select);
            dw_list.SetFocus();
    ...
    function GMTToStr(time,sjrq) {
        var date = new Date(time)
        var Str = date.getFullYear() + '-' +
    (date.getMonth() + 1) +'-'+sjrq
        return Str
    }
