...
function W_OpLog() {
	 ...
	 var ddlb_rzlb= new DropDownListBox( requestor );
	 var sle_yhmc= new SingleLineEdit( requestor );
	 var cbx_sfcg= new CheckBox( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(ddlb_rzlb == null ) ddlb_rzlb= new DropDownListBox( requestor );
		 ddlb_rzlb.Attach( win.Control( "ddlb_rzlb") ); 
		 if(sle_yhmc == null ) sle_yhmc= new SingleLineEdit( requestor );
		 sle_yhmc.Attach( win.Control( "sle_yhmc") ); 
		 if(cbx_sfcg == null ) cbx_sfcg= new CheckBox( requestor );
		 cbx_sfcg.Attach( win.Control( "cbx_sfcg") ); 
		 ...
		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("ddlb_rzlb","SelectionChanged",self.Retrieve); 

		 if(typeof(self.sle_EditChanged)=="function") 
			 win.AttachEvent("sle_yhmc","EditChanged",self.sle_EditChanged); 

		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("cbx_sfcg","Clicked",self.Retrieve); 
	 ...
	 if(typeof(baseUnBind)=="function") baseUnBind(); 
		 ...
		 ddlb_rzlb.Detach(); ddlb_rzlb=null; 
		 sle_yhmc.Detach(); sle_yhmc=null; 
		 cbx_sfcg.Detach(); cbx_sfcg=null; 
    ...
    this.Retrieve = function () {
        ...
        var rzlb = ddlb_rzlb.GetText();
        var sfcg = "N"
        if (cbx_sfcg.GetChecked() == true) {
            sfcg = "Y";
        }

        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), rzlb, sfcg,userid);
    ...
    this.sle_EditChanged = function (text) {
        ...
        var yhmc = sle_yhmc.GetText();
        var ls_filter = ""

        if (yhmc == null || yhmc == "") {
            //
        } else {
            ls_filter = "( username  like '%" + yhmc + "%')"
        };
    ...
    this.dw_List_Clicked = function (xPos, yPos, Row, dwoName, Data) {
        if (Row > 0) {
            fzks_row = Row;
            if (dwoName == "tpdz") {
                var tpdz = dw_list.GetItemString(Row, "tpdz");
                if (tpdz == null && tpdz == "") {
                    requestor.MessageBox("提示", "没有图片地址！")
                } else {
                    ...
                    //        }
                        ...
                        ts_1.Open(node, label, "", "/Wjxz/W_Wjxz_App.aspx?FunID=" + node + '&wjmc=' + tpdz, true);
 
//                    javascript: window.open(tpdz, "_blank", "channelmode");
                }
            }
        };
    }
