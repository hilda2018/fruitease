...
function W_GnkhLxrList() {
	 ...
	 var sle_lxr= new SingleLineEdit( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(sle_lxr == null ) sle_lxr= new SingleLineEdit( requestor );
		 sle_lxr.Attach( win.Control( "sle_lxr") ); 
		 ...
		 if(typeof(self.slekhmc_EditChanged)=="function") 
			 win.AttachEvent("sle_khmc","EditChanged",self.slekhmc_EditChanged); 

		 if(typeof(self.slelxr_EditChanged)=="function") 
			 win.AttachEvent("sle_lxr","EditChanged",self.slelxr_EditChanged); 
    ...
    webReq.SetWebServiceUrl("/Service/GnkhLxr.ashx");
    ...
    this.slekhmc_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(gnkhpd_tymc) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    //#region 过滤客户
    this.sleklxr_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(lxr) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.dw_list_Clicked = function (xPos, yPos, Row, dwoName) {
        var tybm = dw_list.GetItemString(Row, "yw_khbm");
        dw_list.GetChild("khbm", dwc);          
        dwc.Retrieve(tybm);
    }
    //#endregion
