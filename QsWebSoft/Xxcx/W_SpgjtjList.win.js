...
function W_SpgjtjList() {
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(typeof(self.sle_EditChanged)=="function") 
			 win.AttachEvent("sle_sp","EditChanged",self.sle_EditChanged); 
    ...
    webReq.SetWebServiceUrl("/Service/Spgjtj.ashx");   
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), ysfsdm, zydldm, mdg, jdrjc);
    ...
    this.sle_EditChanged = function (text) {
        ...
        var sp = sle_sp.GetText();
        var gj = sle_gj.GetText();
        var ls_filter = "";
        if (sp == "" || sp == null) {
            ls_filter = "";
        } else {
            ls_filter = "(( Upper(spmc) like Upper('%" + text + "%'))) "
        };
        ...
        if (gj == "" || gj == null) {
            ...
            if (ls_filter == "") {
                ls_filter = "(( Upper(gjmc) like Upper('%" + text + "%'))) "
            } else {
                ls_filter = ls_filter + " and (( Upper(gjmc) like Upper('%" + text + "%'))) "
            } 
        };
    ...
    this.dw_List_DoubleClicked = function (xPos, yPos, Row, dwoName) {
        if (Row > 0) {
            if (iw_SpgjtjList_Mx == null)
                iw_SpgjtjList_Mx = new W_SpgjtjList_Mx();

            var Dlwtf = requestor.GetParm("Dlwtf");
            var Spmc = dw_list.GetItemString(Row, "spmc")
            var Gjmc = dw_list.GetItemString(Row, "gjmc") 
            var BeginDate = dp_begin.GetValue();
            var EndDate = dp_end.GetValue();
            var Ysfs = ddlb_ysfs.GetText();

            iw_SpgjtjList_Mx.SetSpmc(Spmc);
            iw_SpgjtjList_Mx.SetGjmc(Gjmc);
            iw_SpgjtjList_Mx.SetBeginDate(BeginDate);
            iw_SpgjtjList_Mx.SetEndDate(EndDate);
            iw_SpgjtjList_Mx.SetYsfs(Ysfs);
           
            requestor.Open("/LoadWindow.ashx?WinName=Xxcx.W_SpgjtjList_Mx", iw_SpgjtjList_Mx);
        }
    }
    //#endregion
    ...
    //#region 获得字段焦点
    this.Help = function () {
        javascript: window.open('\商品国家统计.txt')
    }
    //#endregion
