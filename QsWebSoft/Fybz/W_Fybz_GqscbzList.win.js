...
//    }
  ...
  //#region 删除商品
    ...
    this.dw_List_DoubleClicked = function (xPos, yPos, Row, dwoName) {
        ...
        //起点双击
            if (dwoName == "origin") {               
                if (idw_fybz_qd_Select == null)
                    idw_fybz_qd_Select = new dw_fybz_qd_Select();
                khlx = "hymt";
                var ywy = requestor.GetParm("userid");
                var ShareMode = requestor.GetParm("ShareMode");
                var Dlwtf = requestor.GetParm("Dlwtf");

                idw_fybz_qd_Select.SetDataWindow(dw_list);
                idw_fybz_qd_Select.SetData("fybz_mtzlfy");
                idw_fybz_qd_Select.SetRow(Row);
                requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.dw_fybz_qd_Select&ShareMode=" + ShareMode + "&ywy=" + ywy + "&Dlwtf=" + Dlwtf + "&khlx=" + khlx, idw_fybz_qd_Select);
                dw_list.SetFocus();

            }
            if (dwoName == "destination") {
                if (idw_fybz_zd_Select == null)
                    idw_fybz_zd_Select = new dw_fybz_zd_Select();
                khlx = "hymt";
                var ywy = requestor.GetParm("userid");
                var ShareMode = requestor.GetParm("ShareMode");
                var Dlwtf = requestor.GetParm("Dlwtf");

                idw_fybz_zd_Select.SetDataWindow(dw_list);
                idw_fybz_zd_Select.SetData("fybz_mtzlfy");
                idw_fybz_zd_Select.SetRow(Row);
                requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.dw_fybz_zd_Select&ShareMode=" + ShareMode + "&ywy=" + ywy + "&Dlwtf=" + Dlwtf + "&khlx=" + khlx, idw_fybz_zd_Select);
                dw_list.SetFocus();
...
//            dw_list.SetFilter("( Upper(origin) like Upper('%" + text + "%')) or ( Upper(destination) like Upper('%" + text + "%')) or ( Upper(duration) like Upper('%" + text + "%')) ");
         dw_list.SetFilter("(Upper(origin) like Upper('%" + text + "%')) or (Upper(destination) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.Save = function () {
        ...
        webReq.Invoke("GqscbListSave", "dw_list=" + dw_d_data);
