    ...
    this.dw_List_DoubleClicked = function (xPos, yPos, Row, dwoName) {
        if (Row > 0) {
            if (dwoName == "cdgsjc") {
                ...
                iw_Wldw_Select.SetData("fybz_hdf_cdgs");
            ...
            if (dwoName == "cyrjc") {
                ...
                iw_Wldw_Select.SetDataWindow(dw_list);
                iw_Wldw_Select.SetData("fybz_hdf_cyr");
                iw_Wldw_Select.SetRow(Row);
                requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Wldw_Select&ShareMode=" + ShareMode + "&ywy=" + ywy + "&Dlwtf=" + Dlwtf + "&khlx=" + khlx, iw_Wldw_Select);
                dw_list.SetFocus();
...
//        dw_print.PrintPreview();
    ...
    this.slecgs_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(cdgs) like Upper('%" + text + "%')) or ( Upper(cdgsjc) like Upper('%" + text + "%')) or ( Upper(cdgspym) like Upper('%" + text + "%')) or ( Upper(cyr) like Upper('%" + text + "%')) or ( Upper(cyrjc) like Upper('%" + text + "%')) or ( Upper(cyrpym) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Fybz.W_Fybz_HdfList", "dw_fybz_hdf_list");
        self.Retrieve();
    }
    //#endregion
    ...
    this.Save = function () {
        ...
        for (row = 1; row <= dw_list.RowCount(); row++) {
            var fymc_jsl = dw_list.GetItemString(row, "fymc_jsl");
            dw_list.SetItem(row, "fymc", fymc_jsl);
        }
        if (dw_list.AcceptText() != 1)
            return;
        ...
        webReq.Invoke("HdfListSave", "dw_list=" + dw_d_data);
