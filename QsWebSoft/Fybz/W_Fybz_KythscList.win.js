...
function W_Fybz_KythscList() {

	 var self=this;
	 var requestor= new PBWindow();
	 var dw_list= new DataWindow( requestor );
	 var tb_2= new ToolStrip( requestor );
	 var sle_cgs= new SingleLineEdit( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(sle_cgs == null ) sle_cgs= new SingleLineEdit( requestor );
		 sle_cgs.Attach( win.Control( "sle_cgs") ); 
		 ...
		 if(typeof(self.slecgs_EditChanged)=="function") 
			 win.AttachEvent("sle_cgs","EditChanged",self.slecgs_EditChanged); 
    ...
    var idw_fybz_qd_Select = null;
    var iW_Wldw_KythscShdz_Select = null;
    var idw_fybz_zd_Select = null;
    var iW_Jyd_Select = null;
    //实例化明细单的WebService请求的JS类对象
    var webReq = new WebRequestor();
    webReq.SetWebServiceUrl("/Service/Fybz.ashx");   
    ...
    this.dw_List_DoubleClicked = function (xPos, yPos, Row, dwoName) {
        ...
        //起点双击
            if (dwoName == "qidian") { 
                ...
                iw_Wldw_Select.SetData("hddz_xhgqQd");
...
//                iw_Eport_Select.SetData("qd");           
//                iw_Eport_Select.SetRow(Row);
//                requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Eport_Select", iw_Eport_Select);
//                dw_list.SetFocus();
                }
       //终点 双击
            if (dwoName == "zhongdian") {
                if (iW_Wldw_KythscShdz_Select == null)
                iW_Wldw_KythscShdz_Select = new W_Wldw_KythscShdz_Select();
                var ywy = requestor.GetParm("userid");
                var ShareMode = requestor.GetParm("ShareMode");
                var Dlwtf = requestor.GetParm("Dlwtf");                 
...
//                var Khbm = QsWebSoft.PubMethod.ReadWldwTybm(yw_khbm).value;
                var khbm = "";
                iW_Wldw_KythscShdz_Select.SetDataWindow(dw_list);
                iW_Wldw_KythscShdz_Select.SetData("hddz_shdz5");
                iW_Wldw_KythscShdz_Select.SetRow(Row);
                requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Wldw_KythscShdz_Select&ShareMode=" + ShareMode + "&ywy=" + ywy + "&Dlwtf=" + Dlwtf+"&Khbm"+khbm, iW_Wldw_KythscShdz_Select);
                //requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Wldw_Shdz_Select&ShareMode=" + ShareMode + "&ywy=" + ywy + "&Dlwtf=" + Dlwtf, iw_Wldw_Shdz_Select);
                dw_list.SetFocus();
            }
            if(dwoName == "jydjc"){
                if (iW_Jyd_Select == null)
                iW_Jyd_Select = new W_Jyd_Select();
                var ywy = requestor.GetParm("userid");
                var ShareMode = requestor.GetParm("ShareMode");
                var Dlwtf = requestor.GetParm("Dlwtf");                               
                 
                iW_Jyd_Select.SetDataWindow(dw_list);
                iW_Jyd_Select.SetData("jydjc");
                iW_Jyd_Select.SetRow(Row);
                requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Jyd_Select&ShareMode=" + ShareMode + "&ywy=" + ywy + "&Dlwtf=" + Dlwtf, iW_Jyd_Select);
   ...
   //#region dw_List  ItemChanged
    ...
    //#region 过滤
        //#region 过滤客户
    this.slecgs_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else         
//            dw_list.SetFilter("( Upper(origin) like Upper('%" + text + "%')) or ( Upper(destination) like Upper('%" + text + "%')) or ( Upper(duration) like Upper('%" + text + "%')) ");
         dw_list.SetFilter("(Upper(qidian) like Upper('%" + text + "%')) or (Upper(zhongdian) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Fybz.W_Fybz_MtzlfyList", "dw_fybz_mtzlfy_list");
        self.Retrieve();
    }
    //#endregion
    ...
    this.Save = function () {
        ...
        webReq.Invoke("KythscListSave", "dw_list=" + dw_d_data);
