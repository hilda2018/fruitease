...
function W_Sys_Set_Fun() {
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(typeof(self.jsjry)=="function") 
			 win.AttachEvent("btn_jsjry","Clicked",self.jsjry); 
    ...
    webReq.SetWebServiceUrl("/Service/Module.ashx");
...
this.sleGl_EditChanged = function (text) {
    ...
    if (text == "")
        ...
        dw_1.SetFilter("( Upper(title) like '%" + text + "%')  or ( Lower(title) like '%" + text + "%') or ( Upper(windowname) like '%" + text + "%')  or ( Lower(windowname) like '%" + text + "%') or ( Upper(parentid) like '%" + text + "%')  or ( Lower(parentid) like '%" + text + "%')");
    dw_1.Filter();
    dw_1.Sort();
    dw_1.SetRedraw(true);
}
