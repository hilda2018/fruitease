...
function W_Sys_Warning_SetPermission() {
    ...
    this.UpdatePermission = function () {
        ...
        moduleID = dw_Modules.GetItemString(Row, "yjlxbh");
        roleID = dw_RoleInfo.GetItemString(dw_RoleInfo.GetRow(), "id");
        webReq.Invoke("SetWarning", "roleid=" + roleID + "&moduleid=" + moduleID + "&data=" + ds_right.GetSyntaxData());
    ...
    this.dw_Modules_Clicked = function (xPos, yPos, Row, dwoName) {
        if (Row <= 0 || priorRow == Row)
            return;
        var moduleID = dw_Modules.GetItemString(Row, "yjlxbh");
    ...
    this.dw_1_ItemChanged = function (row, dwoName, data) {
        if (row <= 0)
            return;
        moduleID = dw_1.GetItemString(row, "syjbh");
        roleID = dw_RoleInfo.GetItemString(1, "id");
        funID = dw_1.GetItemString(row, "yjlxbh");
