...
//#region 数据接收
    this.DataReceive = function (mxdbh, bbh) {
        ...
        //更新明细单状态
        var num = QsWebSoft.PubMethod.UpdateMxdAfterReceiveData(mxdbh, bbh).value;
        if (num != 1) {
            alert("提示，更新明细单状态失败!");
        }
    ...
    this.DetailDataReceive = function (mxdbh, bbh) {
        ...
        ds_read_jhdj.Retrieve(mxdbh, bbh);
        ds_read_jhdj_cmd.Retrieve(mxdbh, bbh);
        ds_read_jhdj_fy.Retrieve(mxdbh, bbh);

        if (ds_read_jhdj.RowsCopy(1, 1, DWBUFFER.Primary, ds_read_master, 1, DWBUFFER.Primary) < 0) {
            alert("友情提示,结汇单证主表信息读取数据时出错！");
            return;
        }
        else {
            //修改单证状态
            ds_read_master.SetItem(1, 'zdrq', date.getDate())
            ds_read_master.SetItem(1, 'state', "新制")
            ds_read_master.SetItem(1, 'dz_jsrq', date.getDate())
            ds_read_master.SetItem(1, 'dzjh_zdy', userid)
            ds_read_master.SetItem(1, "dzjh_state", "已接收")
            ds_read_master.SetItem(1, 'fsrq', date.getDate())
        }

        if (ds_read_jhdj_cmd.RowsCopy(1, 1, DWBUFFER.Primary, ds_read_cmd, 1, DWBUFFER.Primary) < 0) {
            alert("友情提示,结汇单证商品信息读取数据时出错！");
            return;
        }
        if (ds_read_jhdj_fy.RowCount() > 0) {
            if (ds_read_jhdj_fy.RowsCopy(1, 1, DWBUFFER.Primary, ds_read_fy, 1, DWBUFFER.Primary) < 0) {
