...
function W_Xtdm_Zskfl() {
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(typeof(self.slezskfl_EditChanged)=="function") 
			 win.AttachEvent("sle_1","EditChanged",self.slezskfl_EditChanged); 
    ...
    webReq.SetWebServiceUrl("/Service/Zskfl.ashx");
...
//#region 过滤
this.slezskfl_EditChanged = function (text) {
    dw_1.SetRedraw(false);
    if (text == "")
        dw_1.SetFilter("");
    else
        dw_1.SetFilter("( Upper(zskfl) like '%" + text + "%')  or ( Lower(zskfl) like '%" + text + "%')");
    dw_1.Filter();
    dw_1.Sort();
    dw_1.SetRedraw(true);
}
