...
function W_Xtdm_Clfs() {
    ...
    webReq.SetWebServiceUrl("/Service/Clfs.ashx");
...
this.Save = function () {
    ...
    for (var i = 1; i <= dw_1.RowCount(); i++) {
        ...
        var xlfs = dw_1.GetItemString(i, "xlfs");
        var li_pos = 0;
        var ycyymc_tctsfh = xlfs;

        ycyymc_tctsfh.replace(/[\ |\~|\`|\!|\@|\#|\$|\%|\^|\&|\*|\(|\)|\-|\_|\+|\=|\||\\|\[|\]|\{|\}|\;|\:|\"|\'|\,|\<|\.|\>|\/|\?]/g, "");

        var li_find1 = 0;
        if (i > 1) {
            li_find1 = dw_1.Find("xlfs = '" + xlfs + "' or xlfs = '" + ycyymc_tctsfh + "' or ycyymc_tctsfh = '" + xlfs+ "' or ycyymc_tctsfh = '" + ycyymc_tctsfh + "'", 1, i - 1)
        }

        var li_find2 = 0;
        if (i < dw_1.RowCount()) {
            li_find1 = dw_1.Find("xlfs= '" + xlfs+ "' or xlfs= '" + ycyymc_tctsfh + "' or ycyymc_tctsfh = '" + xlfs+ "' or ycyymc_tctsfh = '" + ycyymc_tctsfh + "'", i + 1, dw_1.RowCount())
        }
        ...
        if (li_find1 + li_find2 > 0) {
            ...
            requestor.MessageBox("提示", "异常原因：" + xlfs+ "在" + li_find.toString() + "行存在重复，不能保存！");
            return;
        }
        if (dw_1.GetItemStatus(i, 0, DWBUFFER.Primary) == 1) {

            var ctr_code = dw_1.GetItemString(i, "xlfsbm");
            ...
            dw_log.SetItem(row, "changecols", "xlfsbm,xlfs");
