...
function W_Xtdm_Yjlx() {
    ...
    this.bind = function (win) {
        ...
        if (typeof (self.open) == "function")
            win.AttachEvent("btn_open", "Clicked", self.open);
...
this.dw_1_DoubleClicked = function (xPos, yPos, Row, dwoName, Data) {
    var r = dw_1.GetRow();
    if (r > 0) {
        if (iW_Xtdm_Yjlx_Ycyys == null)
            iW_Xtdm_Yjlx_Ycyys = new W_Xtdm_Yjlx_Ycyys();

        var id = dw_1.GetItemString(dw_1.GetRow(), "yjlxbh");
//        alert(id);
//        iW_Xtdm_Yjlx_Yjlx.SetData(id);

        requestor.Open("/LoadWindow.ashx?WinName=xt.W_Xtdm_Yjlx_Ycyys&id=" + id, iW_Xtdm_Yjlx_Ycyys);
        dw_1.SetFocus();
