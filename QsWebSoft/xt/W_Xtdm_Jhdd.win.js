...
function W_Xtdm_Jhdd() {

    var self = this;
    var requestor = new PBWindow();
    var dw_1 = new DataWindow(requestor);
    var tb_1 = new ToolStrip(requestor);
    var sle_jhdd = new SingleLineEdit(requestor);
    ...
    this.bind = function (win) {
        ...
        if (sle_jhdd == null) sle_jhdd = new SingleLineEdit(requestor);
        sle_jhdd.Attach(win.Control("sle_jhdd"));
        ...
        if (typeof (self.sle_jhdd_EditChanged) == "function")
            win.AttachEvent("sle_jhdd", "EditChanged", self.sle_jhdd_EditChanged);
    ...
    webReq.SetWebServiceUrl("/Service/Jhdd.ashx");
...
//#region 过滤接货地点
this.sle_jhdd_EditChanged = function (text) {
    dw_1.SetRedraw(false);
    if (text == "")
        dw_1.SetFilter("");
    else
        dw_1.SetFilter("Upper(jhdd) like Upper('%" + text + "%') ");
    dw_1.Filter();
    dw_1.Sort();
    dw_1.SetRedraw(true);
}
...
//    };
    ...
    for (row = dw_1.RowCount(); row > 0; row--) {
        var jhdd = dw_1.GetItemString(row, "jhdd");
        var jhdd_pym = dw_1.GetItemString(row, "jhdd_pym");
        if (jhdd_pym == "") {
            var jhdd_pym = QsWebSoft.PubMethod.GetPYString(jhdd).value;
            dw_1.SetItem(row, "jhdd_pym", jhdd_pym);
        }
    };
