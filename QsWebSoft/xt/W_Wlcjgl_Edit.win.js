...
function W_Wlcjgl_Edit() {
    ...
    webReq.SetWebServiceUrl("/Service/W_Wlcjgl.ashx");
...
//                var date = new Date();
//                dw_master.SetItem(1, "sqsj", date.getVarDate());
            }
            if (operation == "copy") {
                self.Copy();
...
//        dw_master.SetItem(1, "ze", je_sum);

        //更新行号
        for (row = dw_cmd.RowCount(); row > 0; row--) {
            dw_cmd.SetItem(row, "zbr", zbr);
            dw_cmd.SetItem(row, "zbrq", zbrq);
        }
        ...
        webReq.Invoke("Save_Edit", "&dw_cmd=" + dw_cmd_data );
...
//        if (iW_xt_Bgdsy_Select == null)
//            iW_xt_Bgdsy_Select = new W_xt_Bgdsy_Select();
...
//        
//        iW_xt_Bgdsy_Select.SetRow(row);
//        iW_xt_Bgdsy_Select.SetDwo(dwo);
//        iW_xt_Bgdsy_Select.SetDetailDW(dw_cmd);

//        requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_xt_Bgdsy_Select&ShareMode=" + ShareMode + '&ywy=' + ywy + '&Dlwtf=' + Dlwtf + '&Sqdbh=' + sqdbh + '&bb=' + bb, iW_xt_Bgdsy_Select);
...
//        dw_master.AcceptText();
//        var bb = dw_master.GetItemString(1, "bb");
//        if (bb == "" || bb == null) {
//            requestor.MessageBox("", "请输入币别!");
//            return;
//        }
//        if (iW_xt_Bgdsy_Select == null)
//            iW_xt_Bgdsy_Select = new W_xt_Bgdsy_Select();
...
//        requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_xt_Bgdsy_Select&ShareMode=" + ShareMode + '&ywy=' + ywy + '&Dlwtf=' + Dlwtf + '&bb=' + bb, iW_xt_Bgdsy_Select);
//   
        var row = dw_cmd.InsertRow(0);
    ...
    this.Print = function () {
        ...
        dw_print.SetDataObject("dw_xt_ysjjf_print");
...
//        dw_cmd.InsertRow(0);
    ...
    this.dw_cmd_DoubleClicked = function (xPos, yPos, Row, dwoName) {
        if (Row > 0) {
            if (dwoName == "subordinate_company") {
                if (iw_Wldw_Select == null)
                    iw_Wldw_Select = new W_Wldw_Select();
                khlx = "wts";
                var ywy = requestor.GetParm("userid");
                ...
                iw_Wldw_Select.SetData("W_Wlcjgl");
