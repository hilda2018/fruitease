...
function W_Xtdm_Yjlx_Ycyys() {
    ...
    this.bind = function (win) {
        ...
        if (typeof (self.sle_EditChanged1) == "function")
            win.AttachEvent("sle_2", "EditChanged", self.sle_EditChanged1);
    ...
    var webReq = new WebRequestor();
        webReq.SetWebServiceUrl("/Service/Yjlx.ashx");
    ...
    this.Save = function () {
        if (!dataChanged) {
            requestor.MessageBox("提示", "未调整预警类型资料,不需要保存!");
            return;
        }

        var id = escape(dw_3.GetItemString(1, "yjlxbh"));
        
        //var data = dw_2.GetChanges();
        var data = "";
        for (row = 1; row <= dw_2.RowCount(); row++) {
            data += escape(dw_2.GetItemString(row, "ycyybm")) + ";";
           
        }

        webReq.Invoke("SetYjlxYcyy", "yjlxbh=" + id + "&roles=" + data);
	 ...
	 this.sle_EditChanged = function (text) {
	     ...
	     if (text == "")
	         ...
	         dw_1.SetFilter("Upper(ycyymc) like Upper('%" + text + "%')");
	 ...
	 this.sle_EditChanged1 = function (text) {
	     dw_2.SetRedraw(false);
	     if (text == "")
	         dw_2.SetFilter("");
	     else
	         dw_2.SetFilter("Upper(ycyymc) like Upper('%" + text + "%')");
