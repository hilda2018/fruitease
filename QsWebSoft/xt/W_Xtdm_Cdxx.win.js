...
function W_Xtdm_Cdxx() {
    ...
    this.bind = function (win) {
        ...
        if (typeof (self.sle_cd_EditChanged) == "function")
            win.AttachEvent("sle_cd", "EditChanged", self.sle_cd_EditChanged);
    ...
    webReq.SetWebServiceUrl("/Service/Cdxx.ashx");
...
//#region 过滤车队
this.sle_cd_EditChanged = function (text) {
    dw_1.SetRedraw(false);
    if (text == "")
        dw_1.SetFilter("");
    else
        dw_1.SetFilter("Upper(clxx) like Upper('%" + text + "%') ");
    dw_1.Filter();
    dw_1.Sort();
    dw_1.SetRedraw(true);
}
...
this.Save = function () {
    ...
    for (row = dw_1.RowCount(); row > 0; row--) {
        var clxx = dw_1.GetItemString(row, "clxx");
        var clxx_pym = dw_1.GetItemString(row, "clxx_pym");
        if (clxx_pym == "") {
            var clxx_pym = QsWebSoft.PubMethod.GetPYString(clxx).value;
            dw_1.SetItem(row, "clxx_pym", clxx_pym);
        }
    };
