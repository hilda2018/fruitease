...
function W_Xtdm_Ycyy() {
    ...
    this.bind = function (win) {
        ...
        if (typeof (self.Save1) == "function")
            win.AttachEvent("btn_2", "Clicked", self.Save1);
    ...
    webReq.SetWebServiceUrl("/Service/Ycyy.ashx");
...
this.Save1 = function () {
    ...
    for (var i = 1; i <= dw_1.RowCount(); i++) {
        var ycyymc = dw_1.GetItemString(i, "ycyymc");
        var li_pos = 0;
        ...
        var li_find1 = 0;
        if (i > 1) {
            li_find1 = dw_1.Find("ycyymc = '" + ycyymc + "' or ycyymc = '" + ycyymc_tctsfh + "' or ycyymc_tctsfh = '" + ycyymc + "' or ycyymc_tctsfh = '" + ycyymc_tctsfh + "'", 1, i - 1)
        }
        ...
        if (li_find1 + li_find2 > 0) {
            ...
            requestor.MessageBox("提示", "异常原因：" + ycyymc + "在" + li_find.toString() + "行存在重复，不能保存！");
            return;
        }
        ...
        if (dw_1.GetItemStatus(i, 0, DWBUFFER.Primary) == 1) {
            ...
            var ctr_code = dw_1.GetItemString(i, "ycyybm");
            ...
            dw_log.SetItem(row, "tablename", "t_ycyy");
            dw_log.SetItem(row, "changecols", "ycyymc,sfmx,ycyylb,yy");
