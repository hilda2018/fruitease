...
this.InsertRow = function ()
{
   var userid =  requestor.GetParm("userid");
    ...
    dw_1.SetItem( li_insertrow, "zbr", userid )
}
...
this.sleGl_EditChanged = function (text) {
    ...
    if (text == "")
        ...
        dw_1.SetFilter("(Upper(fybm) like Upper('%" + text + "%')) or (Upper(fymc) like Upper('%" + text + "%'))");
    dw_1.Filter();
    dw_1.Sort();
    dw_1.SetRedraw(true);
}
...
this.Save = function () {
    ...
    for (var i = 1; i <= dw_1.RowCount(); i++) {
        ...
        var fymc= dw_1.GetItemString(i, "fymc");
        var li_pos = 0;
        var ycyymc_tctsfh = fymc;

        ycyymc_tctsfh.replace(/[\ |\~|\`|\!|\@|\#|\$|\%|\^|\&|\*|\(|\)|\-|\_|\+|\=|\||\\|\[|\]|\{|\}|\;|\:|\"|\'|\,|\<|\.|\>|\/|\?]/g, "");

        var li_find1 = 0;
        if (i > 1) {
            li_find1 = dw_1.Find("fymc= '" + fymc+ "' or fymc= '" + ycyymc_tctsfh + "' or ycyymc_tctsfh = '" + fymc+ "' or ycyymc_tctsfh = '" + ycyymc_tctsfh + "'", 1, i - 1)
        }

        var li_find2 = 0;
        if (i < dw_1.RowCount()) {
            li_find1 = dw_1.Find("fymc= '" + fymc+ "' or fymc= '" + ycyymc_tctsfh + "' or ycyymc_tctsfh = '" + fymc+ "' or ycyymc_tctsfh = '" + ycyymc_tctsfh + "'", i + 1, dw_1.RowCount())
        }
        ...
        if (li_find1 + li_find2 > 0) {
            ...
            requestor.MessageBox("提示", "异常原因：" + fymc+ "在" + li_find.toString() + "行存在重复，不能保存！");
            return;
        }

        if (dw_1.GetItemStatus(i, 0, DWBUFFER.Primary) != 0) {
            var fybm = dw_1.GetItemString(i, "fybm");
            ...
            dw_log.SetItem(row, "tablename", "t_fylx");
            dw_log.SetItem(row, "changecols", "");
            dw_log.SetItem(row, "mainid", fybm);
