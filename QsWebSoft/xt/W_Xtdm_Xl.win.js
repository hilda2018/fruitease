...
this.Save = function () {
    ...
    for (var i = 1; i <= dw_1.RowCount(); i++) {
        ...
        var xl = dw_1.GetItemString(i, "xl");
        var li_pos = 0;
        var ycyymc_tctsfh = xl;

        ycyymc_tctsfh.replace(/[\ |\~|\`|\!|\@|\#|\$|\%|\^|\&|\*|\(|\)|\-|\_|\+|\=|\||\\|\[|\]|\{|\}|\;|\:|\"|\'|\,|\<|\.|\>|\/|\?]/g, "");

        var li_find1 = 0;
        if (i > 1) {
            li_find1 = dw_1.Find("xl= '" + xl + "' or xl= '" + ycyymc_tctsfh + "' or ycyymc_tctsfh = '" + xl + "' or ycyymc_tctsfh = '" + ycyymc_tctsfh + "'", 1, i - 1)
        }

        var li_find2 = 0;
        if (i < dw_1.RowCount()) {
            li_find1 = dw_1.Find("xl= '" + xl + "' or xl= '" + ycyymc_tctsfh + "' or ycyymc_tctsfh = '" + xl + "' or ycyymc_tctsfh = '" + ycyymc_tctsfh + "'", i + 1, dw_1.RowCount())
        }
        ...
        if (li_find1 + li_find2 > 0) {
            ...
            requestor.MessageBox("提示", "异常原因：" + xl + "在" + li_find.toString() + "行存在重复，不能保存！");
            return;
        }
        ...
        if (dw_1.GetItemStatus(i, 0, DWBUFFER.Primary) == 3 || dw_1.GetItemStatus(i, 0, DWBUFFER.Primary) == 1) {
            ...
            var ctr_code = dw_1.GetItemString(i, "xlbh");
            ...
            dw_log.SetItem(row, "tablename", "t_xl");
            dw_log.SetItem(row, "changecols", "xl,xldm");
