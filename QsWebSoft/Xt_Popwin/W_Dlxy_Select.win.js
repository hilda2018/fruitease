...
function W_Dlxy_Select() {
    ...
    this.OK = function () {
        ...
        var dlwtf = dw_1.GetItemString(row, 'dlwtf_bm');
        var jsfs = dw_1.GetItemString(row, 'jsfs');
        var dlsxfl = dw_1.GetItemNumber(row, 'dlsxfl');
        if (dw_mx != null) {
            if (column == "dlxyh") {
                dw_mx.SetItem(1, 'dlxyh', code);
                dw_mx.SetItem(1, 'dlwtf', dlwtf);
                if (jsfs == 0) {
                    dw_mx2.SetItem(1, 'dljsfs', "手续费");
                    dw_mx2.SetItem(1, 'dlsxfl', dlsxfl);
                }
                requestor.Close();
            };
        ...
        this.sle_EditChanged = function (text) {
            ...
            if (text == "") {
                ...
                dw_1.SetFilter("( Upper(dlxyh) like '%" + text.toUpperCase() + "%') or ( Lower(dlxyh) like '%" + text.toLowerCase() + "%') or ( dlwtfmc like '%" + text + "%')");
                dw_1.Filter();
                dw_1.Sort();               
            }
            dw_1.SetRedraw(true);
        ...
        this.Retrieve = function () {
            ...
            dw_1.Retrieve(userid,beginDate.getVarDate(), endDate.getVarDate());
