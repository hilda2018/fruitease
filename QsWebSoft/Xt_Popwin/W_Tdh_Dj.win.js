...
//#endregion
    ...
    this.save = function () {
        ...
        var tdh = dw_master.GetItemString(1, "tdh");
        if (tdh == null || tdh == "") {
            alert("请输入提单号!");
            dw_master.SetFocus();
            dw_master.SetColumn("tdh")
            return;
        }
        ...
        var ret = QsWebSoft.PubMethod.UpdateTyzssTdh(mxdbh, tdh).value;
            if (ret > 0) {

                requestor.MessageBox("提示", "提单号保存成功!", ICON.Information);
                dw_master.ResetUpdate();
                requestor.Close();
            }
            else {
                requestor.MessageBox("提示", "提单号保存失败!", ICON.StopSign);
            }
