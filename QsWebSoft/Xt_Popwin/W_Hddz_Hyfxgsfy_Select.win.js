...
function W_Hddz_Hyfxgsfy_Select() {
    ...
    //获取数据窗口
     ...
     this.OK = function () {
         ...
         //空运车队配货信息
         if (dwo == "dw_hddz_hdfyhyfxgsfy_edit_cmd") {
             self.Ky_Cdph();
         };    
     ...
     this.Ky_Cdph = function () {
         ...
         for (row = 1; row <= dw_1.RowCount(); row++) {
             ...
             hgfxsj = dw_1.GetItemDate(row, "hgfxsj");

             if (new Date(hgfxsj) > new Date("1900/1/1")) {
                 dw_d.SetItem(ll_insertrow, "hgfxsj", dw_1.GetItemDate(row, "hgfxsj"));
             ...
             fkcs = QsWebSoft.PubMethod.ReadFycs(ywbh, "010401", sqdbh).value;

             for (var rowfind = 1; rowfind <= dw_d.RowCount(); rowfind++) {
                 if (dw_d.Find("ywbh = '" + ywbh + "' and fybm = '010401'", rowfind, rowfind) > 0) {
                     fkcs++
                 }
             };

             dw_d.SetItem(ll_insertrow, "fkcs", fkcs + 1)
             dw_d.SetItem(ll_insertrow, "fybm", "010401");
             ...
             ywbh = dw_1.GetItemString(row, "ywbh") 
             fkcs = QsWebSoft.PubMethod.ReadFycs(ywbh, "010402", sqdbh).value;

             for (var rowfind = 1; rowfind <= dw_d.RowCount(); rowfind++) {
                 if (dw_d.Find("ywbh = '" + ywbh + "' and fybm = '010402'", rowfind, rowfind) > 0) {
                     fkcs++
                 }
             };

             dw_d.SetItem(ll_insertrow, "fkcs", fkcs + 1)
             dw_d.SetItem(ll_insertrow, "fybm", "010402");
