    ...
    //获取数据窗口
     ...
     this.Ky_Cdph = function () {
         ...
         for (row = 1; row <= dw_1.RowCount(); row++) {
             ...
             dw_d.SetItem(ll_insertrow, "djlx", dw_1.GetItemString(row, "djlx"));           
             count++;
	 ...
	 this.sle_EditChanged = function (text) {
	     ...
	     if (text == "") {
	         ...
	         dw_1.SetFilter("( Upper(hth) like '%" + text.toUpperCase() + "%')  or ( Lower(hth) like '%" + text.toLowerCase() + "%') or ( Upper(ztdh) like '%" + text.toUpperCase() + "%') or ( Lower(ztdh) like '%" + text.toLowerCase() + "%') or ( Upper(bgdh) like '%" + text.toUpperCase() + "%') or ( Lower(bgdh) like '%" + text.toLowerCase() + "%') or ( Upper(khmc) like '%" + text.toUpperCase() + "%') or ( Lower(khmc) like '%" + text.toLowerCase() + "%') or ( Upper(shdwmc) like '%" + text.toUpperCase() + "%') or ( Lower(shdwmc) like '%" + text.toLowerCase() + "%')");
	         dw_1.Filter();
	         dw_1.Sort();
	     }
	     dw_1.SetRedraw(true);
