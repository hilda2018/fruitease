...
function W_Bdsbh_Select() {
    ...
    this.OK = function () {
        ...
        if (dw_mx != null) {
            ...
            if (column == "wlgz_bdsbh") {
                dw_mx.SetItem(1, 'bdsbh', bdsbh);
                dw_mx.SetItem(1, 'bdyssbh', bdyssbh);
                dw_mx.SetItem(RowData, 'zxcph', zxcph);
                requestor.Close();
            };

            if (column == "sj_bdsbh") {
                for (row = dw_mx.RowCount(); row > 0; row--) {
                    var bdsbh_row = dw_mx.GetItemString(row, "bdsbh");
                    if (bdsbh_row == bdsbh) {
                        dw_mx.SetItem(row, "bdsbh", "");
                        dw_mx.SetItem(row, "bdyssbh", "")
                    }
                };
            ...
            if (column == "kysj_bdsbh") {
                dw_mx.SetItem(RowData, 'bdsbh', bdsbh);
                dw_mx.SetItem(RowData, 'bdyssbh', bdyssbh);
                requestor.Close();
            };
            if (column == "kyph_bdsbh") {
                dw_mx.SetItem(RowData, 'bdsbh', bdsbh);
                dw_mx.SetItem(RowData, 'bdyssbh', bdyssbh);
                dw_mx.SetItem(RowData, 'zxcph', zxcph);
                requestor.Close();
        ...
        this.sle_EditChanged = function (text) {
            ...
            if (text == "") {
                ...
                dw_1.SetFilter("( Upper(bdsbh) like '%" + text + "%') or ( Lower(bdyssbh) like '%" + text + "%') ");
                dw_1.Filter();
                dw_1.Sort();
            }
            dw_1.SetRedraw(true);
        ...
        this.Retrieve = function () {
            ...
            var hyky = requestor.GetParm("hyky");
