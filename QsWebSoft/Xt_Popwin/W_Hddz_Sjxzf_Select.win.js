...
//         requestor.MessageBox("", "1111")
         ...
         if (dwo == "dw_hddz_hdfysjxzf_edit_cmd") {
             self.Ky_Cdph();
         };    
     ...
     this.Ky_Cdph = function () {
        ...
        for (row = 1; row <= dw_1.RowCount(); row++) {
            ...
            fkcs = QsWebSoft.PubMethod.ReadJzxFycs(ywbh, jzxh, "010302", sqdbh).value;

            for (var rowfind = 1; rowfind <= dw_d.RowCount(); rowfind++) {
                if (dw_d.Find("ywbh = '" + ywbh + "' and jzxh = '" + jzxh + "' and fybm = '010302'", rowfind, rowfind) > 0) {
                    fkcs++
                }
            };

            dw_d.SetItem(ll_insertrow, "fkcs", fkcs + 1)
            dw_d.SetItem(ll_insertrow, "fybm", "010302");

            dw_d.SetItem(ll_insertrow, "je", 200);
            count++;
