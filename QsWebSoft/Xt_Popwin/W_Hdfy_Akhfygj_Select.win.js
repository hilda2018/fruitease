...
function W_Hdfy_Akhfygj_Select() {
    ...
    //获取数据窗口
     ...
     this.OK = function () {
         ...
         //空运车队配货信息
         if (dwo == "dw_hddz_akhfygj_edit_cmd") {
             self.Ky_Cdph();
         };    
     ...
     this.Ky_Cdph = function () {
         ...
         for (row = 1; row <= dw_1.RowCount(); row++) {
             ...
             var sqdbh = dw_1.GetItemString(row, "sqdbh");
             var cxh = dw_1.GetItemNumber(row, "cxh");
             var li_find = dw_d.Find("sqdbh = '" + sqdbh + "' and cxh_sqdbh=" + cxh + "", 1, dw_d.RowCount());
             if (li_find > 0) {
                 continue;
             }
             ...
             ll_insertrow = dw_d.InsertRow(0);
             dw_d.SetItem(ll_insertrow, "sqdbh", dw_1.GetItemString(row, "sqdbh"));
             dw_d.SetItem(ll_insertrow, "ywbh", dw_1.GetItemString(row, "ywbh"));
             dw_d.SetItem(ll_insertrow, "cxh_sqdbh", dw_1.GetItemNumber(row, "cxh"));
             dw_d.SetItem(ll_insertrow, "cxh_hddz", dw_1.GetItemNumber(row, "hddz_cxh"));
             dw_d.SetItem(ll_insertrow, "fybm", dw_1.GetItemString(row, "fybm"));
             dw_d.SetItem(ll_insertrow, "fymc", dw_1.GetItemString(row, "fymc"));
             dw_d.SetItem(ll_insertrow, "fyje", dw_1.GetItemNumber(row, "je"));
