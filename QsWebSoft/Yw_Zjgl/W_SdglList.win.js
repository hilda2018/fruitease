...
function W_SdglList() {
	 ...
	 var rb_wcf= new RadioButton( requestor );
	 var rb_ycf= new RadioButton( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(rb_wcf == null ) rb_wcf= new RadioButton( requestor );
		 rb_wcf.Attach( win.Control( "rb_wcf") ); 
		 if(rb_ycf == null ) rb_ycf= new RadioButton( requestor );
		 rb_ycf.Attach( win.Control( "rb_ycf") ); 
		 ...
		 if(typeof(self.Rb_Selected)=="function") 
			 win.AttachEvent("rb_wcf","Clicked",self.Rb_Selected); 

		 if(typeof(self.Rb_Selected)=="function") 
			 win.AttachEvent("rb_ycf","Clicked",self.Rb_Selected); 
	 ...
	 if(typeof(baseUnBind)=="function") baseUnBind(); 
		 ...
		 rb_wcf.Detach(); rb_wcf=null; 
		 rb_ycf.Detach(); rb_ycf=null; 
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(userid, Dlwtf, beginDate.getVarDate(), endDate.getVarDate());
        self.Rb_Selected();
    }
    //#endregion
    ...
    this.OpenWindow = function () {
        ...
        if (dw_list.RowCount() <= 0)
            return;
        var sdbh = dw_list.GetItemString(dw_list.GetRow(), 'sdbh')
//        var status = dw_list.GetItemString(dw_list.GetRow(), 'state')
        var operation
//        if (status == "审核通过" & status == "付款") {
//            operation = "show";
//        }
//        else {
            operation = "open";
//        };
        ...
        if (openStyle == "2")
            ...
            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&sdbh=' + sdbh + '&operation=' + operation.toString(), true);
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(sdbh) like '%" + text.toUpperCase() + "%')  or  ( Upper(khmc) like '%" + text.toUpperCase() + "%')");
        dw_list.Filter();
        dw_list.Sort();
    ...
    //#region Rb选择
    this.Rb_Selected = function () {
        dw_list.SetRedraw(false);
        if (rb_wcf.GetChecked() == true) {
            dw_list.SetFilter("full_flag='N' or isnull(full_flag)");
        }
        else {
            dw_list.SetFilter("full_flag='Y'");
        }
