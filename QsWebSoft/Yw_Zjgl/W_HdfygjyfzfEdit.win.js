...
function W_HdfygjyfzfEdit() {
    ...
    //#region 变量定义
    var editMode = "";
    var iw_Hddz_Gjyfzf_Select = null;
    ...
    webReq.SetWebServiceUrl("/Service/Hdfygjyfzf.ashx");
        ...
        if (operation == "new" || operation == "copy" || operation == "open" || operation == "modify") {
            ...
            if (operation == "open") {
                ...
                for (row = dw_jzxxx.RowCount(); row > 0; row--) {
                    var zfdxjc = dw_jzxxx.GetItemString(row, "zfdxjc");
                    var jsdwjc = dw_jzxxx.GetItemString(row, "jsdwjc");
                    if ((zfdxjc == "" || zfdxjc == null) && length(jsdwjc) > 0) {
                        dw_jzxxx.SetItem(row, "zfdxbm", dw_jzxxx.GetItemString(row, "jsdwbm"));
                        dw_jzxxx.SetItem(row, "zfdxmc", dw_jzxxx.GetItemString(row, "jsdwmc"));
                        dw_jzxxx.SetItem(row, "zfdxjc", dw_jzxxx.GetItemString(row, "jsdwjc"));
                        dw_jzxxx.SetItem(row, "zfdxpym", dw_jzxxx.GetItemString(row, "jsdwpym"));
                    }
    ...
    this.Save = function () {
        ...
        for (var i = 1; i <= dw_jzxxx.RowCount(); i++) {
            if (dw_jzxxx.GetItemString(i, "zfdxjc") == '' || dw_jzxxx.GetItemString(i, "zfdxjc") == null) {
                alert("结算部门不能为空!");
                return;
            }
        }
        ...
        for (row = dw_jzxxx.RowCount(); row > 0; row--) {
            ...
            if (row == dw_jzxxx.RowCount()) {
                var hz_ywbh = dw_jzxxx.GetItemString(row, "ywbh");
                var hz_hth = dw_jzxxx.GetItemString(row, "hth");

            }
            else {
                var ywbh = dw_jzxxx.GetItemString(row, "ywbh");
                var hth = dw_jzxxx.GetItemString(row, "hth");
                if (hz_ywbh.indexOf(ywbh) < 0) {
                    hz_ywbh = hz_ywbh + "," + ywbh
                };
                if (hz_hth.indexOf(hth) < 0) {
                    hz_hth = hz_hth + "," + hth
                };

            };

            var fph = dw_jzxxx.GetItemString(row, "fph");
            var ywbh = dw_jzxxx.GetItemString(row, "ywbh");
            var num = QsWebSoft.PubMethod.Zpsfzf(fph, ywbh, "Y");


        }

        dw_master.SetItem(1, "hz_ywbh", hz_ywbh);
        dw_master.SetItem(1, "hz_hth", hz_hth);
        ...
        var ywbh = dw_master.GetItemString(1, "sqdbh");
        ...
        dw_log.SetItem(row, "mainid", sqdbh);
        ...
        webReq.Invoke("Save", "dw_master=" + dw_master_data + "&dw_jzxxx=" + dw_jzxxx_data + "&dw_log=" + dw_log_data + "&sqdbh=" + sqdbh + "&operation=" + editMode);
        ...
        if (webReq.StatusCode() == 200) {
            ...
            for (row = 1; row <= dw_log.RowCount(); row++) {
                dw_log.SetItem(row, "mainid", sqdbh); 
            }
    ...
    this.InsertRow = function () {
        if (iw_Hddz_Gjyfzf_Select == null)
            iw_Hddz_Gjyfzf_Select = new W_Hddz_Gjyfzf_Select();
        var ShareMode = requestor.GetParm("ShareMode");
        var ywy = requestor.GetParm("userid");
        var Dlwtf = requestor.GetParm("Dlwtf");
        row = dw_jzxxx.InsertRow(dw_jzxxx.GetRow());
        var dwo = dw_jzxxx.GetDataObject();
        iw_Hddz_Gjyfzf_Select.SetRow(row);
        iw_Hddz_Gjyfzf_Select.SetDwo(dwo);
        iw_Hddz_Gjyfzf_Select.SetDetailDW(dw_jzxxx);
        requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Hddz_Gjyfzf_Select&ShareMode=" + ShareMode + '&ywy=' + ywy + '&Dlwtf=' + Dlwtf, iw_Hddz_Gjyfzf_Select);
    ...
    this.DeleteRow = function () {
        ...
        var fph = lastdw.GetItemString(row, "fph");
        var ywbh = lastdw.GetItemString(row, "ywbh");
        var num = QsWebSoft.PubMethod.Zpsfzf(fph, ywbh, "N");
    ...
    this.Print = function () {
        ...
        dw_print.SetDataObject("dw_hddz_hdfygjyfzf_print");
    ...
    this.dw_master_Clicked = function (xPos, yPos, Row, dwoName) {
        ...
        if (dwoName == "sjskrbm_t") {
            ...
            iw_Wldw_Select.SetData("hdfygjyfzf_sjskr");
        ...
        if (dwoName == "sjskrkhyh_t") {
            ...
            iw_Wldw_Yh_Select.SetData("hdfygjyfzf_sjskryh");
    ...
    this.dw_jzxxx_Clicked = function (xPos, yPos, Row, dwoName) {
        ...
        if (dwoName == "zfdx_jsbm") {
            ...
            iw_Wldw_Jsbm_Select.SetData("gjyfzf_jsdw");
    ...
    this.dw_jzxxx_ItemChanged = function (row, dwoName, data) {
        if (dwoName == "fybm") {        
            ...
            if (ysfs == "海运") {
                ...
                for (var rowfind = 1; rowfind <= dw_jzxxx.RowCount(); rowfind++) {
                    if (dw_jzxxx.Find("ywbh = '" + ywbh + "' and tpxx = "+tpxx+" and zhlx = '"+zhlx+"' and jzxh = '" + jzxh + "' and fybm = '" + data + "'", rowfind, rowfind) > 0) {
                        fkcs++
                    }
                };
            }           
