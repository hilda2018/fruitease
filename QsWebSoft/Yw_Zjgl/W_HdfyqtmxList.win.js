...
function W_HdfyqtmxList() {
    ...
    this.Cxbp = function () {
        ...
        if (row <= 0)
            return;
        var ysqr = dw_list.GetItemString(row, "ysqr");
        var userid = requestor.GetParm("userid");
        if (userid == "4015") {
            if (ysqr == "Y") {
                ...
                if (state != "提交") {
                    ...
                    requestor.MessageBox("提示", "本单据没有提交，不能撤销!")
                    return
                }
                var sqdbh = dw_list.GetItemString(row, "sqdbh")

                var num = QsWebSoft.PubMethod.HdqtmxfyCx(sqdbh).value;
                if (num == "1") {
            ...
            requestor.MessageBox("提示", "没有撤销权限，请联系陈贤！")
        }
    }
    //#endregion
...
//    }
    ...
    this.Delete = function () {
        ...
        var sqdbh = dw_list.GetItemString(row, "sqdbh");
        if (requestor.MessageBox("提示", "删除检疫费用申请单号为<" + sqdbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_HdfyqtmxList", "dw_hddz_hdfyqtmx_list");
        self.Retrieve();
    }
    //#endregion
 ...
 //#region 提交
    this.Tj = function () {
        ...
        var num = QsWebSoft.PubMethod.GenFksqdJzxxxQtmx(sqdbh).value;
        if (num == "1") {
