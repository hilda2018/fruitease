...
function W_KhwqrzdList() {
    ...
    webReq.SetWebServiceUrl("/Service/Khwqrzd.ashx");
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(),Dlwtf, fklx);
    }
    //#endregion
...
//            }
    ...
    this.slefymc_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(cmd_fymc) like Upper('%" + text.toUpperCase() + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.dw_list_ItemChanged = function (row, dwoName, data) {
        if (dwoName == "cmd_ecgjqr" && data == "Y") 
        {
            var cmd_ecgjje = dw_list.GetItemNumber(row, "cmd_ecgjje")
            
            if (cmd_ecgjje == null || cmd_ecgjje == 0) {
                requestor.MessageBox("提示", "注意再次确认金额是否为0")
                dw_list.SetColumn("cmd_ecgjje")                
            }
            var date = new Date;
            dw_list.SetItem(row, "cmd_ecgjsj", date.getVarDate());
            dw_list.SetItem(row, "sfxg", "Y");
        }       
