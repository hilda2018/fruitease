...
function W_HdfyysfyqrList() {
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), jsdwqr, userid, jdr);
...
//    }
    ...
    this.btn_qx_Clicked = function (text) {
        ...
        for (var row = dw_list.RowCount(); row > 0; row--) {
            ...
            //            var num = QsWebSoft.PubMethod.sfy_yshdfygj(ywbh, jzxh, ztdh).value;
            //            if (num == 1) {
            //                alert("此数据已归集不能去掉。");
            //                return;
            //            }
    ...
    this.ddlb_jdr_SelectionChanged = function () {
        ...
        if (jdr == "全部") {
            ...
            dw_list.SetFilter("( Upper(yw_hddz_jdrjc) like Upper('%" + jdr + "%'))")
        };
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_HdfyysfyqrList", "dw_hddz_ysfyqr_list");
        self.Retrieve();
    }
    //#endregion
    ...
    this.Save = function () {
        ...
        for (var i = 1; i <= dw_list.RowCount(); i++) {
            if (dw_list.GetItemStatus(i, "ysqr", DWBUFFER.Primary) == 1) {
                if (dw_list.GetItemString(i, "ysqr") == "Y" && dw_list.GetItemString(i, "skfs") == "1") {
                    var sqdbh = dw_list.GetItemString(i, "sqdbh");
                    var li_find = dw_log.Find("mainid='" + sqdbh + "'", 1, dw_log.RowCount());
                    if (li_find < 1) {
                        var row = dw_log.InsertRow(0);
                        var ywy = requestor.GetParm("userid");

                        dw_log.SetItem(row, "eid", 'ysqr' + ywy + uuid(8, 16));
                        dw_log.SetItem(row, "tablename", "yw_hddz_fksqd_jzxxx");
                        dw_log.SetItem(row, "changecols", "ysje");
