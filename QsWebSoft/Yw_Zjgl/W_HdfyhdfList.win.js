...
function W_HdfyhdfList() {
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(typeof(self.slehth_EditChanged)=="function") 
			 win.AttachEvent("sle_1","EditChanged",self.slehth_EditChanged); 
		 ...
		 if(typeof(self.Tj)=="function") 
			 win.AttachEvent("btn_1","Clicked",self.Tj); 

		 if(typeof(self.Cxbp)=="function") 
			 win.AttachEvent("btn_cxbp","Clicked",self.Cxbp); 
...
//        }
        ...
        if (requestor.MessageBox("提示", "删除换单费申请单号为<" + sqdbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;
    ...
    this.Print = function () {
        ...
        dw_list.SetItem(dw_list.GetRow(), "sfdy", "Y");
    ...
    this.Cxbp = function () {
        ...
        if (sfsp == "Y") {
            ...
            if (state != "提交") {
                ...
                requestor.MessageBox("提示", "本单据没有提交，不能再提交!")
                return
            }
            var sqdbh = dw_list.GetItemString(row, "sqdbh")

            var num = QsWebSoft.PubMethod.GenFksqdHdf(sqdbh).value;
            if (num == "1") {
