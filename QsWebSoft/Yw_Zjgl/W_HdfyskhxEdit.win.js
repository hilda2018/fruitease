...
function W_HdfyskhxEdit() {
    ...
    this.Save = function () {
        ...
        var skd_cxh = dw_master.GetItemNumber(1, "cmd_cxh");

        var skje_sum = dw_jzxxx.GetItemNumber(1, "skje_sum")
        var zrmbhl = dw_master.GetItemNumber(1, "zrmbhl")
        var skje = dw_master.GetItemNumber(row, "cmd_skje");
        ...
        if (skje_sum == skje) {
            dw_master.SetItem(1, "cmd_hxwcbz", "Y")
        }
        else {
            dw_master.SetItem(1, "cmd_hxwcbz", "N")
        }
        var fymc = ""
        ...
        var jdrjc = dw_master.GetItemString(1, "yw_hddz_sjskd_cmd_jdrjc");
        
        var hz_jzxh = ""
        for (row = dw_jzxxx.RowCount(); row > 0; row--) {
            dw_jzxxx.SetItem(row, "skdbh", skdbh);
            dw_jzxxx.SetItem(row, "skd_cxh", skd_cxh);
            var jdrjc1 = dw_jzxxx.GetItemString(row, "yw_hddz_jdrjc");            
            if (jdrjc1 != jdrjc) {
                alert("核销明细的接单人与上面的接单人不一致，请查证重写！！");
                return;
            }
        }
...
//        }
        ...
        webReq.Invoke("Save", "dw_master=" + dw_master_data + "&dw_jzxxx=" + dw_jzxxx_data + "&skdbh=" + skdbh + "&skd_cxh=" + skd_cxh + "&operation=" + editMode);

        if (webReq.StatusCode() == 200) {
            //服务器成功处理，返回单号
            sqdbh = webReq.ResponseText();
            dw_master.SetItem(1, "skdbh", skdbh);
            dw_master.SetItem(1, "cmd_cxh", skd_cxh);
    ...
    this.InsertRow = function () {
        ...
        if (iw_Hddz_Skhx_Select == null)
            iw_Hddz_Skhx_Select = new W_Hddz_Skhx_Select();
        var ShareMode = requestor.GetParm("ShareMode");
        var ywy = requestor.GetParm("userid");
        var Dlwtf = requestor.GetParm("Dlwtf");
        var row = dw_jzxxx.InsertRow(dw_jzxxx.GetRow());
        var dwo = dw_jzxxx.GetDataObject();
        var jsdwbm = dw_master.GetItemString(1, "cmd_jsdwbm")
        var jsbm = dw_master.GetItemNumber(1, "cmd_jsbm")
        var fydlbm = dw_master.GetItemString(1, "cmd_fydlbm")
        var cmd_skje = dw_master.GetItemNumber(1, "cmd_skje")
        var jdrjc = dw_master.GetItemString(1, "yw_hddz_sjskd_cmd_jdrjc");
        jdrjc = escape(jdrjc);
        ...
        var skje = cmd_skje - skje_sum;

        iw_Hddz_Skhx_Select.SetRow(row);
        iw_Hddz_Skhx_Select.SetDwo(dwo);
        iw_Hddz_Skhx_Select.SetDetailDW(dw_jzxxx);
        if (jsbm == null || jsbm == "") {
            jsbm = 0
        }
        ...
        if (num == "[object Object]") {
           requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Hddz_Skhx_Select&ShareMode=" + ShareMode + '&ywy=' + ywy + '&Dlwtf=' + Dlwtf + '&jsdwbm=' + jsdwbm + '&jsbm=' + jsbm + '&fydlbm=' + fydlbm + '&cmd_skje=' + skje + '&jdrjc=' + jdrjc, iw_Hddz_Skhx_Select);
         }
    }
    //#endregion
    ...
    this.AddRow = function () {
        ...
        var jsdwbm = dw_master.GetItemString(1, "cmd_jsdwbm")
        var jsbm = dw_master.GetItemNumber(1, "jsbm_cxh")
        var fydlbm = dw_master.GetItemString(1, "cmd_fydlbm")
        var cmd_skje = dw_master.GetItemNumber(1, "cmd_skje")
        var jdrjc = dw_master.GetItemString(1, "yw_hddz_sjskd_cmd_jdrjc")
        jdrjc = escape(jdrjc);
        ...
        var jdrjc1 = escape(dw_master.GetItemString(1, "yw_hddz_sjskd_cmd_jdrjc"));
