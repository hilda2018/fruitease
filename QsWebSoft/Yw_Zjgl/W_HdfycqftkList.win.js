...
function W_HdfycqftkList() {
	 ...
	 var ddlb_tkzt= new DropDownListBox( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(ddlb_tkzt == null ) ddlb_tkzt= new DropDownListBox( requestor );
		 ddlb_tkzt.Attach( win.Control( "ddlb_tkzt") ); 
		 ...
		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("ddlb_tkzt","SelectionChanged",self.Retrieve); 
	 ...
	 if(typeof(baseUnBind)=="function") baseUnBind(); 
		 ...
		 ddlb_tkzt.Detach(); ddlb_tkzt=null; 
    ...
    webReq.SetWebServiceUrl("/Service/Hdfycqftk.ashx");   
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), tkzt, zt);
    ...
    this.dw_list_ItemChanged = function (row, dwoName, data) {
        if (row > 0) {
            if (dwoName == "cgsclbz") {
                ...
                if (data == "Y") {
                    dw_list.SetItem(row, "cgsclsj", date.getVarDate())
                } 
                else {
                    dw_list.SetItem(row, "cgsclsj", date_null.getVarDate())
                }
            }
            if (dwoName == "fxgsclbz") {
                var date = new Date()
                var date_null = new Date("1900/01/01")
                if (data == "Y") {
                    dw_list.SetItem(row, "fxgsclsj", date.getVarDate())
                }
                else {
                    dw_list.SetItem(row, "fxgsclsj", date_null.getVarDate())
                }
            }
