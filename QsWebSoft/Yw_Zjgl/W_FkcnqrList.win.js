...
function W_FkcnqrList() {
    ...
    var sle_sqrbh = new SingleLineEdit(requestor);
    var sle_sjskrmc = new SingleLineEdit(requestor);
    var ddlb_sqrgs = new DropDownListBox(requestor);
    var tb_1 = new ToolStrip(requestor);
    var ddlb_fklx = new DropDownListBox(requestor);
    ...
    this.bind = function (win) {
        ...
        if (sle_sqrbh == null) sle_sqrbh = new SingleLineEdit(requestor);
        sle_sqrbh.Attach(win.Control("sle_sqrbh"));
        if (sle_sjskrmc == null) sle_sjskrmc = new SingleLineEdit(requestor);
        sle_sjskrmc.Attach(win.Control("sle_sjskrmc"));
        if (ddlb_sqrgs == null) ddlb_sqrgs = new DropDownListBox(requestor);
        ddlb_sqrgs.Attach(win.Control("ddlb_sqrgs"));
        if (tb_1 == null) tb_1 = new ToolStrip(requestor);
        tb_1.Attach(win.Control("tb_1"));
        if (ddlb_fklx == null) ddlb_fklx = new DropDownListBox(requestor);
        ddlb_fklx.Attach(win.Control("ddlb_fklx"));
        ...
        if (typeof (self.slesqrbh_EditChanged) == "function")
            win.AttachEvent("sle_sqrbh", "EditChanged", self.slesqrbh_EditChanged);

        if (typeof (self.slesjskrmc_EditChanged) == "function")
            win.AttachEvent("sle_sjskrmc", "EditChanged", self.slesjskrmc_EditChanged);

        if (typeof (self.SqrGs) == "function")
            win.AttachEvent("ddlb_sqrgs", "SelectionChanged", self.SqrGs);
        ...
        if (typeof (self.Retrieve) == "function")
            win.AttachEvent("ddlb_fklx", "SelectionChanged", self.Retrieve);

        if (typeof (self.Back) == "function")
            win.AttachEvent("btn_back", "Clicked", self.Back);
    ...
    this.unbind = function () {
        ...
        sle_sqrbh.Detach(); sle_sqrbh = null;
        sle_sjskrmc.Detach(); sle_sjskrmc = null;
        ddlb_sqrgs.Detach(); ddlb_sqrgs = null;
        tb_1.Detach(); tb_1 = null;
        ddlb_fklx.Detach(); ddlb_fklx = null;
    ...
    webReq.SetWebServiceUrl("/Service/Fkcnqr.ashx");
    ...
    this.Retrieve = function () {
        ...
        var fklx = ddlb_fklx.GetText();
        var sqrgs = ddlb_sqrgs.GetText();
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), fklx, sqrgs,userid);
    }
    //#endregion
    ...
    this.Back = function () {
        ...
        var dwo = dw_list.GetItemString(row, "fydlmc");
        var sqdbh = dw_list.GetItemString(row, "sqdbh");
        if (dwo == "非业务费用") {
            var num = QsWebSoft.PubMethod.UpdateFywfy(sqdbh).value;
            if (num > 0) {
                requestor.MessageBox("", "撤销成功!");
            }
        } else {
            requestor.MessageBox("", "该票不是非业务费用,无法撤消!");
            return;
        }
    }
    //#endregion
    ...
    this.OpenWindow = function () {
        ...
        if (fydl == "0103") {
            ...
            requestor.MessageBox("提示","对应费用类型没有，不能打开");
            return;
        }
    ...
    this.dw_List_DoubleClicked = function (xPos, yPos, Row, dwoName) {
        if (Row > 0) {
            if (dwoName == "zfyh" || dwoName == "zfyhzh") {
                if (iw_Wldw_Yh_Select == null)
                    iw_Wldw_Yh_Select = new W_Wldw_Yh_Select();
                var sqrgsbm = dw_list.GetItemString(Row, "sqrgsbm");
                var ywy = requestor.GetParm("userid");
                var ShareMode = requestor.GetParm("ShareMode");
                var Dlwtf = requestor.GetParm("Dlwtf");
                iw_Wldw_Yh_Select.SetDataWindow(dw_list);
                iw_Wldw_Yh_Select.SetData("fkcnqr_zfyh");
                iw_Wldw_Yh_Select.SetRow(Row)
                requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Wldw_Yh_Select&ShareMode=" + ShareMode + "&ywy=" + ywy + "&Dlwtf=" + Dlwtf + "&yw_khbm=" + sqrgsbm, iw_Wldw_Yh_Select);
                dw_list.SetFocus();
            } else {
                self.OpenWindow();
            }
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(sqdbh) like Upper('%" + text.toUpperCase() + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    //#region 过滤申请人编号
    this.slesqrbh_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(zbr) like Upper('%" + text + "%')) or ( Upper(zbrmc) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    //#region 过滤实际收款人名称
    this.slesjskrmc_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(sjskrjc) like Upper('%" + text + "%')) or ( Upper(sjskrmc) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.dw_list_ItemChanged = function (row, dwoName, data) {
        if (dwoName == "zrmbhl") {
            var zrmbje = data * dw_list.GetItemNumber(row, "je");
            zrmbje = parseFloat(zrmbje.toFixed(2));
            dw_list.SetItem(row, "zrmbje", zrmbje);
        }
    ...
    this.Save = function () {
        ...
        for (var i = 1; i <= dw_list.RowCount(); i++) {
            if (dw_list.GetItemStatus(i, 0, DWBUFFER.Primary) == 1) {
                var ywbh = dw_list.GetItemString(i, "sqdbh");
                var row = dw_log.InsertRow(0);
                var ywy = requestor.GetParm("userid");

                var serialNumber = "fkcn" + ywy + uuid(8, 16);

                dw_log.SetItem(row, "eid", serialNumber);
                dw_log.SetItem(row, "tablename", "yw_hddz_fksqd");
                dw_log.SetItem(row, "changecols", "zfsj");
