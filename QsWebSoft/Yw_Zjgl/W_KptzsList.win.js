...
function W_KptzsList() {
    ...
    this.OpenWindow = function () {
        ...
        if (dw_list.RowCount() <= 0)
            return;
        var id = dw_list.GetItemString(dw_list.GetRow(), 'id')
        ...
        if (openStyle == "2")
           ...
           ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&id=' + id  + '&operation=' + operation.toString(), true);
...
//    }
    ...
    this.Delete = function () {
        ...
        if (requestor.MessageBox("提示", "删除开票通知书<" + id + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;

        //执行删除的WebService
        webReq.SetFunctionID(requestor.GetFunctionID());
        webReq.Invoke("Delete", "id=" + id );
    ...
    this.Print = function () {
        ...
        if (row <= 0)
            return;
        dw_print.SetDataObject("dw_ck_kptzs_prn");
        dw_print.Retrieve(dw_list.GetItemString(dw_list.GetRow(), "id"));
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(id) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_KptzsList", "dw_ck_kptzs_list");
        self.Retrieve();
    }
    //#endregion
