...
function W_HdfyhycdfyList() {
    ...
    var ddlb_cdfyzt = new DropDownListBox(requestor);
    ...
    this.bind = function (win) {
        ...
        if (ddlb_cdfyzt == null) ddlb_cdfyzt = new DropDownListBox(requestor);
        ddlb_cdfyzt.Attach(win.Control("ddlb_cdfyzt"));
        ...
        if (typeof (self.Retrieve) == "function")
            win.AttachEvent("ddlb_cdfyzt", "SelectionChanged", self.Retrieve);
    ...
    this.unbind = function () {
        ...
        ddlb_cdfyzt.Detach(); ddlb_cdfyzt = null;
    ...
    this.Retrieve = function () {
        ...
        var cdfyzt = ddlb_cdfyzt.GetText();
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), cdfyzt);
    ...
    this.OpenWindow = function () {
        ...
        if (dw_list.RowCount() <= 0)
            return;
        var ywbh = dw_list.GetItemString(dw_list.GetRow(), 'jzxxx_ywbh')
        var cxh = dw_list.GetItemNumber(dw_list.GetRow(), 'cxh')
        var yfkdbh = dw_list.GetItemString(dw_list.GetRow(), 'jzxxx_yfkdbh')
...
//        var status = dw_list.GetItemString(dw_list.GetRow(), 'state')
        ...
        //失效按钮的控制
         var state = dw_list.GetItemString(dw_list.GetRow(), 'jzxxx_state')
        ...
        if (openStyle == "2")
            ...
            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&ywbh=' + ywbh + '&cxh=' + cxh + '&yfkdbh=' + yfkdbh + '&operation=' + operation.toString(), true);
...
//    }
    ...
    this.Delete = function () {
        ...
        if (requestor.MessageBox("提示", "删除海运车队费用申请单号为<" + yfkdbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;

        //执行删除的WebService
        webReq.SetFunctionID(requestor.GetFunctionID());
        webReq.Invoke("Delete", "yfkdbh=" + yfkdbh);
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(jzxxx_yfkdbh) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_HdfyfywfyList", "dw_hddz_hdfyfywfy_list");
        self.Retrieve();
    }
    //#endregion
