...
//    }
    ...
    this.Delete = function () {
        ...
        var userid = requestor.GetParm("userid");
        if ( zbr != userid) {
            requestor.MessageBox("提示", "不是自己的数据,不能被删除!");
            return;
        }

        var sqdbh = dw_list.GetItemString(row, "sqdbh");
        if (requestor.MessageBox("提示", "删除空运车队费用申请单号为<" + sqdbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_HdfykycdfyList", "dw_hddz_hdfykycdfy_list");
        self.Retrieve();
    }
    //#endregion
 ...
 //#region 提交
    this.Tj = function () {
        ...
        var num = QsWebSoft.PubMethod.GenFksqdKycdJzxxx(sqdbh).value;
        if (num == "1") {
