...
function W_HdfyjydfyyfkList()  {
    ...
    this.OpenWindow = function () {
        ...
        if (dw_list.RowCount() <= 0)
            return;
        var yfkdbh = dw_list.GetItemString(dw_list.GetRow(), 'yfkdbh')
//        var status = dw_list.GetItemString(dw_list.GetRow(), 'state')
        ...
        if (openStyle == "2")
            ...
            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&yfkdbh=' + yfkdbh + '&operation=' + operation.toString(), true);
...
//    }
    ...
    this.Delete = function () {
        ...
        var yfkdbh = dw_list.GetItemString(row, "yfkdbh");
        if (requestor.MessageBox("提示", "删除检疫点预录入申请单号为<" + yfkdbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;
    ...
    this.Print = function () {
        ...
        dw_print.SetDataObject("dw_hddz_hdfyjydfyyfk_print");
        dw_print.Retrieve(dw_list.GetItemString(dw_list.GetRow(), "yfkdbh"));
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(yfkdbh) like '%" + text + "%')  or ( Lower(yfkdbh) like '%" + text + "%')");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_HdfyjydfyyfkList", "dw_hddz_hdfyjydfyyfk_list");
        self.Retrieve();
    }
    //#endregion
    ...
    this.Tj = function () {
        ...
        var num = QsWebSoft.PubMethod.GenFksqdScJzxxx(yfkdbh).value;
        if (num == "1") {
