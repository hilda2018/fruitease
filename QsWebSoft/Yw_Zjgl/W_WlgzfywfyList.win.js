...
function W_WlgzfywfyList() {
    ...
    this.bind = function (win) {
        ...
        if (typeof (self.Cxtj) == "function")
            win.AttachEvent("btn_cxtj", "Clicked", self.Cxtj);
    ...
    webReq.SetWebServiceUrl("/Service/Wlgzfywfy.ashx");   
    ...
    this.Retrieve = function () {
        ...
        var wlbjl = requestor.GetParm("Wlbjl");

        dw_list.Retrieve(userid, beginDate.getVarDate(), endDate.getVarDate(), wlbjl);
...
//    }
    ...
    this.Delete = function () {
        ...
        if (requestor.MessageBox("提示", "删除非业务费用申请单号为<" + sqdbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;
    ...
    this.Print = function () {
        ...
        dw_print.SetDataObject("dw_wlgz_fywfy_print");
 ...
 //#region 提交
    this.Tj = function () {
        ...
        var state = dw_list.GetItemString(row, "state")
        if (state == "提交") {
            requestor.MessageBox("提示", "本单据已经提交，不能提交!")
            return
        }
        var sqdbh = dw_list.GetItemString(row, "sqdbh")

        var num = QsWebSoft.PubMethod.GenWlgzFksqd(sqdbh,"提交").value;
        if (num == "1") {
    ...
    this.Cxtj = function () {
        ...
        var num = QsWebSoft.PubMethod.GenWlgzFksqd(sqdbh, "新制").value;
        if (num == "1") {
