...
function W_YfkdList() {
    ...
    //#region 删除
         this.Delete = function () {
             ...
             if (status == "审核通过" || status == "付款" || status == "入账") {
                 requestor.MessageBox("提示", "当前付款申请单的状态为<" + status + ">,不能被删除!", 1);
                 return;
             }

             var sqdbh = dw_list.GetItemString(row, "sqdbh");
             if (requestor.MessageBox("提示", "删除付款申请单<" + sqdbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
                 return;
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(sqdbh) like '%" + text.toUpperCase() + "%')  or ( Lower(wxfph) like '%" + text.toLowerCase() + "%') or ( Lower(sqdbh) like '%" + text.toLowerCase() + "%')  or ( Lower(skdwmc) like '%" + text.toLowerCase() + "%') or ( Upper(skdwmc) like '%" + text.toUpperCase() + "%')");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.Audit = function () {
        ...
        if (status != "新制" && status !="不通过") {
            requestor.MessageBox("提示", "当前付款申请单的状态为<" + status + ">,不需要提交审核!", 1);
            return;
        }

        var sqdbh = dw_list.GetItemString(row, "sqdbh");
        if (requestor.MessageBox("提示", "是否确定需要提交号码为<" + sqdbh + ">的付款申请?", ICON.Question, BUTTON.YesNo) == 2)
            return;
