...
function W_HdfyyswlfyList() {
    ...
    var ddlb_cqfzt = new DropDownListBox(requestor);
    ...
    this.bind = function (win) {
        ...
        if (ddlb_cqfzt == null) ddlb_cqfzt = new DropDownListBox(requestor);
        ddlb_cqfzt.Attach(win.Control("ddlb_cqfzt"));
        ...
        if (typeof (self.ddlb_jdr_SelectionChanged) == "function")
            win.AttachEvent("ddlb_jdr", "SelectionChanged", self.ddlb_jdr_SelectionChanged);
        ...
        if (typeof (self.Retrieve) == "function")
            win.AttachEvent("ddlb_cqfzt", "SelectionChanged", self.Retrieve);
    ...
    this.unbind = function () {
        ...
        ddlb_cqfzt.Detach(); ddlb_cqfzt = null;
    ...
    webReq.SetWebServiceUrl("/Service/Hdfyyswlfy.ashx");   
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), cqfzt, Dlwtf);

    }
    //#endregion

    //#region 过滤接单人
    this.ddlb_jdr_SelectionChanged = function () {
        var jdrjc = ddlb_jdr.GetText();
        dw_list.SetRedraw(false);
        if (jdrjc == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(yw_hddz_jdrjc) like Upper('%" + jdrjc + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.dw_List_ItemChanged = function (row, dwoName, data) {
        if (dwoName == "jzxxx_wlfyqr") {
            var wlfyje_jsl = dw_list.GetItemNumber(row, "wlfyje_jsl")
            var lclfje_jsl = dw_list.GetItemNumber(row, "lclfje_jsl")
            var tsycdzyjsje_jsl = dw_list.GetItemNumber(row, "tsycdzyjsje_jsl")
            var ywbh = dw_list.GetItemString(row, "jzxxx_ywbh");
            var cxh = dw_list.GetItemNumber(row, "jzxxx_cxh");
            var ywy = requestor.GetParm("userid");
            var date = new Date();
            var num = 0;
            if (data = "Y") {
                var jzxxx_wlfyje = dw_list.GetItemNumber(row, "jzxxx_wlfyje")
                if (jzxxx_wlfyje == null || jzxxx_wlfyje == 0) {
                    dw_list.SetItem(row, "jzxxx_wlfyje", wlfyje_jsl)
                }
                var jzxxx_wlfylclfje = dw_list.GetItemNumber(row, "jzxxx_wlfylclfje")
                if (jzxxx_wlfylclfje == null || jzxxx_wlfylclfje == 0) {
                    dw_list.SetItem(row, "jzxxx_wlfylclfje", lclfje_jsl)
                }
                var jzxxx_tsycdzyjsje = dw_list.GetItemNumber(row, "jzxxx_tsycdzyjsje")
                if (jzxxx_tsycdzyjsje == null || jzxxx_tsycdzyjsje == 0) {
                    dw_list.SetItem(row, "jzxxx_tsycdzyjsje", tsycdzyjsje_jsl)
                }
                dw_list.SetItem(row, "jzxxx_wlfyqrrq", date.getVarDate())
                //资金管理注释
                num = QsWebSoft.PubMethod.GenYswlfy(ywbh, cxh, ywy).value;
            } else {
                dw_list.SetItem(row, "jzxxx_wlfyje", 0)
                dw_list.SetItem(row, "jzxxx_wlfylclfje", 0)
                dw_list.SetItem(row, "jzxxx_tsycdzyjsje", 0)
                num = QsWebSoft.PubMethod.GenYswlfy(ywbh, cxh, ywy).value;
            }
        }
    }
    ...
    this.slebgdh_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(bgdh) like v'%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_HdfyyscqfList", "dw_hddz_yscqfy_list");
        self.Retrieve();
    }
    //#endregion
    ...
    this.Save = function () {
        ...
        webReq.Invoke("YswlfyListSave", "dw_list=" + dw_d_data);
