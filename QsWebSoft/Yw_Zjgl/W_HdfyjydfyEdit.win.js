...
function W_HdfyjydfyEdit() {
	 ...
	 var ds_jydfy= new DataStore( requestor );
	 var ds_thzcf= new DataStore( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(ds_jydfy == null ) ds_jydfy= new DataStore( requestor );
		 ds_jydfy.Attach( win.Control( "ds_jydfy") ); 
		 if(ds_thzcf == null ) ds_thzcf= new DataStore( requestor );
		 ds_thzcf.Attach( win.Control( "ds_thzcf") ); 
	 ...
	 if(typeof(baseUnBind)=="function") baseUnBind(); 
		 ...
		 ds_jydfy.Detach(); ds_jydfy=null; 
		 ds_thzcf.Detach(); ds_thzcf=null; 
    ...
    webReq.SetWebServiceUrl("/Service/Hdfyjydfy.ashx");
    ...
    this.Save = function () {
        ...
        if (dw_master.GetItemString(1, "sqrgsmc") == null || dw_master.GetItemString(1, "sqrgsmc") == "") {
            alert(" 申请人公司名称不能为空!");
            return;
        }
    ...
    this.InsertRow = function () {
        if (iw_Hddz_Jydfy_Select == null)
            iw_Hddz_Jydfy_Select = new W_Hddz_Jydfy_Select();
        var ShareMode = requestor.GetParm("ShareMode");
        var ywy = requestor.GetParm("userid");
        var Dlwtf = requestor.GetParm("Dlwtf");
        row = dw_jzxxx.InsertRow(dw_jzxxx.GetRow());
        var dwo = dw_jzxxx.GetDataObject();
        iw_Hddz_Jydfy_Select.SetRow(row);
        iw_Hddz_Jydfy_Select.SetDwo(dwo);
        iw_Hddz_Jydfy_Select.SetDetailDW(dw_jzxxx);
        requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Hddz_Jydfy_Select&ShareMode=" + ShareMode + '&ywy=' + ywy + '&Dlwtf=' + Dlwtf, iw_Hddz_Jydfy_Select);
...
//        dw_jzxxx.InsertRow(0);
    ...
    this.dw_master_Clicked = function (xPos, yPos, Row, dwoName) {
        ...
        if (dwoName == "sqrgsbm_t") {
            ...
            iw_Wldw_Select.SetData("hdfyjydfy_sqrgs");
            requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Wldw_Select&ShareMode=" + ShareMode + "&ywy=" + ywy + "&Dlwtf=" + Dlwtf + "&khlx=" + khlx, iw_Wldw_Select);
            dw_master.SetFocus();
    ...
    this.dw_jzxxx_ItemChanged = function (row, dwoName, data) {
        if (dwoName == "fybm") {
            ...
            var jydjc = dw_jzxxx.GetItemString(row, "jydjc");
            var xx = dw_jzxxx.GetItemString(row, "xx");
            var xl = dw_jzxxx.GetItemString(row, "xl");
            var tgjysj = new Date(dw_jzxxx.GetItemDate(row, "tgjysj"));
            var je = dw_jzxxx.GetItemNumber(row, "je");
            var jydzcts = dw_jzxxx.GetItemNumber(row, "jydzcts");
          

            var jydfy = ds_jydfy.Retrieve(jydjc, xx, xl, data, tgjysj.getVarDate(), je);
            if (jydfy == 0 || jydfy == null) {
        ...
        if (dwoName == "je") {
            ...
            var jydfy = ds_jydfy.Retrieve(jydjc, xx, xl, fybm, tgjysj.getVarDate(), data);
            if (jydfy == 0) {
                dw_jzxxx.SetItem(row, "fysfhl", "Y")
            } else {
                dw_jzxxx.SetItem(row, "fysfhl", "N")
