...
function W_WldwzjqkList()
{
    ...
    var sle_tymc = new SingleLineEdit( requestor );
    var ddlb_hjje = new DropDownListBox( requestor );
    ...
    this.bind = function ( win )
    {
        ...
        if ( sle_tymc == null ) sle_tymc = new SingleLineEdit( requestor );
        sle_tymc.Attach( win.Control( "sle_tymc" ) );
        if ( ddlb_hjje == null ) ddlb_hjje = new DropDownListBox( requestor );
        ddlb_hjje.Attach( win.Control( "ddlb_hjje" ) );
        ...
        if ( typeof ( self.Cxjs ) == "function" )
            win.AttachEvent( "btn_Cxjs", "Clicked", self.Cxjs );
        ...
        if ( typeof ( self.sletymc_EditChanged ) == "function" )
            win.AttachEvent( "sle_tymc", "EditChanged", self.sletymc_EditChanged );

        if ( typeof ( self.DY_Clicked ) == "function" )
            win.AttachEvent( "btn_1", "Clicked", self.DY_Clicked );

        if ( typeof ( self.Retrieve ) == "function" )
            win.AttachEvent( "ddlb_hjje", "SelectionChanged", self.Retrieve );

        if ( typeof ( self.DYCS_Clicked ) == "function" )
            win.AttachEvent( "btn_2", "Clicked", self.DYCS_Clicked );
    ...
    this.unbind = function ()
    {
        ...
        sle_tymc.Detach(); sle_tymc = null;
        ddlb_hjje.Detach(); ddlb_hjje = null;
    ...
    webReq.SetWebServiceUrl("/Service/WlkhZjqk.ashx");   
    ...
    this.Retrieve = function () {
        ...
        var hjje = ddlb_hjje.GetText();

        dw_list.Retrieve(hjje);
    ...
    //#region 双击
    var iw_WldwzjqkBeizhu = null;
    this.dw_List_DoubleClicked = function (Row) {
        //        if (Row > 0)
        //            self.OpenWindow();
        if (Row > 0) {
            if (iw_WldwzjqkBeizhu == null)
                iw_WldwzjqkBeizhu = new W_WldwzjqkBeizhu();

            var Dlwtf = requestor.GetParm("Dlwtf");
            var row = dw_list.GetRow();
            var khmc = dw_list.GetItemString(row, "khmc");
            var jdrbm = dw_list.GetItemString(row, "yw_wldw_zjqk_jdrbm");
            var jdrmc = dw_list.GetItemString(row, "yw_wldw_zjqk_jdrmc");
            var jdrjc = dw_list.GetItemString(row, "yw_wldw_zjqk_jdrjc");
            var jdrpym = dw_list.GetItemString(row, "yw_wldw_zjqk_jdrpym");
            iw_WldwzjqkBeizhu.Setjdrmc(jdrmc);
            iw_WldwzjqkBeizhu.Setjdrjc(jdrjc);
            iw_WldwzjqkBeizhu.SetKhmc(khmc);
            iw_WldwzjqkBeizhu.SetRow(Row);
            requestor.Open("/LoadWindow.ashx?WinName=Yw_Zjgl.W_WldwzjqkBeizhu&jdrbm=" + jdrbm + "&jdrpym=" + jdrpym, iw_WldwzjqkBeizhu);
            //requestor.Open("/LoadWindow.ashx?WinName=Yw_Zjgl.W_WldwzjqkBeizhu&jdrbm=" + jdrbm , iw_WldwzjqkBeizhu);
...
//            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&sqdbh=' + sqdbh + '&operation=' + operation.toString(), true);
    ...
    //打印次数
    this.DYCS_Clicked = function () {
        var Row = dw_list.GetRow();
        if (Row > 0) {
            var khbm = dw_list.GetItemString(Row, "khbm");

            requestor.Open("/LoadWindow.ashx?WinName=Yw_Zjgl.W_WldwzjqkList_Dycs" + '&khbm=' + khbm);
            dw_list.SetFocus();
        }
    }
    //#region 打印
    ...
    //#endregion
    //打印
    this.DY_Clicked = function () {

        var Row = dw_list.GetRow();        
        if (Row > 0) {            
            var khbm = dw_list.GetItemString(Row, "khbm");
            var khmc = dw_list.GetItemString(Row, "khmc");
            var ohsjje = dw_list.GetItemNumber(Row, "ohsjje");
            var ohsjzq = dw_list.GetItemNumber(Row, "ohsjzq");
            var hssjje = dw_list.GetItemNumber(Row, "hssjje");
            var hssjzq = dw_list.GetItemNumber(Row, "hssjzq");
            var hdfyje = dw_list.GetItemNumber(Row, "hdfyje");
            var hdfyzq = dw_list.GetItemNumber(Row, "hdfyzq");
            var gjyfje = dw_list.GetItemNumber(Row, "gjyfje");
            var gjyfzq = dw_list.GetItemNumber(Row, "gjyfzq");

            if (iW_WldwzjqkList_DY == null) {
                iW_WldwzjqkList_DY = new W_WldwzjqkList_DY();
            }
            iW_WldwzjqkList_DY.setKhbm(khbm);
            iW_WldwzjqkList_DY.setKhmc(khmc);
            iW_WldwzjqkList_DY.setOhsjje(ohsjje);
            iW_WldwzjqkList_DY.setOhsjzq(ohsjzq);
            iW_WldwzjqkList_DY.setHssjje(hssjje);
            iW_WldwzjqkList_DY.setHssjzq(hssjzq);
            iW_WldwzjqkList_DY.setHdfyje(hdfyje);
            iW_WldwzjqkList_DY.setHdfyzq(hdfyzq);
            iW_WldwzjqkList_DY.setGjyfje(gjyfje);
            iW_WldwzjqkList_DY.setGjyfzq(gjyfzq);
            var operation = requestor.GetParm("operation");            
            requestor.Open("/LoadWindow.ashx?WinName=Yw_Zjgl.W_WldwzjqkList_DY" + '&operation=' + operation.toString(), iW_WldwzjqkList_DY);
            dw_list.SetFocus();
        }
    }
    ...
    this.slekh_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(khjc) like Upper('%" + text + "%')) or ( Upper(khmc) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    //#region 过滤合同号
    this.sletymc_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(tymc) like Upper('%" + text + "%')) or ( Upper(tyjc) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
...
//            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&ywbh=' + ywbh  + '&operation=' + operation.toString() , true);
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_WldwzjqkList", "dw_wldw_zjqk_list");
        self.Retrieve();
    }
    //#endregion
    ...
    this.Save = function () {
        ...
        webReq.Invoke("save", "dw_list=" + dw_d_data);
    ...
    //#region 过滤取消
    this.Cxjs = function () {
        var num = QsWebSoft.PubMethod.Ccgc("p_wldw_zjqk_list").value;
        if (num == 1) {
            requestor.MessageBox("提示", "执行成功！");
        } else {
            requestor.MessageBox("提示", "执行失败！");
        }
    }
    //#endregion
}
