...
function W_Hdfy_CzdList() {
    ...
    this.OpenWindow = function () {
        ...
        if (dw_list.RowCount() <= 0)
            return;
        var czdbh = dw_list.GetItemString(dw_list.GetRow(), 'czdbh')
...
//        }
        ...
        if (openStyle == "2")
            ...
            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&czdbh=' + czdbh + '&operation=' + operation.toString(), true);
...
//        }
        ...
        var czdbh = dw_list.GetItemString(row, "czdbh");
        if (requestor.MessageBox("提示", "删除出账单编号为<" + czdbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;
        ...
        dw_log.SetItem(row, "mainid", czdbh);
        ...
        webReq.Invoke("Delete", "czdbh=" + czdbh + "&dw_log=" + dw_log_data);
    ...
    this.slekh_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(sjskrmc) like Upper('%" + text + "%')) or ( Upper(sjskrpym) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_Hdfy_YsjjfList", "dw_hddz_ysjjf_list");
        self.Retrieve();
    }
    //#endregion
