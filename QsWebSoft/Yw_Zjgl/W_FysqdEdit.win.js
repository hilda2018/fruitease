...
//            }
            ...
            if (dw_mx.RowCount() > 0) {
                var mxje = dw_mx.GetItemNumber(1, "fyje_sum");
            }
            else {
                mxje = 0;
            }
...
//            dw_master.SetItem(dw_master.GetRow(), "sqfkje", dw_mx.GetItemNumber(1, "fyje_sum"));
            //            var zje = dw_mx.GetItemNumber(1, "fyje_sum");
            var zje = dw_master.GetItemNumber(1, "sqfkje")
            var zusdhl = dw_master.GetItemNumber(dw_master.GetRow(), "zusdhl");
            var zrmbhl = dw_master.GetItemNumber(dw_master.GetRow(), "zrmbhl");
            var zusdje = zje * zusdhl;
            zusdje = parseFloat(zusdje.toFixed(2));
            var zrmbje = zje * zrmbhl;
            zrmbje = parseFloat(zrmbje.toFixed(2));
            dw_master.SetItem(dw_master.GetRow(), "zusdje", zusdje);
            dw_master.SetItem(dw_master.GetRow(), "zrmbje", zrmbje)
    ...
    //#region 双击选择
        this.dw_master_Clicked = function (xPos, yPos, Row, dwoName) {
            ...
            if (dwoName == "skdw_bm_t") {
                ...
                var num  = QsWebSoft.PubMethod.CheckGcFy(khbm).value;
                if (num >= 1) {
                    alert("该客户今天已经做过费用申请，请注意!");
                }
            }
        };
        //#endregion
    ...
    //#region 添加明细
        this.AddRow = function () {
            var fymc = "";
            if (dw_mx.RowCount() > 1) {
                fymc = dw_mx.GetItemString(dw_mx.RowCount(), "fymc");
            }
            dw_mx.ScrollToRow(dw_mx.InsertRow(0));
            var row = dw_mx.GetRow();
            dw_mx.SetItem(row, "fymc", fymc);
            dw_mx.SetFocus();
    ...
    //#region SetZje
        this.SetZje = function () {
            ...
            var zje = 0;
//            if (dw_mx.RowCount() > 0) {
//                zje = dw_mx.GetItemNumber(1, "fyje_sum");
...
//            }
        ...
        this.dw_master_ItemChanged = function (row, dwoName, data) {
            ...
            if (dwoName == "wbbb") {
                ...
                var zje = dw_master.GetItemNumber(row, "sqfkje");
                var zusdje = zje * zusdhl;
                zusdje = parseFloat(zusdje.toFixed(2));
                var zrmbje = zje * zrmbhl;
                zrmbje = parseFloat(zrmbje.toFixed(2));
