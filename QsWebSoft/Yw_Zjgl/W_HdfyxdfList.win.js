...
function W_HdfyxdfList() {
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(typeof(self.OpenGj)=="function") 
			 win.AttachEvent("btn_gj","Clicked",self.OpenGj); 
    ...
    webReq.SetWebServiceUrl("/Service/Hdfyxdf.ashx");   
...
//        var status = dw_list.GetItemString(dw_list.GetRow(), 'state')
    ...
    //#region 国家设置
    var iw_HdfyxdfgjEdit = null;
    var iw_Hddz_Beizhu = null;
    this.OpenGj = function (Row) {
        

        if (iw_HdfyxdfgjEdit == null)
            iw_HdfyxdfgjEdit = new W_HdfyxdfgjEdit();
        ...
        var userid = requestor.GetParm("userid");
        //        if (zbr == userid) {
        iw_HdfyxdfgjEdit.SetDataWindow(dw_list);
        iw_HdfyxdfgjEdit.SetData(ywbh);
        iw_HdfyxdfgjEdit.SetRow(Row);
        iw_HdfyxdfgjEdit.SetZbr(zbr);
        requestor.Open("/LoadWindow.ashx?WinName=Yw_Zjgl.W_HdfyxdfgjEdit", iw_HdfyxdfgjEdit);
...
//        }
        ...
        if (requestor.MessageBox("提示", "删除消毒费申请单号为<" + sqdbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;
    ...
    this.Print = function () {
        ...
        dw_print.SetDataObject("dw_hddz_hdfyxdf_print");
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_HdfyxdfList", "dw_hddz_hdfyxdf_list");
        self.Retrieve();
    }
    //#endregion
