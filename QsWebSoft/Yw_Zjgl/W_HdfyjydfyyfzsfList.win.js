...
//    }
    ...
    this.Delete = function () {
        ...
        if (status == "提交") {
            requestor.MessageBox("提示", "当前检疫点费用的状态为<" + status + ">,不能被删除!", 1);
            return;
        }
        ...
        if (requestor.MessageBox("提示", "删除检疫点费用应付转实付费用申请单号为<" + sqdbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;
    ...
    this.Print = function () {
        ...
        if (row <= 0)
            return;
        dw_print.SetDataObject("dw_hddz_hdfyjydfyyfzsf_print");
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(sqdbh) like '%" + text + "%')  or ( Lower(sqdbh) like '%" + text + "%')");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_HdfyjydfyyfzsfList", "dw_hddz_hdfyjydfyyfzsf_list");
        self.Retrieve();
    }
    //#endregion
    ...
    this.Tj = function () {
        ...
        var num = QsWebSoft.PubMethod.GenFksqdYfzsfJzxxx(sqdbh).value;
        if (num == "1") {
