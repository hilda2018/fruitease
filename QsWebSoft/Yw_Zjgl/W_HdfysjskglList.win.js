...
function W_HdfysjskglList() {
	 ...
	 var ddlb_skr= new DropDownListBox( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(ddlb_skr == null ) ddlb_skr= new DropDownListBox( requestor );
		 ddlb_skr.Attach( win.Control( "ddlb_skr") ); 
		 ...
		 if(typeof(self.Skr_SelectionChanged)=="function") 
			 win.AttachEvent("ddlb_skr","SelectionChanged",self.Skr_SelectionChanged); 
		 ...
		 if(typeof(self.sjsjg_dr)=="function") 
			 win.AttachEvent("btn_dr","Clicked",self.sjsjg_dr); 
    ...
    this.OpenWindow = function () {
        ...
        if (dw_list.RowCount() <= 0)
            return;
        var skdbh = dw_list.GetItemString(dw_list.GetRow(), 'skdbh')
//        var status = dw_list.GetItemString(dw_list.GetRow(), 'state')
        ...
        var sfhx = dw_list.GetItemString(dw_list.GetRow(), 'sfhx')
        if (sfhx == "Y") {
            operation = "show";
        }
        //先取得页面父窗口的TabStrip控件
        ...
        if (openStyle == "2")
            ...
            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&skdbh=' + skdbh + '&operation=' + operation.toString(), true);
...
//    }
    ...
    //#endregion
    //数据导入
    this.sjsjg_dr = function () {

        if (iW_HdfysjskglList_dr == null)
            iW_HdfysjskglList_dr = new W_HdfysjskglList_dr();           
            var ywy = requestor.GetParm("userid");
            var ShareMode = requestor.GetParm("ShareMode");
            var Dlwtf = requestor.GetParm("Dlwtf");            
            requestor.Open("/LoadWindow.ashx?WinName=Yw_Zjgl.W_HdfysjskglList_dr&ShareMode=" + ShareMode + "&ywy=" + ywy + "&Dlwtf=" + Dlwtf, iW_HdfysjskglList_dr);
            dw_list.SetFocus();
    }
    ...
    this.Delete = function () {
        ...
        var sfhx = dw_list.GetItemString(row, "sfhx");
       
        if (sfhx == "Y") {
            requestor.MessageBox("提示", "数据已经被核销,不能被删除!");
            return;
        }
        ...
        var skdbh = dw_list.GetItemString(row, "skdbh");
        if (requestor.MessageBox("提示", "删除实际收款单号为<" + skdbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;
        //执行删除的WebService
        webReq.SetFunctionID(requestor.GetFunctionID());
        webReq.Invoke("Delete", "skdbh=" + skdbh);
    ...
    this.Print = function () {
        ...
        dw_print.Retrieve(dw_list.GetItemString(dw_list.GetRow(), "skdbh"));
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(skdbh) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    //#region 过滤收款人
//    this.sle_skr_EditChanged = function (text) {
//        dw_list.SetRedraw(false);
//        if (text == "")
//            dw_list.SetFilter("");
//        else
//            dw_list.SetFilter("( Upper(skrjc) like Upper('%" + text + "%'))");
//        dw_list.Filter();
//        dw_list.Sort();
//        dw_list.SetRedraw(true);
//    }
    ...
    //#region 过滤收款人
    this.Skr_SelectionChanged = function () {
        var skrjc = ddlb_skr.GetText();
        dw_list.SetRedraw(false);
        if (skrjc == "全部")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(skrjc) like Upper('%" + skrjc + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_HdfysjskglList", "dw_hddz_sjskd_list");
        self.Retrieve();
    }
    //#endregion
 ...
 //#region 提交
    this.Tj = function () {
        ...
        var state = dw_list.GetItemString(row, "state")
        if (state == "提交") {
            if (requestor.MessageBox("提示!", "是否撤销提交?", ICON.Question, BUTTON.YesNo) == 1) {
                var skdbh = dw_list.GetItemString(row, "skdbh")
                var num = QsWebSoft.PubMethod.GenSJskdFy(skdbh,"").value;
                if (num == "1") {
                    dw_list.SetItem(row, "state", "")
                    dw_list.ResetUpdate();
            ...
            var skdbh = dw_list.GetItemString(row, "skdbh")
            var num = QsWebSoft.PubMethod.GenSJskdFy(skdbh,"提交").value;
            if (num == "1") {
