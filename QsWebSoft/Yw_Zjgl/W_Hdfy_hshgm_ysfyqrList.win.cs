...
{
    public partial class W_Hdfy_hshgm_ysfyqrList
    {
        public W_Hdfy_hshgm_ysfyqrList()
        {
        }
        public override void OnLoad()
        {
            ...
            this.ds_jyd.DataWindowObject = "dw_wldw_jyd_select";
            this.ds_jyd.Retrieve();
            ddlb_jyd.Items.Add("全部");
            for (int row = 1; row <= this.ds_jyd.RowCount; row++)
            {
                var jyjjc = this.ds_jyd.GetItemString(row, "khjc");
                ddlb_jyd.Items.Add(jyjjc);
            }
            
            // 数据检索
             this.dw_list.Retrieve(DateTime.Parse(this.dp_begin.Value.ToString()), DateTime.Parse(this.dp_end.Value.ToString()),"未确认");
            //注册相关的js文件
            this.RegisterClientScriptInclude("ExtPB_Demo", "/Beta3/ExtPB_Demo.js");
            this.RegisterClientScriptInclude("W_Hdfy_hshgm_ysfyqrList", "/Yw_Zjgl/W_Hdfy_hshgm_ysfyqrList.win.js");
