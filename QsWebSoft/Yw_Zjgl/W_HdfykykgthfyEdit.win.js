...
function W_HdfykykgthfyEdit() {
	 ...
	 var ds_kythfy= new DataStore( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(ds_kythfy == null ) ds_kythfy= new DataStore( requestor );
		 ds_kythfy.Attach( win.Control( "ds_kythfy") ); 
	 ...
	 if(typeof(baseUnBind)=="function") baseUnBind(); 
		 ...
		 ds_kythfy.Detach(); ds_kythfy=null; 
    ...
    //#region 变量定义
    var editMode = "";
    var iw_Hddz_Kykgthfy_Select = null;
    ...
    webReq.SetWebServiceUrl("/Service/Hdfykykgthfy.ashx");
    ...
    this.InsertRow = function () {
        if (iw_Hddz_Kykgthfy_Select == null)
            iw_Hddz_Kykgthfy_Select = new W_Hddz_Kykgthfy_Select();
        var ShareMode = requestor.GetParm("ShareMode");
        var ywy = requestor.GetParm("userid");
        var Dlwtf = requestor.GetParm("Dlwtf");
        row = dw_jzxxx.InsertRow(dw_jzxxx.GetRow());
        var dwo = dw_jzxxx.GetDataObject();
        iw_Hddz_Kykgthfy_Select.SetRow(row);
        iw_Hddz_Kykgthfy_Select.SetDwo(dwo);
        iw_Hddz_Kykgthfy_Select.SetDetailDW(dw_jzxxx);
        requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Hddz_Kykgthfy_Select&ShareMode=" + ShareMode + '&ywy=' + ywy + '&Dlwtf=' + Dlwtf, iw_Hddz_Kykgthfy_Select);
    ...
    this.Print = function () {
        ...
        dw_print.SetDataObject("dw_hddz_hdfykykgthfy_print");
...
//        dw_jzxxx.InsertRow(0);
    ...
    this.dw_master_Clicked = function (xPos, yPos, Row, dwoName) {
        ...
        if (dwoName == "sjskrbm_t") {
            ...
            iw_Wldw_Select.SetData("hdfykykgthfy_sjskr");
        ...
        if (dwoName == "sjskrkhyh_t") {
            ...
            iw_Wldw_Yh_Select.SetData("hdfykykgthfy_sjskryh");
    ...
    this.dw_jzxxx_DoubleClicked = function (xPos, yPos, Row, dwoName) {
        if (dwoName == "zfdxjc") {
            ...
            iw_Wldw_Zfdx_Select.SetData("hdfykykgthfy_zfdx");
    ...
    this.dw_jzxxx_ItemChanged = function (row, dwoName, data) {
        if (dwoName == "fybm") {
            ...
            fkcs = QsWebSoft.PubMethod.ReadFycs(ywbh,tpxx, zhlx, data, sqdbh).value;
            ...
            var xhgqjc = dw_jzxxx.GetItemString(row, "xhgqjc");
            var sjkgsj = new Date(dw_jzxxx.GetItemDate(row, "sjkgsj"));
            var je = dw_jzxxx.GetItemNumber(row, "je");
            var zmz = dw_jzxxx.GetItemNumber(row, "zmz");

            var kythfy = ds_kythfy.Retrieve(xhgqjc, data, sjkgsj.getVarDate(), je, zmz);
            if (kythfy == 0) {
        ...
        if (dwoName == "je") {
            ...
            var sjkgsj = new Date(dw_jzxxx.GetItemDate(row, "sjkgsj"));
            var zmz = dw_jzxxx.GetItemNumber(row, "zmz");
            var kythfy = ds_kythfy.Retrieve(xhgqjc, fybm, sjkgsj.getVarDate(), data, zmz);
            if (kythfy == 0) {
                dw_jzxxx.SetItem(row, "fysfhl", "Y")
            } else {
                dw_jzxxx.SetItem(row, "fysfhl", "N")
    ...
    this.Sp = function () {
        ...
        if (message_num == 1) {
            var num = QsWebSoft.PubMethod.GenFksqdKytd_sp(sqdbh, userid, "提交", "部门审批通过").value;
            if (num == 1) {
                self.Exit();
            }
        } else if (message_num == 2) {
            var num = QsWebSoft.PubMethod.GenFksqdKytd_sp(sqdbh, userid, "新制", "部门审批不通过").value;
            if (num == 1) {
                self.Exit();
            }
    ...
    this.Cxsp = function () {
        var sqdbh = dw_master.GetItemString(1, "sqdbh");
        var userid = requestor.GetParm("userid");
        var ysqr = dw_master.GetItemString(1, "ysqr");
        if (ysqr == "Y") {
            requestor.MessageBox("提示", "本单据由审批人员审批，撤销需要审批人员撤销!")
            return
        }
        var message_num = requestor.MessageBox("提示", "是否撤销审批?", ICON.Question, BUTTON.YesNo);
        if (message_num == 1) {
            var num = QsWebSoft.PubMethod.GenFksqdKyJzxxx_sp(sqdbh, "", "新制", "部门撤销审批").value;
            if (num == 1) {
                self.Exit();
            }
        } else if (message_num == 2) {
