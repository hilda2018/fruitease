...
function W_HdfykhxxzdList() {
    ...
    webReq.SetWebServiceUrl("/Service/Hdfykhzd.ashx");   
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(Dlwtf, beginDate.getVarDate(), endDate.getVarDate(), fydl,sfhx);
    ...
    this.dw_List_DoubleClicked = function (xPos, yPos, Row, dwoName) {
        if (Row > 0)
            if (dwoName == "fj") {
                ...
                var fybm = dw_list.GetItemString(Row, "fybm");
                var fymc = dw_list.GetItemString(Row, "fymc");
    ...
    this.slefydl_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(fydlbm) like Upper('%" + text + "%')) or ( Upper(fydlmc) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_HdfykhxxzdList", "dw_hddz_khxxzd_list");
        self.Retrieve();
    }
    //#endregion
