...
function W_HdfyhdfEdit() {
    ...
    var ds_hdf = new DataStore(requestor);
    ...
    this.bind = function (win) {
        ...
        if (ds_hdf == null) ds_hdf = new DataStore(requestor);
        ds_hdf.Attach(win.Control("ds_hdf"));
    ...
    webReq.SetWebServiceUrl("/Service/Hdfyhdf.ashx");
    ...
    this.Save = function ()
    {
        ...
        for ( var i = dw_jzxxx.RowCount(); i > 0; i-- )
        {
            var ywbh1 = dw_jzxxx.GetItemString( i, "ywbh" );
            if ( ywbh1 == "" || ywbh1 == '' )
            {
                alert( "业务编号不能为空！不能保存！！" );
                return;
        ...
        for ( row = dw_jzxxx.RowCount(); row > 0; row-- )
        {
            ...
            var jzxh = dw_jzxxx.GetItemString( row, "hz_jzxh" )
            if ( jzxh == null || jzxh == "" )
            {
                alert( "集装箱号为空不能保存!" );
                dw_jzxxx.SetFocus();
    ...
    this.InsertRow = function () {
        if (iw_Hddz_Hdf_Select == null)
            iw_Hddz_Hdf_Select = new W_Hddz_Hdf_Select();
        var ShareMode = requestor.GetParm("ShareMode");
        var ywy = requestor.GetParm("userid");
        var Dlwtf = requestor.GetParm("Dlwtf");
        var row = dw_jzxxx.InsertRow(dw_jzxxx.GetRow());
        var dwo = dw_jzxxx.GetDataObject();
        iw_Hddz_Hdf_Select.SetRow(row);
        iw_Hddz_Hdf_Select.SetDwo(dwo);
        iw_Hddz_Hdf_Select.SetDetailDW(dw_jzxxx);
        requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Hddz_Hdf_Select&ShareMode=" + ShareMode + '&ywy=' + ywy + '&Dlwtf=' + Dlwtf, iw_Hddz_Hdf_Select);
    ...
    this.Print = function () {
        ...
        var num = QsWebSoft.PubMethod.UpdateDyzt(sqdbh).value;
    }
    //#endregion
...
//        dw_jzxxx.InsertRow(0);
    ...
    this.dw_master_Clicked = function (xPos, yPos, Row, dwoName) {
        ...
        if (dwoName == "sjskrbm_t") {
            ...
            iw_Wldw_Select.SetData("hdfyhycqf_sjskr");
    ...
    this.dw_jzxxx_ItemChanged = function (row, dwoName, data) {
        if (dwoName == "fybm") {
            ...
            var cdgsjc = dw_jzxxx.GetItemString(row, "cdgsjc");
            var cyrjc = dw_jzxxx.GetItemString(row, "cyrjc");
            var xx = dw_jzxxx.GetItemString(row, "xx");
            var xl = dw_jzxxx.GetItemString(row, "xl");
            var yjkgsj = new Date(dw_jzxxx.GetItemDate(row, "yjkgsj"));
            var je = dw_jzxxx.GetItemNumber(row, "je");
            var jzxsl = dw_jzxxx.GetItemNumber(row, "jzxsl");

            var hdf = ds_hdf.Retrieve(cdgsjc, cyrjc, xx, xl, data, yjkgsj.getVarDate(), je, jzxsl);
            if (hdf == 0) {
        ...
        if (dwoName == "je") {
            ...
            var hdf = ds_hdf.Retrieve(cdgsjc, cyrjc, xx, xl, fybm, yjkgsj.getVarDate(), data, jzxsl);
...
//#endregion
    ...
    this.Recover = function () {
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_HdfyhdfEdit", "dw_hddz_hdfyhdf_edit_cmd");
