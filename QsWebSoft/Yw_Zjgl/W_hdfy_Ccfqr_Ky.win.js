...
function W_hdfy_Ccfqr_Ky() {
    ...
    var ddlb_ccfqr = new DropDownListBox(requestor);
    ...
    this.bind = function (win) {
        ...
        if (ddlb_ccfqr == null) ddlb_ccfqr = new DropDownListBox(requestor);
        ddlb_ccfqr.Attach(win.Control("ddlb_ccfqr"));
        ...
        if (typeof (self.Retrieve) == "function")
            win.AttachEvent("ddlb_ccfqr", "SelectionChanged", self.Retrieve);
    ...
    this.unbind = function () {
        ...
        ddlb_ccfqr.Detach(); ddlb_ccfqr = null;
...
//            return;
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), ccfqr);
        dw_log.Retrieve(userid, "kybg");
    }
    //#endregion
...
//    }
    ...
    this.dw_List_ItemChanged = function (row, dwoName, data) {
        if (dwoName == "sfsq") {
            dw_list.AcceptText();
            if (data == "Y") {
                var yjccfy = dw_list.GetItemNumber(row, "yjccfy");
                dw_list.SetItem(row, "sjccfy", yjccfy);
            }
            if (data == "N") {

                dw_list.SetItem(row, "sjccfy",0);
            }
...
//  
    ...
    this.slekh_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(khjc) like Upper('%" + text + "%')) or ( Upper(shdwjc) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Save = function () {
        ...
        for (var i = 1; i <= dw_list.RowCount(); i++) {
            if (dw_list.GetItemStatus(i, 0, DWBUFFER.Primary) == 1) {
                ...
                dw_log.SetItem(row, "changecols", "bgdh,bghgbm,bghg,bghgjc,bghgpym,bgsj");
        ...
        webReq.Invoke("ListCcfKySave", "dw_list=" + dw_d_data + "&dw_log=" + dw_log_data);
