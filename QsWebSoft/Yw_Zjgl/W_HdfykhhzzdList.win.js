...
function W_HdfykhhzzdList() {
    ...
    this.bind = function (win) {
        ...
        if (ddlb_fydl == null) ddlb_fydl = new DropDownListBox(requestor);
        ddlb_fydl.Attach(win.Control("ddlb_fydl"));
        ...
        if (ds_zljs == null) ds_zljs = new DataStore(requestor);
        ds_zljs.Attach(win.Control("ds_zljs"));
        ...
        if (typeof (self.btnzljs_Clicked) == "function")
            win.AttachEvent("btn_zljs", "Clicked", self.btnzljs_Clicked);
        ...
        if (typeof (self.Retrieve) == "function")
            win.AttachEvent("ddlb_fydl", "SelectionChanged", self.Retrieve);
    ...
    this.Retrieve = function () {
        ...
        var jdrbm = "全部";
        if (jdrjc.indexOf("||") > 0) {
            var wz = jdrjc.indexOf("||")
            jdrbm = jdrjc.substr(wz + 2, jdrjc.length - wz - 2);
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), fydl, userid, jdrbm);
    ...
    this.OpenWindow = function () {
        ...
        var fydlbm = dw_list.GetItemString(dw_list.GetRow(), 'fydlbm')
        var ywy = ""// dw_list.GetItemString(dw_list.GetRow(), "ywy");
        var userid = requestor.GetParm("userid");
        var operation = requestor.GetParm("operation");
        var jdrbm = dw_list.GetItemString(dw_list.GetRow(), "jdrbm");
        
        var beginDate = dw_list.GetItemString(dw_list.GetRow(), "begin");
        var endDate = dw_list.GetItemString(dw_list.GetRow(), "end");
        ...
        if (openStyle == "2")
            ...
            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&jsdwbm=' + jsdwbm + '&cxh=' + cxh + '&fydlbm=' + fydlbm + '&beginDate=' + beginDate + '&endDate=' + endDate + '&operation=' + operation.toString() + '&jdrbm=' + jdrbm, true);
...
//        }
        ...
        dw_print.Retrieve(jsdwbm, cxh, fydlbm, beginDate, endDate, "N", jdrbm);
    ...
    this.slekh_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(jsdwjc) like Upper('%" + text + "%')) or ( Upper(jsdwpym) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.btnzljs_Clicked = function ()
    {
        ...
        for ( row = 1; row <= dw_list.RowCount(); row++ )
        {
            ...
            var jsdwbm = dw_list.GetItemString( row, "jsdwbm" )
            var jsdwmc = dw_list.GetItemString( row, "jsdwmc" )
            var jsbm = dw_list.GetItemString( row, "jsbm" )
            var fydlbm = dw_list.GetItemString( row, "fydlbm" )
            var fydlmc = dw_list.GetItemString( row, "fydlmc" )
            var jdrbm = dw_list.GetItemString( row, "jdrbm_1" )
            var jdrmc = dw_list.GetItemString( row, "jdrbm_2" )
            ...
            var ye_qm = dw_list.GetItemNumber( row, "ye_qm" )
            var fydlbm = dw_list.GetItemString( row, "fydlbm" )

            var beginDate = dw_list.GetItemString( dw_list.GetRow(), "begin" );
            var endDate = dw_list.GetItemString( dw_list.GetRow(), "end" );

            ds_zljs.Retrieve( jsdwbm, cxh, beginDate, endDate, fydlbm, jdrbm );
            alert( jsdwbm );
            alert( cxh );
            alert( beginDate );
            alert( endDate );
            ...
            alert( jdrbm );
            var xg_flag = "1";
            //            requestor.MessageBox("", jsdwbm);
            //            var num = QsWebSoft.PubMethod.GenWldwZlrq(jsdwbm,jsdwmc,jsbm,fydlbm,fydlmc,jdrbm,jdrmc).value;

            if ( ye_qm <= 0 )
            {
                var zlrq = new Date( "1900/01/01" );
                var num = QsWebSoft.PubMethod.GenWldwZlrq( jsdwbm, jsdwmc, jsbm, fydlbm, fydlmc, jdrbm, jdrmc, zlrq ).value;
            }
            else
            {

                for ( row_ds = 1; row_ds <= ds_zljs.RowCount(); row_ds++ )
                {
                    if ( xg_flag == "1" )
                    {
                        var ysje = ds_zljs.GetItemNumber( row_ds, "ysje" )

                        if ( parseFloat( ye_qm.toFixed( 2 ) ) > parseFloat( ysje.toFixed( 2 ) ) )
                        {
                            ye_qm = ye_qm - ysje
                        ...
                        {
                            var zlrq = new Date( ds_zljs.GetItemDate( row_ds, "zlrq" ) );
                            var num = QsWebSoft.PubMethod.GenWldwZlrq( jsdwbm, jsdwmc, jsbm, fydlbm, fydlmc, jdrbm, jdrmc, zlrq ).value;
                            xg_flag = "0";
                            if ( num == "1" )
                            {
        ...
        requestor.MessageBox( "提示", "结转完成，其中成功" + ljzqts.toString() + "条，错误" + ljcwts.toString() );
        self.Retrieve()
    }
    //#endregion
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_HdfykhhzzdList", "dw_hddz_hdfykhhzzd_list");
        self.Retrieve();
    }
    //#endregion
    ...
    this.Tj = function () {
        ...
        var num = QsWebSoft.PubMethod.GenFksqdJzxxx(sqdbh).value;
        if (num == "1") {
