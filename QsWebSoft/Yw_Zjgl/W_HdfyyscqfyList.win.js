...
function W_HdfyyscqfyList() {
    ...
    webReq.SetWebServiceUrl("/Service/Hdfyyscqf.ashx");   
    ...
    this.dw_List_ItemChanged = function (row, dwoName, data) {
        if (dwoName == "jzxxx_yscqfqr") {
            var jzxxx_yscqfje = dw_list.GetItemNumber(row, "jzxxx_yscqfje")
            var gscqfje = dw_list.GetItemNumber(row, "gscqfje")
            ...
            var num = 0;
            if (data = "Y") {
                if (jzxxx_yscqfje == null || jzxxx_yscqfje == 0) {
                    dw_list.SetItem(row, "jzxxx_yscqfje", gscqfje)
                }
                dw_list.SetItem(row, "jzxxx_yscqfqrrq", date.getVarDate())
                //资金管理注释
                num = QsWebSoft.PubMethod.GenYscqf(ywbh, cxh, ywy).value;
            } else {
                dw_list.SetItem(row, "jzxxx_yscqfje", 0)
                dw_list.SetItem(row, "jzxxx_yscqfqrrq", date.getVarDate())
                num = QsWebSoft.PubMethod.GenYscqf(ywbh, cxh, ywy).value;
            }
        }
    ...
    this.Save = function () {
        ...
        webReq.Invoke("YscqfListSave", "dw_list=" + dw_d_data);
