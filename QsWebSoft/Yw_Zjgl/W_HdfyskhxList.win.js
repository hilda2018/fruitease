...
function W_HdfyskhxList() {
    ...
    var sle_skrjc = new SingleLineEdit(requestor);
    ...
    this.bind = function (win) {
        ...
        if (sle_skrjc == null) sle_skrjc = new SingleLineEdit(requestor);
        sle_skrjc.Attach(win.Control("sle_skrjc"));
        ...
        if (typeof (self.sle_skrjc_EditChanged) == "function")
            win.AttachEvent("sle_skrjc", "EditChanged", self.sle_skrjc_EditChanged);
        ...
        if (typeof (self.Retrieve) == "function")
            win.AttachEvent("ddlb_hxzt", "Modified", self.Retrieve);
    ...
    this.unbind = function () {
        ...
        sle_skrjc.Detach(); sle_skrjc = null;
    ...
    webReq.SetWebServiceUrl("/Service/Hdfyskhx.ashx");   
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), hxzt);
    ...
    this.OpenWindow = function () {
        ...
        var skd_cxh = dw_list.GetItemNumber(dw_list.GetRow(), 'cmd_cxh')
//        var status = dw_list.GetItemString(dw_list.GetRow(), 'state')
        ...
        if (openStyle == "2")
            ...
            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&skdbh=' + skdbh + '&skd_cxh=' + skd_cxh + '&operation=' + operation.toString(), true);
...
//    }
    ...
    this.Delete = function () {
        ...
        if (requestor.MessageBox("提示", "删除收款单编号为<" + skdbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;
    ...
    this.Print = function () {
        ...
        dw_print.SetDataObject("dw_hddz_skhx_print");
        dw_print.Retrieve(dw_list.GetItemString(dw_list.GetRow(), "skdbh"), dw_list.GetItemNumber(dw_list.GetRow(), "skd_cxh"));
    ...
    //#region 收款人过滤
    this.sle_skrjc_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
    ...
    this.slekh_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(lkrmc) like Upper('%" + text + "%')) or ( Upper(lkrjc) like Upper('%" + text + "%')) or ( Upper(lkrpym) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.slejsdw_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(cmd_jsdwjc) like Upper('%" + text + "%')) or ( Upper(cmd_jsdwpym) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_HdfyskhxList", "dw_hddz_skhx_list");
        self.Retrieve();
    }
    //#endregion
