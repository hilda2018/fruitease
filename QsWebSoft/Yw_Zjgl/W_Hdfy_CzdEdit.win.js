...
function W_Hdfy_CzdEdit() {
    ...
    var iW_Hdfy_Czd_Select = null;
    var iw_Wldw_Jsbm_Select = null;
    //实例化明细单的WebService请求的JS类对象
    var webReq = new WebRequestor();
    webReq.SetWebServiceUrl("/Service/Hdfy_Czdgl.ashx");
    ...
    this.Save = function () {
        ...
        if (dw_master.GetItemString(1, "sffbm") == "Y" && (dw_master.GetItemString(1, "jsbm") == "" || dw_master.GetItemString(1, "jsbm") == null)) {
            requestor.MessageBox("","请输入结算部门");
        }

        var row = dw_master.GetRow();
        var czdbh = dw_master.GetItemString(row, "czdbh");
        ...
        for (row = dw_cmd.RowCount(); row > 0; row--) {
            dw_cmd.SetItem(row, "czdbh", czdbh);
        ...
        //数据同步生鲜港
        var ywbh = dw_master.GetItemString(1, "czdbh");
        ...
        dw_log.SetItem(row, "tablename", "yw_hddz_czd");
        ...
        webReq.Invoke("Save", "dw_master=" + dw_master_data + "&dw_cmd=" + dw_cmd_data + "&dw_log=" + dw_log_data + "&czdbh=" + czdbh + "&operation=" + editMode);

        if (webReq.StatusCode() == 200) {
            //服务器成功处理，返回单号
            czdbh = webReq.ResponseText();
            dw_master.SetItem(1, "czdbh", czdbh);
            ...
            //更新对应表数据
            QsWebSoft.PubMethod.GetCzdMxhz(czdbh);
    ...
    this.InsertRow = function () {
        ...
        if (dw_master.GetItemString(1, "jsdwjc") == "" || dw_master.GetItemString(1, "jsdwjc") == null) {
            requestor.MessageBox("", "请选择结算单位!");
            return;
        }
        if (iW_Hdfy_Czd_Select == null)
            iW_Hdfy_Czd_Select = new W_Hdfy_Czd_Select();
        var ShareMode = requestor.GetParm("ShareMode");
        var ywy = requestor.GetParm("userid");
        var Dlwtf = requestor.GetParm("Dlwtf");
        row = dw_cmd.InsertRow(dw_cmd.GetRow());
        var dwo = dw_cmd.GetDataObject();
        var czdbh = dw_master.GetItemString(1, "czdbh");
        var jdrbm = dw_master.GetItemString(1, "jdrbm");
        var khbm = dw_master.GetItemString(1, "jsdwbm");

        iW_Hdfy_Czd_Select.SetRow(row);
        iW_Hdfy_Czd_Select.SetDwo(dwo);
        iW_Hdfy_Czd_Select.SetDetailDW(dw_cmd);

        requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Hdfy_Czd_Select&ShareMode=" + ShareMode + '&ywy=' + ywy + '&Dlwtf=' + Dlwtf + '&czdbh=' + czdbh + '&jdrbm=' + jdrbm + '&khbm=' + khbm, iW_Hdfy_Czd_Select);
    ...
    this.AddRow = function () {
        ...
        requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Hdfy_Czd_Select&ShareMode=" + ShareMode + '&ywy=' + ywy + '&Dlwtf=' + Dlwtf + '&jdrbm=' + jdrbm + '&khbm=' + khbm, iW_Hdfy_Czd_Select);
    }
    //#endregion
...
//        dw_cmd.InsertRow(0);
    ...
    this.dw_master_Clicked = function (xPos, yPos, Row, dwoName) {
        ...
        if (dwoName == "jdrjc_t") {
            ...
            var Dlwtf = requestor.GetParm("Dlwtf");
            iw_Wldw_Select.SetDataWindow(dw_master);
            iw_Wldw_Select.SetData("hddz_czd_jdr");

            var jsdwbm = dw_master.GetItemString(1, "jsdwbm");

            requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Wldw_Select&ShareMode=" + ShareMode + "&ywy=" + ywy + "&Dlwtf= " + Dlwtf + "&khlx=" + jdr + "&czd_jsdwbm=" + jsdwbm, iw_Wldw_Select);
            dw_master.SetFocus();
        }
        ...
        if (dwoName == "jsdwjc_t") {
            ...
            iw_Wldw_Select.SetData("hddz_czd_jsdw");

            var jdrbm = dw_master.GetItemString(1, "jdrbm");

            requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Wldw_Select&ShareMode=" + ShareMode + "&ywy=" + ywy + "&Dlwtf=" + Dlwtf + "&khlx=" + khlx + "&czd_jdrbm=" + jdrbm, iw_Wldw_Select);
            dw_master.SetFocus();
