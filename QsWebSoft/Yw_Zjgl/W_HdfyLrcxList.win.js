...
function W_HdfyLrcxList()
{
    ...
    var ddlb_bm = new DropDownListBox( requestor );
    var ddlb_qrqk = new DropDownListBox( requestor );
    ...
    var ddlb_jsqk = new DropDownListBox( requestor );
    var ddlb_zydl = new DropDownListBox( requestor );
    ...
    var ds_bm = new DataStore( requestor );
    ...
    this.bind = function ( win )
    {
        ...
        if ( ddlb_bm == null ) ddlb_bm = new DropDownListBox( requestor );
        ddlb_bm.Attach( win.Control( "ddlb_bm" ) );
        if ( ddlb_qrqk == null ) ddlb_qrqk = new DropDownListBox( requestor );
        ddlb_qrqk.Attach( win.Control( "ddlb_qrqk" ) );
        ...
        if ( ddlb_jsqk == null ) ddlb_jsqk = new DropDownListBox( requestor );
        ddlb_jsqk.Attach( win.Control( "ddlb_jsqk" ) );
        if ( ddlb_zydl == null ) ddlb_zydl = new DropDownListBox( requestor );
        ddlb_zydl.Attach( win.Control( "ddlb_zydl" ) );
        ...
        if ( ds_bm == null ) ds_bm = new DataStore( requestor );
        ds_bm.Attach( win.Control( "ds_bm" ) );
        ...
        if ( typeof ( self.slebm_EditChanged ) == "function" )
            win.AttachEvent( "ddlb_bm", "SelectionChanged", self.slebm_EditChanged );

        if ( typeof ( self.Retrieve ) == "function" )
            win.AttachEvent( "ddlb_qrqk", "SelectionChanged", self.Retrieve );
        ...
        if ( typeof ( self.Retrieve ) == "function" )
            win.AttachEvent( "ddlb_jsqk", "SelectionChanged", self.Retrieve );

        if ( typeof ( self.Retrieve ) == "function" )
            win.AttachEvent( "ddlb_zydl", "SelectionChanged", self.Retrieve );

        if ( typeof ( self.slegys_EditChanged ) == "function" )
            win.AttachEvent( "sle_gys", "EditChanged", self.slegys_EditChanged );

        if ( typeof ( self.slecyr_EditChanged ) == "function" )
            win.AttachEvent( "sle_cyr", "EditChanged", self.slecyr_EditChanged );

        if ( typeof ( self.slepm_EditChanged ) == "function" )
            win.AttachEvent( "sle_pm", "EditChanged", self.slepm_EditChanged );
    ...
    this.unbind = function ()
    {
        ...
        ddlb_bm.Detach(); ddlb_bm = null;
        ddlb_qrqk.Detach(); ddlb_qrqk = null;
        ...
        ddlb_jsqk.Detach(); ddlb_jsqk = null;
        ddlb_zydl.Detach(); ddlb_zydl = null;
    ...
    webReq.SetWebServiceUrl("/Service/HddzLrcx.ashx");   
    ...
    this.Retrieve_jdr = function ()
    {
        ds_bm.DataWindowObject = "dd_jdr_bm_select";
        var jdrjc = ddlb_jdrjc.GetText();     
        ds_bm.Retrieve( jdrjc );

        ddlb_bm.Reset();
        ddlb_bm.AddItem( "全部" );
        ddlb_bm.SetText( "全部" );   
        for ( row = 1; row <= ds_bm.RowCount(); row++ )
        {    
            var bm = ds_bm.GetItemString( row, "bm" );
            ddlb_bm.AddItem( bm );
        }
    ...
    this.slebm_EditChanged = function (  )
    {
        var bm = ddlb_bm.GetText()
        dw_list.SetRedraw( false );
        if ( bm == "" || bm == "全部" )
            dw_list.SetFilter( "" );
        else
            dw_list.SetFilter( "bm = '" + bm + "'" );
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.Retrieve = function ()
    {
        ...
        var jsqk = ddlb_jsqk.GetText();
        var zydl = ddlb_zydl.GetText();
        var jdrjc = ddlb_jdrjc.GetText();
        var qrqk = ddlb_qrqk.GetText();
        ...
        dw_list.Retrieve( kgzt, beginDate.getVarDate(), endDate.getVarDate(), Dlwtf, ysfs, jsqk, zydl, userid, qrqk, gjzt, jdrjc );
    ...
    this.dw_List_ItemChanged = function (row, dwoName, data) {
        ...
        if (dwoName == "jzxxx_jssfwc") {
            if (data == "Y") {
                dw_list.SetItem(row, "jzxxx_jswcsj", date.getVarDate())
            } else {
                dw_list.SetItem(row, "jzxxx_jswcsj", date_null.getVarDate())
            }
        }

        if (dwoName == "jzxxx_lrcx_qrbz") {
                var date = new Date();
                dw_list.SetItem(row, "jzxxx_lrcx_qrrq", date.getVarDate());
        }
    }
    //#region 关闭窗口
    this.Exit = function () {
    ...
    this.OpenWindow = function () {
        ...
        var ysfs = escape(dw_list.GetItemString(dw_list.GetRow(), 'ysfs'))
        ...
        if (openStyle == "2")
            ...
            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&ywbh=' + ywbh + '&cxh=' + cxh + '&ysfs=' + ysfs + '&operation=' + operation.toString(), true);
...
//        if (requestor.MessageBox("提示", "删除客户费用归集编号为<" + yshdfygjbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
//            return;
...
//    //#region 过滤接单人
//    this.ddlb_jdrjc_SelectionChanged = function () {
...
//    }
    ...
    this.slejsdw_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(jzxxx_jsdwjc) like Upper('%" + text + "%')) or ( Upper(jzxxx_jsdwpym) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    //#endregion
    //#region 过滤供应商
    this.slegys_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(gwgysjc) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.slecyr_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(cyrjc) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
