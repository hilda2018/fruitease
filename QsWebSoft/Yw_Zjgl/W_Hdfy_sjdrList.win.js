...
function W_Hdfy_sjdrList() {
    ...
    this.bind = function (win) {
        ...
        if (typeof (self.recall) == "function")
            win.AttachEvent("btn_recall", "Clicked", self.recall);
    ...
    this.Tj = function () {
        ...
        var sjdrbh = dw_list.GetItemString(row, "sjdrbh")
        var num = QsWebSoft.PubMethod.GetTJSJGL(sjdrbh).value; 
        if (num == "1") {
            dw_list.SetItem(row, "state", "提交")
            dw_list.ResetUpdate();
            requestor.MessageBox("提示", "报批成功!")
            var num1 = QsWebSoft.PubMethod.getState(sjdrbh).value;
        }
        else {
    ...
    this.recall = function () {
        ...
        var sjdrbh = dw_list.GetItemString(row, "sjdrbh");
        var num = QsWebSoft.PubMethod.DeleteSjdr(sjdrbh).value;
        if (num == "1") {
        
            dw_list.SetItem(row, "state", "提交")
            dw_list.ResetUpdate();
            requestor.MessageBox("提示", "撤回成功!")
             
        }
        else {
            requestor.MessageBox("提示", "撤回失败!")
    ...
    this.OpenWindow = function () {
        ...
        if (dw_list.RowCount() <= 0)
            return;
        var sjdrbh = dw_list.GetItemString(dw_list.GetRow(), 'sjdrbh')
        ...
        if (openStyle == "2")
            ...
            //ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&sjdrbh=' + sjdrbh + '&operation=' + operation.toString(), true);
            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&sjdrbh=' + sjdrbh + '&sjdrbh1=' + sjdrbh1 + '&operation=' + operation.toString(), true);
...
//    }
    ...
    this.New = function () {
        ...
        var sjdrbh1 = QsWebSoft.PubMethod.ReturnSjdrSjdrbhString().value;
    ...
    this.Delete = function () {
        ...
        var sjdrbh = dw_list.GetItemString(row, "sjdrbh");
        if (requestor.MessageBox("提示", "删除数据录入编号为<" + sjdrbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;

        //执行删除的WebService
        webReq.SetFunctionID(requestor.GetFunctionID());
        webReq.Invoke("SjdrDelete", "sjdrbh=" + sjdrbh);
...
//        dw_print.Retrieve(dw_list.GetItemString(dw_list.GetRow(), "sjdrbh"));
...
//    }
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(sjdrbh) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.slekh_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(zbr) like Upper('%" + text + "%')) or ( Upper(zbr) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Copy = function () {
        ...
        if (openStyle == "2")
            ...
            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&ywbh=' + sjdrbh + '&operation=' + operation.toString(), true);
    }
    //#endregion
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_Hdfy_sjdrList", "dw_hddz_yfjjf_list");
        self.Retrieve();
    }
    //#endregion
...
//        }
        ...
        //执行保存的WebService
        webReq.SetFunctionID(requestor.GetFunctionID());
        webReq.Invoke("SjdrSave", "dw_list=" + dw_d_data);
...
//        if (state == "提交") {
//            var sjdrbh = dw_list.GetItemString(row, "sjdrbh")
//            var num = QsWebSoft.PubMethod.GenYfjjf(sjdrbh,"新制").value;
//            if (num == "1") {
//                dw_list.SetItem(row, "state", "提交")
...
//        } else {
//            var sjdrbh = dw_list.GetItemString(row, "sjdrbh")
//            var num = QsWebSoft.PubMethod.GenYfjjf(sjdrbh,"提交").value;
//            if (num == "1") {
//                dw_list.SetItem(row, "state", "提交")
