...
function W_HddzList_Sjzf() {
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("ddlb_qrzt","SelectionChanged",self.Retrieve); 
    ...
    this.Retrieve = function () {
        ...
        //        requestor.MessageBox("Dlwtf", Dlwtf);
        //        requestor.MessageBox("yslx", yslx)
        //        requestor.MessageBox("Tybm", Tybm)
        //        requestor.MessageBox("Sfqytybm", Sfqytybm)
        //        requestor.MessageBox("qrzt", qrzt)
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), Dlwtf, yslx, Tybm, Sfqytybm, qrzt, jdrjc);
    ...
    this.Save = function () {
        ...
        webReq.Invoke("ListSjzfSave", "dw_list=" + dw_d_data);

        if (webReq.StatusCode() == 200) {
            //服务器成功处理，返回单号      
            dw_list.ResetUpdate();
            //            var num = QsWebSoft.PubMethod.InertSjzf().value;
    ...
    this.Qx = function () {
        for (row = 1; row <= dw_list.RowCount(); row++) {
            if (cbx_qx.GetChecked() == true) {
                var sjzfwc = dw_list.GetItemString(row, "yw_hddz_sjzf_sjzfwc");
                if (sjzfwc == "Y") {
                    alert("第" + row + "行中这票业务已经完成不能更改");
                    return;

                }
                dw_list.SetItem(row, "yw_hddz_sjzf_cnqr", "Y");
            } else {
                var sjzfwc = dw_list.GetItemString(row, "yw_hddz_sjzf_sjzfwc");
                if (sjzfwc == "Y") {
                    alert("第" + row + "行中这票业务已经完成不能更改");
                ...
                dw_list.SetItem(row, "yw_hddz_sjzf_cnqr", "N");
            }
        }
    };
    //#endregion
    ...
    this.dw_List_Clicked = function (xPos, yPos, Row, dwoName, Data) {
        if (Row > 0) {
            ...
            if (dwoName == 'yw_hddz_sjzf_cnqr') {
                var yw_hddz_sjzf_sjzfwc = dw_list.GetItemString(Row, "yw_hddz_sjzf_sjzfwc");
                if (yw_hddz_sjzf_sjzfwc == "Y") {
                    alert("第" + Row + "行，这票已经做过了，不能修改。");
                    return;
                }
            }
