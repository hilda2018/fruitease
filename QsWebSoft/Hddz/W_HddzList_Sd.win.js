...
function W_HddzList_Sd() {
    ...
    this.bind = function (win) {
        ...
        if (typeof (self.sleshdw_EditChanged) == "function")
            win.AttachEvent("sle_shdw", "EditChanged", self.sleshdw_EditChanged);
        ...
        if (typeof (self.Retrieve) == "function")
            win.AttachEvent("ddlb_sdzt", "SelectionChanged", self.Retrieve);
        ...
        if (typeof (self.Sdqsb) == "function")
            win.AttachEvent("btn_sdqsb", "Clicked", self.Sdqsb);

        if (typeof (self.Down) == "function")
            win.AttachEvent("btn_1", "Clicked", self.Down);
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), sdzt, Dlwtf, Tybm, Sfqytybm, userid, jdrjc);
    ...
    this.Print = function () {
        ...
        if (row <= 0) {
            requestor.MessageBox("", "表格为空，无法打印");
            return;
        };

        var sdttsfyz = dw_list.GetItemString(row, "sdttsfyz");
        if (sdttsfyz == "N") {
            requestor.MessageBox("", "税单抬头不一致，无法打印");
            return;
        };

        var sfyxz = dw_list.GetItemString(row, "sfyxz");
        if (sfyxz == "N") {
            requestor.MessageBox("", "未选择任何一行，无法打印");
            return;
        }


        var zds = QsWebSoft.PubMethod.GenSdglCxh().value;
        ...
        for (var i = 1; i <= dw_list.RowCount(); i++) {
            var dyxz = dw_list.GetItemString(i, "sfyxz");
            var xz = dw_list.GetItemString(i, "xz");
            if (dyxz == "Y") {
                if (xz == "Y") {
                    dw_list.SetItem(i, "sddybh", dateStr);
                    dw_list.SetItem(i, "sdkhqzrq", nowDate.getVarDate());
                }
            }
        }
        self.Save();

        dw_print.SetDataObject("dw_hddz_list_sd_print");
        dw_print.Retrieve(dateStr);
    ...
    //#region 过滤税单抬头
    this.sleshdw_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(shdwjc) like Upper('%" + text + "%')) or ( Upper(shdwmc) like Upper('%" + text + "%')) or ( Upper(shdwpym) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
  ...
  //#region 存盘
    this.Save = function () {
        ...
        for (var i = 1; i <= dw_list.RowCount(); i++) {
            ...
            if (dw_list.GetItemStatus(i, "sfcs", DWBUFFER.Primary) == 1) {
                ...
                dw_log.SetItem(row, "tablename", "yw_hddz_ocrwjsb");
        ...
        webReq.Invoke("ListSdSave", "dw_list=" + dw_d_data + "&dw_log=" + dw_log_data);
    ...
    //#region 获得字段焦点
    this.Help = function () {
        javascript: window.open('\税单管理.txt')
    }
    //#endregion
    ...
    this.dw_List_ItemChanged = function (row, dwoName, data) {
        if (dwoName == "sdlqr") {
            ...
            if (data == "" || data == null) {
                dw_list.SetItem(row, "sdlqsj", date_null.getVarDate())
            } else {
                dw_list.SetItem(row, "sdlqsj", date.getVarDate())
            }

        }
        if (dwoName == "sfcs") {
            var sfyzbsd = dw_list.GetItemString(row, "sfyzbsd");
            if (sfyzbsd == "Y") {
                if (data == "Y") {

                    var ywbh = dw_list.GetItemString(row, "ywbh");
                    var i = QsWebSoft.PubMethod.HddzListSdOcrwjsb('Y', ywbh).value;


                } if (data == "N") {
                     var ywbh = dw_list.GetItemString(row, "ywbh");
                    var i = QsWebSoft.PubMethod.HddzListSdOcrwjsb('N', ywbh).value;
                }
            }
    ...
    this.Sdqsb = function () {
        var row = dw_list.GetRow();
        if (row > 0) {
            var sddybh = dw_list.GetItemString(row, "sddybh");
            if (sddybh.length > 1) {
                window.open("http://www.fruitease.com:8009/wenjian/sdqsb/" + sddybh + ".pdf", "_blank", "channelmode");
            }
    ...
    this.Down = function () {
        ...
        if (dw_list.RowCount() > 0) {
            ...
            for (var i = 1; i <= dw_list.RowCount(); i++) {
                ...
                var ywbh = dw_list.GetItemString(i, "ywbh");
                dw_2.Retrieve(ywbh);
                for (var ii = 1; ii <= dw_2.RowCount(); ii++) {
                    requestor.MessageBox("", "4");
                    var filename = dw_2.GetItemString(ii, "wjmc");
                    var filenameurl = dw_2.GetItemString(ii, "wjlj");
                    QsWebSoft.PubMethod.DownFile(filename, filenameurl).value;
                }
            }
            requestor.MessageBox("", "下载完成!");
        }
    }
}
