...
function W_Xtdm_bjhlr() {
    ...
    this.bind = function (win) {
        ...
        if (typeof (self.CopyRow) == "function")
            win.AttachEvent("btn_duplicate", "Clicked", self.CopyRow);

        if (typeof (self.DeleteRow) == "function")
            win.AttachEvent("btn_delete", "Clicked", self.DeleteRow);
    ...
    webReq.SetWebServiceUrl("/Service/Fsdz.ashx");   
    ...
    this.InsertRow = function () {
        dw_list.InsertRow(0);
        var userid = requestor.GetParm("userid");
        dw_list.SetItem(dw_list.GetRow(), "register", userid);

        var date = new Date();     
        dw_list.SetItem(dw_list.GetRow(), "register_date",date.getVarDate());
    }
    ...
    this.OpenWindow = function () {
        ...
        if (dw_list.RowCount() <= 0)
            return;
        var number = dw_list.GetItemString(dw_list.GetRow(), 'number')
//        var status = dw_list.GetItemString(dw_list.GetRow(), 'state')
        ...
        if (openStyle == "2")
            ...
            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&number=' + number + '&operation=' + operation.toString(), true);
    }
    //#endregion
...
//    }
    ...
    this.slekh_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(ztdh) like Upper('%" + text + "%')) or ( Upper(ztdh) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Yw_Zjgl.W_Hdfy_YsjjfList", "dw_xt_ysjjf_list");
        self.Retrieve();
    }
    //#endregion
    ...
    this.Save = function () {
        ...
        webReq.Invoke("Wtdm_gqlr", "dw_list=" + dw_d_data);
    ...
    this.Tj = function () {
        ...
        if (state == "提交") {
            ...
            var num = QsWebSoft.PubMethod.GenYsjjf(sqdbh,"新制").value;
            if (num == "1") {
            ...
            var sqdbh = dw_list.GetItemString(row, "sqdbh")
            var num = QsWebSoft.PubMethod.GenYsjjf(sqdbh,"提交").value;
            if (num == "1") {
                dw_list.SetItem(row, "state", "提交")
    ...
    this.Filter = function () {
        ...
        requestor.Open("/LoadWindow.ashx?WinName=xt.W_Filter&Zdm=" + cloname + '&colType=' + colType, iw_Filter);
        dw_list.SetFocus();
