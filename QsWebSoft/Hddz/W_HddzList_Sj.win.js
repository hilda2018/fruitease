...
function W_HddzList_Sj() {
    ...
    this.Retrieve = function () {
        ...
        var sfxgbgdh = requestor.GetParm( "Sfxgbgdh" );
        var ohxt_clbz = "N";
        ...
        dw_list.Retrieve( beginDate.getVarDate(), endDate.getVarDate(), sjzt, Dlwtf, yslx, Tybm, Sfqytybm, rqlx, ohxt_clbz, userid, jdrjc, sfxgbgdh );
        dw_log.Retrieve(userid, "sjgl");
    }
    //#endregion
    ...
    this.Save = function () {
        ...
        for (var i = 1; i <= dw_list.RowCount(); i++) {
            if (dw_list.GetItemStatus(i, 0, DWBUFFER.Primary) == 1) {
                ...
                dw_log.SetItem(row, "changecols", "bgdh,sjzfrq,sjzfkhlx,sjzfrq,gs,zzs,bzj,zbj,sjzfrq_bj,sjbz,ohxt_clbz");
                dw_log.SetItem(row, "mainid", ywbh);
                dw_log.ScrollToRow(row);

                var row1 = dw_log.InsertRow(0);
                //var maxNumber =  self.SerialNumber(log_num);
                var serialNumber = dw_log.GetItemString(row1, "eid_max") //"hyfx" + userid + yearMonDay + maxNumber;

                dw_log.SetItem(row1, "eid", serialNumber);
                dw_log.SetItem(row1, "tablename", "yw_hddz_sjzf");
        ...
        if (webReq.StatusCode() == 200) {
            ...
            var num_zbj = QsWebSoft.PubMethod.UpdateSjzf_sjzfrq().value;
    ...
    this.dw_List_Clicked = function (xPos, yPos, Row, dwoName, Data) {
        ...
        if (dwoName == "sjzfkh_jsbm") {
            ...
            iw_Wldw_Jsbm_Select.SetData("sjgl_jsdw");
