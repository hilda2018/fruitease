...
function W_HddzList_Ocrwjsb() {
	 ...
	 var ddlb_wjlx= new DropDownListBox( requestor );
	 var rb_wdy= new RadioButton( requestor );
	 var rb_ysb= new RadioButton( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(ddlb_wjlx == null ) ddlb_wjlx= new DropDownListBox( requestor );
		 ddlb_wjlx.Attach( win.Control( "ddlb_wjlx") ); 
		 if(rb_wdy == null ) rb_wdy= new RadioButton( requestor );
		 rb_wdy.Attach( win.Control( "rb_wdy") ); 
		 if(rb_ysb == null ) rb_ysb= new RadioButton( requestor );
		 rb_ysb.Attach( win.Control( "rb_ysb") ); 
		 ...
		 if(typeof(self.ddlb_Wjlx_SelectionChanged)=="function") 
			 win.AttachEvent("ddlb_wjlx","SelectionChanged",self.ddlb_Wjlx_SelectionChanged); 

		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("rb_wdy","Clicked",self.Retrieve); 

		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("rb_ysb","Clicked",self.Retrieve); 
	 ...
	 if(typeof(baseUnBind)=="function") baseUnBind(); 
		 ...
		 ddlb_wjlx.Detach(); ddlb_wjlx=null; 
		 rb_wdy.Detach(); rb_wdy=null; 
		 rb_ysb.Detach(); rb_ysb=null; 
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), lb);
    ...
    this.OpenWindow = function () {
        ...
        if (row >= 0) {
            var wjlx = dw_list.GetItemString(row, "wjlx");
            var wjmc = dw_list.GetItemString(row, "wjmc");
            if (wjlx == "文件归档"){
                javascript: window.open("http://www.fruitease.com:8009/wenjian/wjgd/" + wjgd, "_blank", "channelmode");
               
            }else
            {
                 javascript: window.open("http://www.fruitease.com:8009/wenjian/CIQ TAX/" + wjmc, "_blank", "channelmode");
            }
        }
       }
...
//    //#endregion
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(wjmc) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.ddlb_Wjlx_SelectionChanged = function (index) {
        var wjlx = ddlb_wjlx.GetText();
        if (wjlx == "全部") {
            dw_list.SetFilter("")
        } else {
            dw_list.SetFilter("wjlx = '" + wjlx + "'")
        }
        dw_list.Filter()
    };
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Hddz.W_HddzList_Tsgl", "dw_hddz_list_Tsgl");
        self.Retrieve();
    }
    //#endregion
    ...
    this.Save = function () {
        ...
        webReq.Invoke("ListOcrwjsbSave", "dw_list=" + dw_d_data );
    ...
    //#region 获得字段焦点
    this.Help = function () {
        javascript: window.open('\OCR文件未识别管理.txt')
    };
    //#endregion
