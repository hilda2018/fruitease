...
function W_HddzEdit() {
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(typeof(self.dw_fsdz_ItemChanged)=="function") 
			 win.AttachEvent("dw_fsdz","ItemChanged",self.dw_fsdz_ItemChanged); 
...
//        }
    ...
    this.CloseQuery = function () {
        ...
        dw_dzqk.AcceptText();
        if ((dw_master.ModifiedCount() + dw_jzxxx.DeletedCount() + dw_jzxxx.ModifiedCount() + dw_spxx.DeletedCount() + dw_spxx.ModifiedCount() + dw_zzxx.DeletedCount() + dw_zzxx.ModifiedCount() + dw_fsdz.DeletedCount() + dw_fsdz.ModifiedCount() + dw_sjzs.DeletedCount() + dw_sjzs.ModifiedCount() + dw_sjbzcl.DeletedCount() + dw_sjbzcl.ModifiedCount() + dw_zsbh.DeletedCount() + dw_zsbh.ModifiedCount() + dw_sjsfdj.DeletedCount() + dw_sjsfdj.ModifiedCount() + dw_dzqk.DeletedCount() + dw_dzqk.ModifiedCount()) > 0) {
    ...
    this.Save = function () {
        ...
        if (dw_master.GetItemString(1, "khmc") == "" || dw_master.GetItemString(1, "khmc") == null) {
        ...
        if (dw_master.GetItemString(1, "ysmxcd") == "") {
            requestor.MessageBox("提示", "请输入应收明细程度!");
            return;
        }



        var count = dw_dzqk.RowCount();
        for (var i = 1; i <= count; i++) {
            var sdzxjg = dw_dzqk.GetItemString(i, "sdzxjg");
            if (sdzxjg == "不通过") {
                var sfsdzb = dw_dzqk.GetItemString(i, "sfsdzb");
                if (sfsdzb == "Y") {
                    alert("审单结果和收到正本不匹配！请确认");
                    return;
                }
            }
        }
        ...
        var sp_jg_show = true;
        var spmc = "";
        var dw_spxx_count = dw_spxx.RowCount();
        for (var i = 1; i <= dw_spxx_count; i++) {
            var djjg = dw_spxx.GetItemNumber(i, "djjg");
            var spbm = dw_spxx.GetItemString(i, "spbm");
            var ycd = dw_master.GetItemString(1, "ycd");
            var xtywbh = QsWebSoft.PubMethod.dw_spxxzgj_sdj(ycd, spbm, djjg).value;
            if (xtywbh == 1) {
                spmc = dw_spxx.GetItemString(i, "spmc");
                sp_jg_show = false;
            }
        }
        if (sp_jg_show == false) {
            //alert(spmc + ":超出了价格范围！");   //这个价格范围在基础数据中的产品信息库编辑中维护（最低价，最高价）
            if (!confirm(spmc + ":超出了价格范围,是否要保存！！")) {
                return;
            } 
        }
        ...
        if (zydl == "自营") {
            if (mygb != ycd) {
                if (requestor.MessageBox("提示", "贸易国别与原产地不一致，是否保存？", ICON.Question, BUTTON.YesNo) == 2) {
                    return;
                }
            }
        }
        ...
        for (row = dw_jzxxx.RowCount(); row > 0; row--) {
            ...
            if (jzxh != null && jzxh != "") {
                var jy = QsWebSoft.PubMethod.JzxhJy(jzxh).value;
                if (jy != jzxh) {
                    alert("集装箱号输入错误，请重新输入");
                    return;
                }
            }
            ...
            if (jzxh != null && jzxh != "") {
                if (row > 1) {
                    var li_find = dw_jzxxx.Find("jzxh = '" + jzxh + "'", 1, row - 1);
                    if (li_find > 0) {
                        alert("同一票业务下，集装箱号不能重复发！！");
                        return;
                    }
                }
            }
            //            for (var j = 1; j < row; j++) {
            //                var jzxh1 = dw_jzxxx.GetItemString(j, "jzxh");
            //                if (jzxh1 == jzxh) {
            //                    alert("同一票业务下，集装箱号不能重复发！！");
            //                    return;
            //                }
        ...
        for (var i = 1; i <= dw_dzqk.RowCount(); i++) {
            ...
            var dzzt = dw_master.GetItemString(row, "dzqk");
            var ywbh = dw_master.GetItemString(row, "ywbh");
            var dzzt_1 = QsWebSoft.PubMethod.SelectDzzt(ywbh, dzzt).value;
            if (dzzt_1 != null && dzzt_1 != "") {
                dw_master.SetItem(row, "dzqk", dzzt_1);
            }

            //如果收到正本或则收到副本则对应数据明细条数就自动插入一条
            var sfsdzb = dw_dzqk.GetItemString(i, "sfsdzb");
            var sfsdfb = dw_dzqk.GetItemString(i, "sfsdfb");
            if (sfsdzb == "Y" || sfsdfb == "Y") {
                dw_dzqk.SetItem(i, "sdmxsj", 1);
            }
        }
        ...
        if (dw_dzqk.RowCount() > 0) {
            ...
            for (var i = 1; i <= dw_dzqk.RowCount(); i++) {
                ...
                //非正本提单已收到
                    ...
                    //非提单正本未收到,但副本已到
                    if (dw_dzqk.GetItemString(i, "dzmc") != "提单" && dw_dzqk.GetItemString(i, "zfbqk") == "正本") {
                        if (dw_dzqk.GetItemString(i, "sfsdzb") != "Y" && dw_dzqk.GetItemString(i, "sfsdfb") == "Y") {
                            dzqk += str + "正本未收到,但副本已到" + str2 + ";";
                            //防止dzqk字符串被重置为已收齐,此处将dzqknum设为999
                            dzqknum = 999;
                        }
                        //非提单正本未收到
                        if (dw_dzqk.GetItemString(i, "sfsdzb") != "Y" && dw_dzqk.GetItemString(i, "sfsdfb") != "Y") {
                    ...
                    //非提单副本未收到
                    if (dw_dzqk.GetItemString(i, "dzmc") != "提单" && dw_dzqk.GetItemString(i, "zfbqk") == "副本") {
                        if (dw_dzqk.GetItemString(i, "sfsdzb") != "Y" && dw_dzqk.GetItemString(i, "sfsdfb") != "Y") {
                            dzqk += str + "副本未收到" + str2 + ";";
                        }
                    }
        ...
        for (row = dw_spxx.RowCount(); row > 0; row--) {
            ...
            var tsbz = dw_master.GetItemString(1, "tsbz");

            var sfyecdm = QsWebSoft.PubMethod.CheckSsgdmkhxxtx(gj, spbm, shdwbm).value;

            if (tsbz != "Y") {
                if (sfyecdm > 0 && dmbz != "Y") {
                    if (requestor.MessageBox("提示", "对应国家商品及收货单位按规则需要设置为进生鲜港业务,是否设置本票业务为进生鲜港业务?", ICON.Question, BUTTON.YesNo) == 1) {
                        //                    dw_master.SetItem(1, "dmbz", "Y");
                    }
                }
                if (sfyecdm == 0 && dmbz == "Y") {
                    if (requestor.MessageBox("提示", "对应国家商品及收货单位按规则不需要设置为进生鲜港业务,是否设置本票业务为非进生鲜港业务?", ICON.Question, BUTTON.YesNo) == 1) {
                        //                    dw_master.SetItem(1, "dmbz", "N");
                    }
                };
            }

            dw_master.SetItem(1, "tsbz", "Y");



            //处理y证提示
            var spbm = dw_spxx.GetItemString(row, "spbm");
            var yz = QsWebSoft.PubMethod.ReadGjSpYz(ycd, spbm).value;

            if (yz == "Y") {
                var sfdz_row = dw_fsdz.Find("dzdm = 'Y'");
                if (sfdz_row > 0) {
                    var dzbh = dw_fsdz.GetItemString(sfdz_row, "dzbh");
                    if (dzbh == null || dzbh == "") {
                        requestor.MessageBox("提示", "本单证需要输入原产地证明Y证，请检查！");
                    }
                }
                else {
                    requestor.MessageBox("提示", "本单证需要输入原产地证明Y证，请检查！");
        ...
        var li_yzbh = dw_fsdz.Find("dzdm = 'Y'", 1, dw_fsdz.RowCount());
        if (li_yzbh > 0) {
            var yzbh = dw_master.GetItemString(1, "yzbh");
            var sphs = dw_spxx.RowCount();
            if (yzbh != null && yzbh != "") {
                var dzbh = "<" + yzbh + ":1-" + sphs + ">";
                dw_fsdz.SetItem(li_yzbh, "dzbh", dzbh);
            }
        }

        //        if (dw_master.GetItemString(1, "jydbm") == "" || dw_master.GetItemString(1, "jydbm") == null) {
        //            var jyjbm = dw_master.GetItemString(1, "jyjbm");
        //            if (jyjbm == "40010350") {
        //                dw_master.SetItem(1, "jydbm", "40010475");
        //                dw_master.SetItem(1, "jyd", "上海洋山港区检疫点");
        //                dw_master.SetItem(1, "jydjc", "洋山港区检疫点");
        //                dw_master.SetItem(1, "jydpym", "shysgqjyd");

        //                if (zdmc == null || zdmc == "") {
        //                    zdmc = "jydbm,jyd,jydjc,jydpym";
        //                } else {
        //                    if (zdmc.indexOf("jydbm,jyd,jydjc,jydpym") < 1) {
        //                        zdmc = zdmc + ',' + "jydbm,jyd,jydjc,jydpym";
        //                    };
        //                };
        //            }
        //        }
        ...
        dw_khsdttsz.Retrieve(dw_master.GetItemString(1, "khbm"), dw_master.GetItemString(1, "shdwbm"));
        if (dw_khsdttsz.RowCount() > 0) {
            dw_master.SetItem(1, "ckdwbm", dw_khsdttsz.GetItemString(1, "ckdwbm"));
            dw_master.SetItem(1, "ckdwmc", dw_khsdttsz.GetItemString(1, "ckdwmc"));
            dw_master.SetItem(1, "ckdwjc", dw_khsdttsz.GetItemString(1, "ckdwjc"));
            dw_master.SetItem(1, "ckdwpym", dw_khsdttsz.GetItemString(1, "ckdwpym"));
        }
        ...
        if (dw_master_data == "" && dw_jzxxx_data == "" && dw_spxx_data == "" && dw_zzxx_data == "" && dw_fsdz_data == "" && dw_sjzs_data == "" && dw_sjbzcl_data == "" && dw_zsbh_data == "" && dw_sjsfdj_data == "" && dw_dzqk_data == "")
            return;
        ...
        webReq.Invoke("Save", "dw_master=" + dw_master_data + "&dw_jzxxx=" + dw_jzxxx_data + "&dw_spxx=" + dw_spxx_data + "&dw_zzxx=" + dw_zzxx_data + "&dw_fsdz=" + dw_fsdz_data + "&dw_sjzs=" + dw_sjzs_data + "&dw_sjbzcl=" + dw_sjbzcl_data + "&dw_zsbh=" + dw_zsbh_data + "&dw_sjsfdj=" + dw_sjsfdj_data + "&dw_dzqk=" + dw_dzqk_data + "&dw_sxgsjjl=" + dw_sxgsjjl_data + "&dw_log=" + dw_log_data + "&ywbh=" + ywbh + "&operation=" + editMode);
    ...
    //#endregion
    this.MdgAddJdrjc = function () {
        //通过目的港查询jdrjc
        dw_master.AcceptText();
        var mdg = dw_master.GetItemString(1, "mdg");
        var jdrjc = QsWebSoft.PubMethod.MdgAndJdrjc(mdg).value;
        if (jdrjc != null) {
            var jdrjc1 = dw_master.GetItemString(1, "jdrjc");
            if (jdrjc != jdrjc1) {
                alert("目的港的接单人与默认接单人不符，请重新输入。。");
                return;
            }
        }
    }
    ...
    this.Print_dbh = function () {
        ...
        dw_print_cns.Retrieve(dw_master.GetItemString(1, "ywbh"));
        dw_print_cns.PrintPreview();
    }
...
//       dw_fsdz.SetItem(li_insert, "dzdm", "A")
   ...
   //#region 新建
    this.AddOcr = function () {
        ...
        dw_master.SetItem(1, "jydbm", "40010126")
        dw_master.SetItem(1, "jydmc", "龙吴检疫局")
        dw_master.SetItem(1, "jydjc", "龙吴检疫局")
        dw_master.SetItem(1, "jydpym", "lwjy")
...
//        };
        ...
        var fkfs = dw_fpzb.GetItemString(1, "fkfs");
        if (fkfs == null || fkfs == "") {
            dw_fpzb.SetItem(1, "fkfs", "TT BY SELLER'S ORDER");
        }
        zdmc = "";
        self.SetZje();
        dw_dzqk.Reset();
    ...
    this.dw_master_Clicked = function (xPos, yPos, Row, dwoName) {
        ...
        if (dwoName == "jdr_t") {
            ...
            var ysmxcd1 = QsWebSoft.PubMethod.Jdrbm_Ysmxcd(jdrbm).value;
            if (ysmxcd1 == null) {
                dw_master.SetItem(1, "ysmxcd", "");
            }

            dw_master.SetItem(1, "ysmxcd", ysmxcd1)
        }
        if (dwoName == "wlgs_t") {
            if (iw_Wldw_Select == null)
                iw_Wldw_Select = new W_Wldw_Select();
        ...
        if (dwoName == "hdgs_t") {
            ...
            var Dlwtf = requestor.GetParm("Dlwtf");
            iw_Wldw_Select.SetDataWindow(dw_master);
            iw_Wldw_Select.SetData("hddz_hdrjc");
            //生鲜港 
            if (zdmc == null || zdmc == "") {
    ...
    this.dw_master_ItemChanged = function (row, dwoName, data) {
        ...
        if (dwoName == "yjkgsj") {
            var yjkgsj1 = dw_master.GetItemDate(1, "yjkgsj");
            dw_tdzb.SetItem(1, "yjkgsj", yjkgsj1);

            var yjkgsj2 = dw_tdzb.GetItemDate(1, "yjkgsj");
            dw_master.SetItem(1, "yjkgsj", yjkgsj2);
        ...
        if (dwoName == "cm" || dwoName == "zz_cm" || dwoName == "hcorhbh" || dwoName == "zz_hcorhbh" || dwoName == "hcorhbh_zq") {
            dw_master.SetItem(1, "sffshb", "N");
            //生鲜港
            if (zdmc == null || zdmc == "") {
                zdmc = "cm,zz_cm,hcorhbh,zz_hcorhbh,hcorhbh_zq";
            } else {
                if (zdmc.indexOf(dwoName) < 1) {
                    zdmc = zdmc + ',' + "cm,zz_cm,hcorhbh,zz_hcorhbh,hcorhbh_zq";
                };
            };
        }
    }
...
//        zdmc_dzyq = "Y";
    ...
    this.dw_jzxxx_ItemChanged = function (row, dwoName, data) {
        zdmc_jzxxx = "Y";
        if (data == '散货' || data == '散货集装箱') {
            dw_jzxxx.SetItem(row, "zhlx", "散装");
        } else {
            dw_jzxxx.SetItem(row, "zhlx", "集装箱");
        }
    };
...
//        zdmc_dzyq = "Y";
    ...
    //#region dw_jzxxx dw_jzxxx_Clicked
    this.dw_fsdz_ItemChanged = function (Row, dwoName, data) {
        if (dwoName == "dzbh") {
           
            for (var i = 0; i < data.length; i++) {
                strCode = data.charCodeAt(i); 
                if ((strCode > 65248) || (strCode == 12288)) {
                    requestor.MessageBox("提示", "全角字母不能输入,请重新输入！")
                    dw_fsdz.SetItem(Row, "dzbh", "");
                    return;
                }
            }
        }
...
//            dw_zzxx.dw_zzxx_GetFocus()
    ...
    this.Recover_dzqk = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Hddz.W_HddzEdit", "dw_hddz_edit_dzqk");
        self.Retrieve();
    }
    //#endregion
    ...
    this.dw_dzqk_DoubleClicked = function (xPos, yPos, Row, dwoName) {
        ...
        var zfbqk = dw_dzqk.GetItemString(Row, "zfbqk");
        var dzmc = dw_dzqk.GetItemString(Row, "dzmc");
        var tdsfzb = dw_dzqk.GetItemNumber(Row, "tdsfzb");
        var userid = requestor.GetParm("userid");

        if (dzmc == "提单" && (zfbqk == "" || zfbqk == null || zfbqk == "未知")) {
            requestor.MessageBox("提示", "请先选择提单类型！");
            return;
        }
