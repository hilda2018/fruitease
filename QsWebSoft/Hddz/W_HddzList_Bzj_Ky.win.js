...
function W_HddzList_Bzj_Ky() {
	 ...
	 var ddlb_bzjzt= new DropDownListBox( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(ddlb_bzjzt == null ) ddlb_bzjzt= new DropDownListBox( requestor );
		 ddlb_bzjzt.Attach( win.Control( "ddlb_bzjzt") ); 
		 ...
		 if(typeof(self.slebzjsjh_EditChanged)=="function") 
			 win.AttachEvent("sle_3","EditChanged",self.slebzjsjh_EditChanged); 
		 ...
		 if(typeof(self.Hgtbcl)=="function") 
			 win.AttachEvent("btn_1","Clicked",self.Hgtbcl); 

		 if(typeof(self.Exit)=="function") 
			 win.AttachEvent("btn_close","Clicked",self.Exit); 

		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("ddlb_bzjzt","SelectionChanged",self.Retrieve); 

		 if(typeof(self.slehz_spmc_EditChanged)=="function") 
			 win.AttachEvent("sle_hz_spmc","EditChanged",self.slehz_spmc_EditChanged); 
	 ...
	 if(typeof(baseUnBind)=="function") baseUnBind(); 
		 ...
		 ddlb_bzjzt.Detach(); ddlb_bzjzt=null; 
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), bzjzt, Dlwtf, Tybm, Sfqytybm, userid,jdrjc);
        dw_log.Retrieve(userid, "kbzj");
    }
    //#endregion
    ...
    this.dw_List_DoubleClicked = function (xPos, yPos, Row, dwoName) {
        if (Row > 0) {
            if (dwoName == "bzjzfrjc") {               
                ...
                iw_Wldw_Select.SetData("hddz_bzj_bzjzfr");
    ...
    this.Print = function () {
        ...
        if (row <= 0)
            return;
        dw_print.SetDataObject("dw_hddz_print_skdbtzspb");
    ...
    //#region 过滤保证金收据号
    this.slebzjsjh_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(bzjdjh) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Save = function () {
        ...
        for (row = dw_list.RowCount(); row > 0; row--) {
            bzjpd_jsl = dw_list.GetItemString(row, "bzjpd_jsl");
            if (bzjpd_jsl == "N") {
                alert("第" + row + "行的保证金未处理完整，请检查!");
                return;
            }
        }
        ...
        for (var i = 1; i <= dw_list.RowCount(); i++) {
            ...
            if (dw_list.GetItemStatus(i, 0, DWBUFFER.Primary) == 1) {
                ...
                dw_log.SetItem(row, "changecols", "bzjdjh,bzj,sjzfrq,bzjjzsj,zgs,zzzs,tbzj,tkdzsj");
        ...
        webReq.Invoke("ListBzjKySave", "dw_list=" + dw_d_data+"&dw_log="+dw_log_data);
    ...
    //#region 海关退保处理
     this.Hgtbcl = function () {
         var hgclbh_new = QsWebSoft.PubMethod.Readkybzjhgcl().value;
         for (row = dw_list.RowCount(); row > 0; row--) {
             var xz = dw_list.GetItemString(row, "xz");
             var date = new Date();
             var hgclbh = dw_list.GetItemString(row, "hgclbh");
             if (xz == "Y" && (hgclbh == null || hgclbh == "")) {
                 dw_list.SetItem(row, "hgclbh", hgclbh_new)
                 dw_list.SetItem(row, "hgclsj", date.getVarDate())
             }
    ...
    //#region dw_list ItemChanged
     this.dw_List_ItemChanged = function (row, dwoName, data) {
         ...
         ////
         if (dwoName == "bzjymxz") {
             var sjzfrq = new Date(dw_list.GetItemDate(row, "sjzfrq"));
             if (data == "产地证") {
                 sjzfrq.setDate(sjzfrq.getDate() + 90);
             } else {
                 sjzfrq.setDate(sjzfrq.getDate() + 180);
             }
             dw_list.SetItem(row, "bzjjzsj", sjzfrq.getVarDate());
         ...
         if (row > 0) {
             if (dwoName == "zzzs") {
                 var bzj = dw_list.GetItemNumber(row, "bzj")
                 var zgs = dw_list.GetItemNumber(row, "zgs")
                 if (bzj == null) {
                     bzj = 0;
                 }
                 if (zgs == null) {
                 ...
                 var tbzj = bzj - zgs - data

                 dw_list.SetItem(row, "tbzj", tbzj)
             };
             if (dwoName == "zgs") {
                 var bzj = dw_list.GetItemNumber(row, "bzj")
                 var zzzs = dw_list.GetItemNumber(row, "zzzs")
                 if (bzj == null) {
                     bzj = 0;
                 }
                 if (zzzs == null) {
    ...
    //#region 获得字段焦点
    this.Help = function () {
        javascript: window.open('\空运保证金.txt')
    }
    //#endregion
