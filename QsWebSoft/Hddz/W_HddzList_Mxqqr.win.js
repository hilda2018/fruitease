...
function W_HddzList_Mxqqr() {
    ...
    var ddlb_mxqzt = new DropDownListBox(requestor);
    ...
    this.bind = function (win) {
        ...
        if (ddlb_mxqzt == null) ddlb_mxqzt = new DropDownListBox(requestor);
        ddlb_mxqzt.Attach(win.Control("ddlb_mxqzt"));
        ...
        if (typeof (self.Retrieve) == "function")
            win.AttachEvent("ddlb_mxqzt", "SelectionChanged", self.Retrieve);
    ...
    this.unbind = function () {
        ...
        ddlb_mxqzt.Detach(); ddlb_mxqzt = null;
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), mxqzt, Dlwtf);
        dw_log.Retrieve(userid, "mxqr");
    }
    //#endregion
    ...
    this.dw_List_ItemChanged = function (Row, dwoName, data) {
        if (Row > 0) {
            if (dwoName == "mxq") {
                var userid = requestor.GetParm("userid");
                var date = new Date()
                dw_list.SetItem(Row, "mxq_lrry", userid);
                dw_list.SetItem(Row, "mxq_lrsj", date.getVarDate());
            }
        }
    }
    ...
    this.Save = function () {
        ...
        for (var i = 1; i <= dw_list.RowCount(); i++) {
            if (dw_list.GetItemStatus(i, 0, DWBUFFER.Primary) == 1) {
                ...
                dw_log.SetItem(row, "changecols", "mxq,khmxq");
        ...
        webReq.Invoke("ListMxqqrSave", "dw_list=" + dw_d_data + "&dw_log=" + dw_log_data);
    ...
    //#region 获得字段焦点
    this.Help = function () {
        javascript: window.open('\超期费免箱期确认.txt')
    }
    //#endregion
