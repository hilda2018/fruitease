...
function W_HddzList_Hd_Ky()
{

    var self = this;
    var requestor = new PBWindow();
    var dw_yjkgsj = new DataWindow( requestor );
    ...
    this.bind = function ( win )
    {
        ...
        if ( dw_yjkgsj == null ) dw_yjkgsj = new DataWindow( requestor );
        dw_yjkgsj.Attach( win.Control( "dw_yjkgsj" ) );
        ...
        if ( typeof ( self.Dhfpcf ) == "function" )
            win.AttachEvent( "btn_dhfpcf", "Clicked", self.Dhfpcf );
    ...
    this.unbind = function ()
    {
        ...
        dw_yjkgsj.Detach(); dw_yjkgsj = null;
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), hdzt, userid,jdrjc);
        dw_log.Retrieve(userid, "kyhd");
    }
    //#endregion
        ...
        if (Row > 0) {
            if (dwoName == "ywbh") {
                ...
                if (readonly == "no") {
                    ...
                    iw_Wldw_Select.SetData("hddz_ky_xhgq");
    ...
    this.dw_List_ItemChanged = function (row, dwoName, data) {
        if (dwoName == "yjkgsj") {
            var ts = dw_list.GetItemString(row, "ts")
            var ywbh = dw_list.GetItemString(row, "ywbh")
            ...
            QsWebSoft.PubMethod.GenYjkgsjjhczsj(ywbh, data, htjhthsj_lb);

        }
        if (dwoName == "kyhd_sfyccf") {
            var sfcf = dw_list.GetItemNumber(row, "sfcf");
            var kyhd_beizhu = "";
            kyhd_beizhu = dw_list.GetItemString(row, "kyhd_beizhu");
            var ywbh = dw_list.GetItemString(row, "ywbh");
            var cxh = dw_list.GetItemNumber(row, "yw_hddz_kyhd_cxh");

            if (sfcf > 0) {
                if (kyhd_beizhu == null) {
                    kyhd_beizhu = "";
                }
                QsWebSoft.PubMethod.UpdateHddzKyCf(ywbh,cxh, data, kyhd_beizhu);
            } else {
                if (kyhd_beizhu == null) {
                    kyhd_beizhu = "";
                }

                QsWebSoft.PubMethod.InsertHddzKyCf(ywbh, data, kyhd_beizhu);
            }

        }

        if (dwoName == "kyhd_beizhu") {
            var sfcf = dw_list.GetItemNumber(row, "sfcf");
            var kyhd_sfyccf = "";
            kyhd_sfyccf = dw_list.GetItemString(row, "kyhd_sfyccf");
            var ywbh = dw_list.GetItemString(row, "ywbh");
            var cxh = dw_list.GetItemNumber(row, "yw_hddz_kyhd_cxh");
            ...
            if (sfcf > 0) {
                ...
                QsWebSoft.PubMethod.UpdateHddzKyCf(ywbh, cxh, kyhd_sfyccf, data);
            } else {
                if (kyhd_sfyccf == null) {
                    kyhd_sfyccf = "";
                }

                QsWebSoft.PubMethod.InsertHddzKyCf(ywbh, kyhd_sfyccf, data);
            }
        ...
        if (dwoName == "yjkgsj") {
            dw_list.AcceptText();
            var sj = new Date();
            dw_list.SetItem(row, "yjkgsjsrsj", sj.getVarDate());
        }
        if (dwoName == "sjkgsj") {
            dw_list.AcceptText();
            var ywy = requestor.GetParm("userid");
            var date = new Date();
            dw_list.SetItem(row, "sjkgsj_lrsj", date.getVarDate())
            dw_list.SetItem(row, "sjkgsj_czry", ywy)
        }
    }
    //#endregion
    ...
    this.Save = function ()
    {
        ...
        {
            if ( dw_list.GetItemStatus( i, "yjkgsj", DWBUFFER.Primary ) == 1 || dw_list.GetItemStatus( i, "sjkgsj", DWBUFFER.Primary ) == 1 )
            {
                var insertrow = dw_yjkgsj.InsertRow( 0 );
                dw_yjkgsj.SetItem( insertrow, "ywbh", dw_list.GetItemString( i, "ywbh" ) );
                dw_yjkgsj.SetItem( insertrow, "zbr", userid );
                dw_yjkgsj.SetItem( insertrow, "zbrq", date.getVarDate() );
                dw_yjkgsj.SetItem( insertrow, "yjkgsj", dw_list.GetItemDate( i, "yjkgsj" ) );
                dw_yjkgsj.SetItem( insertrow, "sjkgsj", dw_list.GetItemDate( i, "sjkgsj" ) );
            }
        }
        ...
        for ( var i = 1; i <= dw_list.RowCount(); i++ )
        {
            if ( dw_list.GetItemStatus( i, "hcorhbh", DWBUFFER.Primary ) == 1 || dw_list.GetItemStatus( i, "yjkgsj", DWBUFFER.Primary ) == 1 || dw_list.GetItemStatus( i, "sjkgsj", DWBUFFER.Primary ) == 1 || dw_list.GetItemStatus( i, "kyhdsj", DWBUFFER.Primary ) == 1 || dw_list.GetItemStatus( i, "sjhdsj", DWBUFFER.Primary ) == 1 || dw_list.GetItemStatus( i, "bjyw", DWBUFFER.Primary ) == 1 )
            {
                ...
                dw_log.SetItem( row, "changecols", "hcorhbh,yjkgsj,sjkgsj,kyhdsj,sjhdsj,bjyw" );
        ...
        dw_yjkgsj.AcceptText()
        var dw_yjkgsj_data = dw_yjkgsj.GetChanges();  //获取改变的
        //        if (dw_yjkgsj_data == "")

        //执行保存的WebService
        webReq.SetFunctionID( requestor.GetFunctionID() );
        webReq.Invoke( "ListHdkySave", "dw_list=" + dw_d_data + "&dw_log=" + dw_log_data + "&dw_yjkgsj=" + dw_yjkgsj_data );
    ...
    //#region 获得字段焦点
    this.Help = function () {
        javascript: window.open('\空运到港换单.txt')
    }
    //#endregion
    ...
    this.Dcrz = function ()
    {
        ...
        var num = QsWebSoft.PubMethod.Dcrq( userid, "空运换单", sjts ).value;
    ...
    //#region 到货分批拆分
    this.Dhfpcf = function () {
        if (iw_HddzList_Hd_Kycf == null)
            iw_HddzList_Hd_Kycf = new W_HddzList_Hd_Kycf();
        var row = dw_list.GetRow();
        var Dlwtf = requestor.GetParm("Dlwtf");
        var ywbh = dw_list.GetItemString(row, "ywbh")
        var zbr = dw_list.GetItemString(row, "zbr")
        var userid = requestor.GetParm("userid");  
        iw_HddzList_Hd_Kycf.SetZbr(zbr);
        requestor.Open("/LoadWindow.ashx?WinName=Hddz.W_HddzList_Hd_Kycf&ywbh="+ywbh, iw_HddzList_Hd_Kycf);
