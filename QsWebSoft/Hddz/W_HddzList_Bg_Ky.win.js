...
function W_HddzList_Bg_Ky()
{
    ...
    this.bind = function ( win )
    {
        ...
        if ( typeof ( self.OpenYtjk ) == "function" )
            win.AttachEvent( "btn_dkytjk", "Clicked", self.OpenYtjk );
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), bgzt, Dlwtf, userid, jdrjc);
    ...
    this.dw_List_ItemChanged = function (row, dwoName, data) {
        ...
        if (dwoName == "kylhsj") {
            ...
            var date = new Date();
            dw_list.SetItem(row, "kylhsj_lrsj", date.getVarDate())
            var userid = requestor.GetParm("userid");
            dw_list.SetItem(row, "kylhsj_srry", userid)
        }
        if (dwoName == "sfjc") {

            var date = new Date();
            dw_list.SetItem(row, "jclrsj", date.getVarDate())
            var userid = requestor.GetParm("userid");
            dw_list.SetItem(row, "jclrry", userid)
        }
    ...
    this.Save = function () {
        ...
        webReq.Invoke("ListBgKySave", "dw_list=" + dw_d_data + "&dw_log=" + dw_log_data);
    ...
    this.Dcrz = function ()
    {
        ...
        var num = QsWebSoft.PubMethod.Dcrq( userid, "空运报关", sjts ).value;
