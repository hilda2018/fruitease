...
function W_HddzList_Cdgl() {
    ...
    var cbx_qx = new CheckBox(requestor);
    ...
    var sle_zzcm = new SingleLineEdit(requestor);
    var sle_zzhc = new SingleLineEdit(requestor);
    ...
    this.bind = function (win) {
        ...
        if (cbx_qx == null) cbx_qx = new CheckBox(requestor);
        cbx_qx.Attach(win.Control("cbx_qx"));
        ...
        if (sle_zzcm == null) sle_zzcm = new SingleLineEdit(requestor);
        sle_zzcm.Attach(win.Control("sle_zzcm"));
        if (sle_zzhc == null) sle_zzhc = new SingleLineEdit(requestor);
        sle_zzhc.Attach(win.Control("sle_zzhc"));
        ...
        if (typeof (self.slecdgs_EditChanged) == "function")
            win.AttachEvent("sle_cdgs", "EditChanged", self.slecdgs_EditChanged);

        if (typeof (self.Qx) == "function")
            win.AttachEvent("cbx_qx", "Clicked", self.Qx);

        if (typeof (self.Update) == "function")
            win.AttachEvent("btn_update", "Clicked", self.Update);
        ...
        if (typeof (self.slehc_EditChanged) == "function")
            win.AttachEvent("sle_hc", "EditChanged", self.slehc_EditChanged);

        if (typeof (self.slezzcm_EditChanged) == "function")
            win.AttachEvent("sle_zzcm", "EditChanged", self.slezzcm_EditChanged);

        if (typeof (self.slezzhc_EditChanged) == "function")
            win.AttachEvent("sle_zzhc", "EditChanged", self.slezzhc_EditChanged);
    ...
    webReq.SetWebServiceUrl("/Service/Cdgl.ashx");   
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), dgzt, jdrjc);
    ...
    //#region 过滤航次
    this.slehc_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
    ...
    //#region 过滤中转船名
    this.slezzcm_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(zz_cm) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    //#region 过滤中转航次
    this.slezzhc_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(zz_hcorhbh) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    //#region 过滤船代公司
    this.slecdgs_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(cdgs) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.dw_List_Clicked = function (xPos, yPos, Row, dwoName, Data) {
        dw_list.AcceptText();
        //        var sfdf = dw_list.GetItemString(Row, "qrdf");
        if (Row > 0) {
            //            if (dwoName == "qrdf") {
            //                requestor.MessageBox("", Data);
            //                if (Data == "Y") {
            //                    var date = new Date();
            //                    dw_list.SetItem(Row, "qrdfsj", date.getVarDate());
            //                } else if (sfdf == "N" || sfdf == "" || sfdf == null) {
            //                    dw_list.SetItem(Row, "qrdfsj", "");
            //                }
            //            }
            fzks_row = Row;
        };
    ...
    this.Update = function () {
        ...
        iw_Wldw_Select.SetData("cdgl_cdgs");
