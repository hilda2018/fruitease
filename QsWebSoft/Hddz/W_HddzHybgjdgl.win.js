...
function W_HddzHybgjdgl() {
    ...
    var cbx_yfx = new CheckBox(requestor);
    ...
    this.bind = function (win) {
        ...
        if (cbx_yfx == null) cbx_yfx = new CheckBox(requestor);
        cbx_yfx.Attach(win.Control("cbx_yfx"));
        ...
        if (typeof (self.Retrieve) == "function")
            win.AttachEvent("cbx_yfx", "Clicked", self.Retrieve);
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), ts, wdg, dgzt, yfx, jdrjc, userid);
        //        var ywbh = sle_ywbh.GetText();
        //        var kh = sle_kh.GetText();
        //        var tdh = sle_tdh.GetText();
        //        var jzxh = sle_jzxh.GetText();
        //        self.sle_SelectionChanged();
        //        self.sle_EditChanged(ywbh);
        //        self.slekh_EditChanged(kh);
        //        self.sletdh_EditChanged(tdh);
        //        self.slejzxh_EditChanged(jzxh);
    }
    //#endregion
    ...
    this.slekh_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(khmc) like Upper('%" + text + "%')) or ( khmc like '%" + text + "%')");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.dw_List_ItemChanged = function (Row, dwoName, Data) {
        if (dwoName == "jsbz") {
            var ywbh = dw_list.GetItemString(Row, "ywbh");
            var num = QsWebSoft.PubMethod.UpdateJsbz(ywbh, Data).value;
