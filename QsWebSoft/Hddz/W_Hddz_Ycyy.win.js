...
//var jzxh = null;
//this.setDataJzxh = function (parm) {
//    jzxh = parm;
//}
...
this.Add = function () {
    ...
    ds_role.Retrieve(userid, "wyqx0001");
    if (ds_role.RowCount() > 0) {
        qxqx = "Y";
    }

    dw_list.Retrieve(column, yybm, qxqx, cxh)
    dw_list.AcceptText()
...
this.save = function () {
    ...
    for (row = dw_list.RowCount(); row > 0; row--) {
        ...
        //dw_list.SetItem(row, "jzxh",jzxh);

        var yy_jsl = dw_list.GetItemString(row, "yy_jsl");
        dw_list.SetItem(row, "yy", yy_jsl);
    ...
    for (var i = 1; i <= dw_list.RowCount(); i++) {
        ...
        if (yy == "N") {
            if (dw_list.GetItemStatus(i, 0, DWBUFFER.Primary) == 1 || dw_list.GetItemStatus(i, 0, DWBUFFER.Primary) == 3) {
                ...
                dw_log.SetItem(row, "parameters", yybm);
    ...
    webReq.Invoke("YcyySave", "dw_list=" + dw_d_data + "&dw_log=" + dw_log_data);
