...
function W_HddzList_Fx_Ky() {
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), fxzt, Dlwtf, userid,jdrjc);
    ...
    this.dw_List_ItemChanged = function (row, dwoName, data) {
        if (dwoName == "hgfxfs") {
            ...
            if (data == "查验放行") {
                //                var xhgqbm = dw_list.GetItemString(row, "admin0624")
                ...
                dw_list.SetItem(row, "cydbm", "admin0624")
                dw_list.SetItem(row, "cyd", "浦东机场西区海关查验点")
                dw_list.SetItem(row, "cydjc", "西区查验点")
                dw_list.SetItem(row, "cydpym", "pdjcxqhgcyd")
            };
        ...
        if (dwoName == "hgfxsj") {
            ...
            var hgfxsj_srsc = dw_list.GetItemNumber(row, "hgfxsj_srsc");
            if (hgfxsj_srsc > 30 || hgfxsj_srsc < -10) {
                requestor.MessageBox("提示", "你输入的放行时间已经超出正常应输入时间，请检查是否正确！")
                return 1
            }
    ...
    this.Save = function () {
        ...
        webReq.Invoke("ListKyfxSave", "dw_list=" + dw_d_data );
    ...
    //#region 获得字段焦点
    this.Help = function () {
        javascript: window.open('\空运放行.txt')
    }
    //#endregion   
