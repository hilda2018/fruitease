...
function W_HddzList_Spcx() {
    ...
    var ddlb_spcxzt = new DropDownListBox(requestor);
    ...
    this.bind = function (win) {
        ...
        if (ddlb_spcxzt == null) ddlb_spcxzt = new DropDownListBox(requestor);
        ddlb_spcxzt.Attach(win.Control("ddlb_spcxzt"));
        ...
        if (typeof (self.slespmc_EditChanged) == "function")
            win.AttachEvent("sle_tdh", "EditChanged", self.slespmc_EditChanged);

        if (typeof (self.slespywmc_EditChanged) == "function")
            win.AttachEvent("sle_2", "EditChanged", self.slespywmc_EditChanged);

        if (typeof (self.sleycd_EditChanged) == "function")
            win.AttachEvent("sle_3", "EditChanged", self.sleycd_EditChanged);
        ...
        if (typeof (self.Retrieve) == "function")
            win.AttachEvent("ddlb_spcxzt", "SelectionChanged", self.Retrieve);

        if (typeof (self.Retrieve) == "function")
            win.AttachEvent("ddlb_sjzt", "SelectionChanged", self.Retrieve);

        if (typeof (self.slegwgys_EditChanged) == "function")
            win.AttachEvent("sle_4", "EditChanged", self.slegwgys_EditChanged);
    ...
    this.unbind = function () {
        ...
        ddlb_spcxzt.Detach(); ddlb_spcxzt = null;
    ...
    this.Retrieve = function () {
        ...
        var spcxzt = ddlb_spcxzt.GetText();
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), spcxzt, sjzt, Dlwtf, Tybm, Sfqytybm, userid, jdrjc);
    ...
    //#region 获得字段焦点
    this.Help = function () {
        javascript: window.open('\按商品查询.txt')
    }
    //#endregion
