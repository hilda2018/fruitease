...
function W_HddzList_Tggz_Select() {
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("sle_1","Modified",self.Retrieve); 

		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("sle_hth","Modified",self.Retrieve); 

		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("sle_tdh","Modified",self.Retrieve); 

		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("sle_bgdh","Modified",self.Retrieve); 

		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("sle_khjc","Modified",self.Retrieve); 

		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("sle_spmc","Modified",self.Retrieve); 
		 ...
		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("sle_hz_jzxh","Modified",self.Retrieve); 
    ...
    this.ddlb_fxzt_SelectionChanged = function () {
        ...
        if (fxzt == "") {
            ...
            dw_list.SetFilter("( Upper(tgfx) like Upper('%" + fxzt + "%'))")
        };
        self.ddlb_wlzt_SelectionChanged;
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    ...
    //#region 物流状态
    this.ddlb_wlzt_SelectionChanged = function () {
        ...
        if (wlzt == "") {
            ...
            dw_list.SetFilter("( Upper(wlzt) like Upper('%" + wlzt + "%'))")
        };
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    ...
    this.OpenTggz = function () {
        ...
        if (iW_Wldw_Tggz == null) {
            iW_Wldw_Tggz = new W_Wldw_Tggz();
        }

        requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Wldw_Tggz&ywbh=" + ywbh + '&hddz_cxh=' + cxh.toString(), iW_Wldw_Tggz);
        dw_list.SetFocus();
    ...
    //#region 过滤客户
    this.slekhjc_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(yw_hddz_khjc) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.sle_hz_jzxh_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(yw_hddz_jzxxx_jzxh) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Save = function () {
        ...
        for (var i = 1; i <= dw_list.RowCount(); i++) {
            if (dw_list.GetItemStatus(i, 0, DWBUFFER.Primary) == 1) {
                ...
                dw_log.SetItem(row, "changecols", "hgfxfs,hgfxsj,qrdfsj");
        ...
        webReq.Invoke("YljzxwlgzSaves", "dw_list=" + dw_d_data + "&dw_log=" + dw_log_data);
