...
function W_HddzList_Fx() {
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(typeof(self.sle_cm_EditChanged)=="function") 
			 win.AttachEvent("sle_cm","EditChanged",self.sle_cm_EditChanged); 

		 if(typeof(self.Qbfx)=="function") 
			 win.AttachEvent("btn_qbfx","Clicked",self.Qbfx); 
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), fxzt, userid, jdrjc);
        dw_log.Retrieve(userid, "hyfx");
    }
    //#endregion
    ...
    this.dw_List_DoubleClicked = function (xPos, yPos, Row, dwoName) {
        if (Row > 0) {
            if (dwoName == "cyd") {
                ...
                self.OpenWindow();
            } else if (dwoName == "hgfxsj") {
                var bgsj_new = new Date();
                dw_list.SetItem(Row, "hgfxsj", bgsj_new.getVarDate());
            }
    ...
    this.dw_List_ItemChanged = function (row, dwoName, data) {
        ...
        if (dwoName == "pcyjhsj") {
            var date = new Date();
            var userid = requestor.GetParm("userid");            
            var pcyjhsj = dw_list.GetItemDate(dw_list.GetRow(), "pcyjhsj");
            dw_list.SetItem(dw_list.GetRow(), "pcyjhsj_tjry", userid);
            dw_list.SetItem(dw_list.GetRow(), "pcyjhsj_tjsj", date.getVarDate());
        }
    ...
    //#region 按船名过滤
    this.sle_cm_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(cm) like Upper('%" + text + "%')) or ( Upper(zz_cm) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
...
//    }
    ...
    this.Save = function () {
        ...
        webReq.Invoke("ListFxSave", "dw_list=" + dw_d_data + "&dw_log=" + dw_log_data);
    ...
    this.Qbfx = function () {
        var rq = new Date();
        for (row = 1; row <= dw_list.RowCount(); row++) {
            dw_list.SetItem(row, "hgfxfs", "正常放行");

            dw_list.SetItem(row, "hgfxsj", rq.getVarDate());
