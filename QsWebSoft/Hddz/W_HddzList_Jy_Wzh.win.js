...
function W_HddzList_Jy_Wzh() {
    ...
    this.bind = function (win) {
        ...
        if (typeof (self.sletgdh_EditChanged) == "function")
            win.AttachEvent("sle_tgdh", "Modified", self.sletgdh_EditChanged);
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), jyzt, ysfs, userid, jdrjc);
    ...
    this.dw_List_ItemChanged = function (row, dwoName, data) {
        if (dwoName == "bjh") {
            var date = new Date();
            dw_list.SetItem(row, "bjsj", date.getVarDate())
        }
        if (dwoName == "tgdh") {
            var tgdh = data;
...
//            dw_list.SetItem(row, "bjh", bjh)
        ...
        if (dwoName == "gjsbzt") {
            ...
            if (data == "交单") {
                dw_list.SetItem(row, "wzhjdsj", date.getVarDate())
            };
            if (data == "查验C") {
                dw_list.SetItem(row, "wzhfxsj", date.getVarDate())
                var wzhjdsj = dw_list.GetItemDate(row, "wzhjdsj");
                if (wzhjdsj == null || Date(wzhjdsj) == Date("1900/1/1")) {                  
                    dw_list.SetItem(row, "wzhjdsj", date.getVarDate())
                }  
            }
    ...
    this.Save = function () {
        ...
        webReq.Invoke("ListJyWzhSave", "dw_list=" + dw_d_data );
    ...
    //#region 获得字段焦点
    this.Help = function () {
        javascript: window.open('\检疫无纸化放行模块.txt')
    }
    //#endregion
