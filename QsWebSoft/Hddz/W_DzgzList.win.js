...
function W_DzgzList() {
    ...
    var cbx_wdg = new CheckBox(requestor);
    var cbx_dgzt = new CheckBox(requestor);
    var cbx_zj = new CheckBox(requestor);
    var ddlb_dzzt = new DropDownListBox(requestor);
    ...
    this.bind = function (win) {
        ...
        if (cbx_wdg == null) cbx_wdg = new CheckBox(requestor);
        cbx_wdg.Attach(win.Control("cbx_wdg"));
        if (cbx_dgzt == null) cbx_dgzt = new CheckBox(requestor);
        cbx_dgzt.Attach(win.Control("cbx_dgzt"));
        if (cbx_zj == null) cbx_zj = new CheckBox(requestor);
        cbx_zj.Attach(win.Control("cbx_zj"));
        if (ddlb_dzzt == null) ddlb_dzzt = new DropDownListBox(requestor);
        ddlb_dzzt.Attach(win.Control("ddlb_dzzt"));
        ...
        if (typeof (self.dw_bsnr_DoubleClicked) == "function")
            win.AttachEvent("dw_bsnr", "DoubleClicked", self.dw_bsnr_DoubleClicked);
        ...
        if (typeof (self.Retrieve) == "function")
            win.AttachEvent("cbx_wdg", "Clicked", self.Retrieve);

        if (typeof (self.Retrieve) == "function")
            win.AttachEvent("cbx_dgzt", "Clicked", self.Retrieve);

        if (typeof (self.sle_SelectionChanged) == "function")
            win.AttachEvent("cbx_zj", "Clicked", self.sle_SelectionChanged);

        if (typeof (self.sle_SelectionChanged) == "function")
            win.AttachEvent("ddlb_dzzt", "SelectionChanged", self.sle_SelectionChanged);

        if (typeof (self.sle_SelectionChanged) == "function")
            win.AttachEvent("ddlb_tdzt", "SelectionChanged", self.sle_SelectionChanged);

        if (typeof (self.sle_SelectionChanged) == "function")
            win.AttachEvent("ddlb_sdzt", "SelectionChanged", self.sle_SelectionChanged);

        if (typeof (self.sle_SelectionChanged) == "function")
            win.AttachEvent("ddlb_ctr_area2", "SelectionChanged", self.sle_SelectionChanged);
    ...
    this.unbind = function () {
        ...
        cbx_wdg.Detach(); cbx_wdg = null;
        cbx_dgzt.Detach(); cbx_dgzt = null;
        cbx_zj.Detach(); cbx_zj = null;
        ddlb_dzzt.Detach(); ddlb_dzzt = null;
    ...
    this.Retrieve = function () {
        ...
        var dgzt = "N"
        if (cbx_dgzt.GetChecked() == true) {
            dgzt = "Y";
        }

        var wdg = "N"
        if (cbx_wdg.GetChecked() == true) {
            wdg = "Y";
        }
        var ts = parseInt(sle_ts.GetText());
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), Dlwtf, Tybm, Sfqytybm, dgzt, wdg, ts, jdrjc);
    ...
    this.CloseQuery = function () {
        ...
        if ((dw_bsnr.ModifiedCount() + dw_bsnr.DeletedCount()) > 0) {
    ...
    this.AddRow = function () {
        ...
        var insertrow = dw_bsnr.InsertRow(0);
        dw_bsnr.SetItem(insertrow, "ywbh", ywbh);
        dw_bsnr.SetItem(insertrow, "cxh", cxh);
        dw_bsnr.ScrollToRow(insertrow);
        dw_bsnr.SetFocus();
    ...
    this.DeleteRow = function () {
        var status = dw_bsnr.GetItemStatus(row, 0, DWBUFFER.Primary);
    ...
    this.dw_List_RowFocusChanged = function (CurrentRow) {
        ...
        var cxh = dw_list.GetItemNumber(CurrentRow, "cxh"); 
        dw_bsnr.Retrieve(ywbh, cxh.toString());

    }
    

    //#region 过滤
    this.sle_SelectionChanged = function (text) {

        dw_list.SetRedraw(false);

        var dzzt = ddlb_dzzt.GetText();
        var tdzt = ddlb_tdzt.GetText();
        var sdzt = ddlb_sdzt.GetText();
        var ctr_area2 = ddlb_ctr_area2.GetText();
        var zj = cbx_zj.GetChecked();
        var userid = requestor.GetParm("userid");
        var ls_filter = "";
        if (dzzt == "" || dzzt == "全部") {
            ls_filter = "";
        ...
        if (tdzt == "" || tdzt == "全部") {
            ...
            if (ls_filter == "") {
                ls_filter = "tdzt_jsl = '" + tdzt + "'";
            } else {
                ls_filter = ls_filter + " and tdzt_jsl = '" + tdzt + "'";
            }
        }
        if (sdzt == "" || sdzt == "全部") {
            ls_filter = ls_filter;
            ...
            if (ls_filter == "") {
                ...
                ls_filter = ls_filter + " and sdzt = '" + sdzt + "'";
            }
        }
        ...
        if (ctr_area2 == "" || ctr_area2 == "全部") {
            ...
            if (ls_filter == "") {
                ls_filter = "ctr_area2 = '" + ctr_area2 + "'";
            } else {
                ls_filter = ls_filter + " and ctr_area2 = '" + ctr_area2 + "'";
            }
        }
        ...
        if (zj == false) {
            ...
            if (ls_filter == "") {
                ls_filter = "zbr = '" + userid + "'";
            } else {
                ls_filter = ls_filter + " and zbr = '" + userid + "'";
            }
        }
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Hddz.W_DzgzList", "dw_dzgz_list");
        self.Retrieve();
    }
    //#endregion
    ...
    this.Save = function () {
        ...
        for (row = dw_bsnr.RowCount(); row > 0; row--) {
            dw_bsnr.SetItem(row, "cxh_mx", row);
        }
        ...
        webReq.Invoke("ListDzgzSave", "dw_bsnr=" + dw_bsnr_data);
    ...
    //#region 双击
    this.dw_bsnr_DoubleClicked = function (xPos, yPos, Row, dwoName) {
         if (Row > 0)
             if (iw_Hddz_Dzsd == null)
                 iw_Hddz_Dzsd = new W_Hddz_Dzsd();

             var Dlwtf = requestor.GetParm("Dlwtf");
             var ywbh = dw_bsnr.GetItemString(Row, "ywbh")
             var cxh = dw_bsnr.GetItemNumber(Row, "cxh")
             var userid = "AAAA";

             iw_Hddz_Dzsd.SetDataWindow(dw_bsnr);
