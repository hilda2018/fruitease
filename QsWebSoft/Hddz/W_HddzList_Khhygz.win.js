...
function W_HddzList_Khhygz() {
	 ...
	 var cbx_jrjhthwcgq= new CheckBox( requestor );
	 ...
	 var ddlb_thjhzt= new DropDownListBox( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(cbx_jrjhthwcgq == null ) cbx_jrjhthwcgq= new CheckBox( requestor );
		 cbx_jrjhthwcgq.Attach( win.Control( "cbx_jrjhthwcgq") ); 
		 ...
		 if(ddlb_thjhzt == null ) ddlb_thjhzt= new DropDownListBox( requestor );
		 ddlb_thjhzt.Attach( win.Control( "ddlb_thjhzt") ); 
		 ...
		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("cbx_jrjhthwcgq","Clicked",self.Retrieve); 
		 ...
		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("ddlb_tgzt","Modified",self.Retrieve); 

		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("ddlb_wlzt","Modified",self.Retrieve); 

		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("ddlb_wllx","Modified",self.Retrieve); 

		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("ddlb_thjhzt","Modified",self.Retrieve); 
    ...
    this.Retrieve = function () {
        ...
        var thjhzt = ddlb_thjhzt.GetText();
        var rqlx = ddlb_rqlx.GetText();
        var jdrjc = ddlb_jdrjc.GetText();
        var jrjhthwcgq = "N";
        if (cbx_jrjhthwcgq.GetChecked()) {
            jrjhthwcgq = "Y";
        }
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), Dlwtf, tgzt, wlzt, wllx, thjhzt, Tybm, Sfqytybm, rqlx, jrjhthwcgq, jdrjc, userid);
...
//        }
    ...
    //#region 过滤商品名称
    this.slespmc_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(spmc) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    //#region 过滤商品英文名称
    this.slespywmc_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(spmc_yw) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    //#region 获得字段焦点
    this.Help = function () {
        javascript: window.open('\客户海运跟踪.txt')
    }
    //#endregion
