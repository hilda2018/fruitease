...
function W_HddzList_Hd() {
	 ...
	 var cbx_xsdet= new CheckBox( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(cbx_xsdet == null ) cbx_xsdet= new CheckBox( requestor );
		 cbx_xsdet.Attach( win.Control( "cbx_xsdet") ); 
		 ...
		 if(typeof(self.jydwmc_EditChanged)=="function") 
			 win.AttachEvent("sle_3","EditChanged",self.jydwmc_EditChanged); 
		 ...
		 if(typeof(self.Retrieve)=="function") 
			 win.AttachEvent("cbx_xsdet","Clicked",self.Retrieve); 

		 if(typeof(self.Szyjhdsj)=="function") 
			 win.AttachEvent("btn_szyjhdsj","Clicked",self.Szyjhdsj); 
	 ...
	 if(typeof(baseUnBind)=="function") baseUnBind(); 
		 ...
		 cbx_xsdet.Detach(); cbx_xsdet=null; 
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), hdzt, xsdthd_retrieve, xsdet_retrieve, userid, jdrjc);
        dw_log.Retrieve(userid, "hyhd");
    }
    //#endregion
    ...
    this.dw_List_ItemChanged = function (row, dwoName, data) {
        if (dwoName == "sjkgsj") {
            ...
            for (row1 = dw_list.RowCount(); row1 > 0; row1--) {
                ...
                if (ls_sjkgsj == "1900-01-01" && cm_dqh == cm) {
                    ...
                    dw_list.SetItem(row1, "sjkgsj_lrsj", date_new.getVarDate());
                    dw_list.SetItem(row1, "sjkgsj_czry", ywy);
                }
            }
            dw_list.SetItem(row, "sjkgsj_lrsj", date_new.getVarDate());
        ...
        if (dwoName == "yjkgsj") {
            ...
            dw_list.SetItem(row, "yjkgsj_lrsj", sj.getVarDate());
            dw_list.SetItem(row, "yjkgsj_czry", sj.getVarDate());
        }
    ...
    this.jydwmc_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(jydwmc) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Save = function () {
        ...
        for (var i = 1; i <= dw_list.RowCount(); i++) {
            if (dw_list.GetItemStatus(i, 0, DWBUFFER.Primary) == 1) {
                ...
                dw_log.SetItem(row, "changecols", "yjkgsj,sjkgsj,sjhdsj,xhgqbm,xhgq,xhgqjc,xhgqpym");
        ...
        webReq.Invoke("ListHdSave", "dw_list=" + dw_d_data + "&dw_log=" + dw_log_data);
    ...
    //#region 设置预计换单时间
    this.Szyjhdsj = function () {
        if (iw_Set_Rq == null)
            iw_Set_Rq = new W_Set_Rq();


        iw_Set_Rq.SetDataWindow(dw_list);
        iw_Set_Rq.SetData("yghd");
        requestor.Open("/LoadWindow.ashx?WinName=Hddz.W_Set_Rq", iw_Set_Rq);
