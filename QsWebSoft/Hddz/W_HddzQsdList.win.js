...
function W_HddzQsdList() {
    ...
    this.OpenWindow = function () {
        ...
        if (dw_list.RowCount() <= 0)
            return;
        var qsdbh = dw_list.GetItemString(dw_list.GetRow(), 'qsdbh')
         var ywy = "" 
        ...
        if (openStyle == "2")
            ...
            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&qsdbh=' + qsdbh + '&operation=' + operation.toString(), true);
...
//    }
    ...
    this.Delete = function () {
        ...
        var qsdbh = dw_list.GetItemString(row, "qsdbh");
        if (requestor.MessageBox("提示", "删除签收单编号为<" + qsdbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;

        //执行删除的WebService
        webReq.SetFunctionID(requestor.GetFunctionID());
        webReq.Invoke("Delete", "qsdbh=" + qsdbh);
    ...
    this.Print = function () {
        ...
        dw_print.Retrieve(dw_list.GetItemString(dw_list.GetRow(), "qsdbh"));
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(qsdbh) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.slekh_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(jsdwmc) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Hddz.W_HddzQsdList", "dw_hddz_hdfyhxdcf_list");
        self.Retrieve();
    }
    //#endregion
    ...
    this.Tj = function () {
        ...
        var qsdbh = dw_list.GetItemString(row, "qsdbh")
        var num = QsWebSoft.PubMethod.GenQsd(qsdbh).value;
        if (num == "1") {
