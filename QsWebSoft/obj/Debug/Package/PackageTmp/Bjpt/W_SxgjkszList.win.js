...
function W_SxgjkszList() {
    ...
    this.bind = function (win) {
        ...
        if (typeof (self.sleid_EditChanged) == "function")
            win.AttachEvent("sle_1", "EditChanged", self.sleid_EditChanged);

        if (typeof (self.slebh_EditChanged) == "function")
            win.AttachEvent("sle_kh", "EditChanged", self.slebh_EditChanged);

        if (typeof (self.slejk_EditChanged) == "function")
            win.AttachEvent("sle_hth", "EditChanged", self.slejk_EditChanged);
        ...
        if (typeof (self.Tj) == "function")
            win.AttachEvent("btn_prn", "Clicked", self.Tj);
    ...
    this.Delete = function () {
        ...
        if (requestor.MessageBox("提示", "删除生鲜港接口设置编号为<" + id + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;
    ...
    //#region 过滤id
    this.sleid_EditChanged = function (text) {
    ...
    //#region 过滤编号
    this.slebh_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(code) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    //#region 过滤接口
    this.slejk_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(intername) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Commodity.W_SxgjkszList", "dw_sxgjksz_list");
        self.Retrieve();
    }
    //#endregion
