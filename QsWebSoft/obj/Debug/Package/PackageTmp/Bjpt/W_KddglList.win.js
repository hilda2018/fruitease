...
function W_KddglList() {
    ...
    this.OpenWindow = function () {
        ...
        if (dw_list.RowCount() <= 0)
            return;
        var kddbh = dw_list.GetItemString(dw_list.GetRow(), 'kddbh') 
        ...
        if (openStyle == "2")
            ...
            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&kddbh=' + kddbh + '&operation=' + operation.toString(), true);
    ...
    this.Delete = function () {
        ...
        var kddbh = dw_list.GetItemString(row, "kddbh");
        if (requestor.MessageBox("提示", "删除快递申请单号为<" + kddbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;

        //执行删除的WebService
        webReq.SetFunctionID(requestor.GetFunctionID());
        webReq.Invoke("Delete", "kddbh=" + kddbh);
    ...
    this.Print = function () {
        ...
        dw_print.Retrieve(dw_list.GetItemString(dw_list.GetRow(), "kddbh"));
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(kddbh) like '%" + text + "%')  or ( Lower(kddbh) like '%" + text + "%')");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Bjpt.W_KddglList", "dw_kddgl_list");
        self.Retrieve();
    }
    //#endregion
