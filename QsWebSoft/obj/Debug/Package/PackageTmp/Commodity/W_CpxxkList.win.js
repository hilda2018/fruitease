...
function W_CpxxkList() {
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(typeof(self.slegj_EditChanged)=="function") 
			 win.AttachEvent("sle_kh","EditChanged",self.slegj_EditChanged); 

		 if(typeof(self.slejyclyq_EditChanged)=="function") 
			 win.AttachEvent("sle_hth","EditChanged",self.slejyclyq_EditChanged); 
    ...
    this.OpenWindow = function () {
        ...
        if (dw_list.RowCount() <= 0)
            return;
        var cpxxkbm = dw_list.GetItemString(dw_list.GetRow(), 'cpxxkbm')
        ...
        if (openStyle == "2")
            ...
            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&cpxxkbm=' + cpxxkbm + '&operation=' + operation.toString(), true);
    ...
    this.dw_List_DoubleClicked = function (xPos, yPos, Row, dwoName) {
        if (Row > 0) {
             if (dwoName == "fj") {
                ...
                var ShareMode = requestor.GetParm("ShareMode");
                var Dlwtf = requestor.GetParm("Dlwtf");
                var cpxxkbm = dw_list.GetItemString(Row, "cpxxkbm");
                var lx = "cpxxk"
                requestor.Open("/LoadWindow.ashx?WinName=Hddz.W_Fj_Edit&ShareMode=" + ShareMode + "&ywy=" + ywy + "&Dlwtf=" + Dlwtf + "&ywbh=" + cpxxkbm + "&lx=" + lx, iw_Fj_Edit);
                dw_list.SetFocus();
...
//    }
    ...
    this.Delete = function () {
        ...
        var cpxxkbm = dw_list.GetItemString(row, "cpxxkbm");
        if (requestor.MessageBox("提示", "删除产品信息库编号为<" + cpxxkbm + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;

        //执行删除的WebService
        webReq.SetFunctionID(requestor.GetFunctionID());
        webReq.Invoke("Delete", "cpxxkbm=" + cpxxkbm);
    ...
    this.Print = function () {
        ...
        dw_print.Retrieve(dw_list.GetItemString(dw_list.GetRow(), "cpxxkbm"));
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(spbm) like '%" + text + "%')  or ( Lower(spbm) like '%" + text + "%') or ( Upper(spzwmc) like '%" + text + "%')  or ( Lower(spzwmc) like '%" + text + "%')  or ( Upper(spywmc) like '%" + text + "%')  or ( Lower(spywmc) like '%" + text + "%')");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.slegj_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(ctr_code) like '%" + text + "%')  or ( Lower(ctr_code) like '%" + text + "%') or ( Upper(ctr_cname) like '%" + text + "%')  or ( Lower(ctr_cname) like '%" + text + "%') or ( Upper(ctr_ename) like '%" + text + "%')  or ( Lower(ctr_ename) like '%" + text + "%')");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    //#region 过滤合同号
    this.slejyclyq_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(jyclyq) like '%" + text + "%')  or ( Lower(jyclyq) like '%" + text + "%')");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Commodity.W_CpxxkList", "dw_cpxxk_list");
        self.Retrieve();
    }
    //#endregion
...
//        }
//        var cpxxkbm = dw_list.GetItemString(row, "cpxxkbm")
////        var num = QsWebSoft.PubMethod.GenQtysk(cpxxkbm).value;
//        if (num == "1") {
