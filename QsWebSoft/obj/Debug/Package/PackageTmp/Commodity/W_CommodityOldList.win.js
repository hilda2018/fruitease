...
function W_CommodityOldList() {
    ...
    this.bind = function (win) {
        ...
        if (typeof (self.Filter) == "function")
            win.AttachEvent("m_1", "Clicked", self.Filter);

        if (typeof (self.Export) == "function")
            win.AttachEvent("导出ToolStripMenuItem", "Clicked", self.Export);
    ...
    this.OpenWindow = function () {
        ...
        var status = dw_list.GetItemString(row, "status");
        var userid = requestor.GetParm("userid");
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "") {
            ...
            dw_list.SetFilter("( Upper(name) like '%" + text + "%')   or ( Lower(name) like '%" + text + "%')   or ( Upper(spzwtc) like '%" + text + "%')   or ( Lower(spzwtc) like '%" + text + "%') or ( Upper(spywtc) like '%" + text + "%') or ( Lower(spywtc) like '%" + text + "%')");
        }
