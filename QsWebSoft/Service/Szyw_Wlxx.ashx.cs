...
namespace QsWebSoft.Service
{
    ...
    /// SalesOrder 的摘要说明
    /// </summary>
    public class Szyw_Wlxx : ServiceBase
    {
        ...
        protected void SetUpdatebgdh(string ztdh, string hgdc, DateTime jgwq)
        {
            this.DBHelp.BeginTransAction();
            try
            {

                SqlCommand cmd = this.DBHelp.GetCommand("update yw_hddz set hgdc = @hgdc, jgwq = ( case when @jgwq < '2000/01/01' then null else @jgwq end) from yw_hddz where ztdh = @ztdh;  INSERT INTO s_log_err (eid,tablename,changecols,mainid,cjsj,cscs,sfcg,funname) select 'szbgxx'+convert(varchar(35),getdate(),113)+ywbh,'yw_hddz','hgdc,jgwq',ywbh,getdate(),0,0,'DataToFreshPort' from yw_hddz where ztdh = @ztdh;");
                SqlParameter param1 = new SqlParameter("@hgdc", hgdc);
                SqlParameter param2 = new SqlParameter("@ztdh", ztdh);
                SqlParameter param3 = new SqlParameter("@jgwq", jgwq);
                cmd.Parameters.Add(param1);
                cmd.Parameters.Add(param2);
        ...
        protected void Save()
        {
            ...
            string dw_log = Request.Form["dw_log"].ToString();
            SafeDS ds_master = new SafeDS("dw_wlxx_edit");
            SafeDS ds_jzxxx = new SafeDS("dw_wlxx_edit_jzxxx");
            SafeDS ds_spxx = new SafeDS("dw_wlxx_edit_spxx");
        ...
        protected void ListSave()
        {
            ...
            SafeDS ds_list = new SafeDS("dw_wlxx_list");
            string dw_log = Request.Form["dw_log"].ToString();
            SafeDS ds_log = new SafeDS("dw_s_log_list");
            try
            {
