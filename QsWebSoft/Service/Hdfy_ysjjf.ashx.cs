...
namespace QsWebSoft.Service
{
    ...
    /// SalesOrder 的摘要说明
    /// </summary>
    public class Hdfy_ysjjf : ServiceBase
    {
        //单据删除
        protected  void Delete()
        {
            ...
            SqlCommand master = DBHelp.GetCommand("delete from yw_hddz_ysjjf Where sqdbh =@sqdbh");
            SqlCommand cmd = DBHelp.GetCommand("delete from yw_hddz_ysjjf_cmd Where sqdbh=@sqdbh");
            ...
            if (successed)
            {
                Response.Write("应收加急费编号为<" + sqdbh + ">,已被成功删除");
             
            }
            else
            {
                this.SetErrorInfo("应收加急费编号为<" + sqdbh + ">,删除失败");
            }
        }
        ...
        protected  void Save()
        {
            ...
            SafeDS ds_master = new SafeDS("dw_hddz_ysjjf_edit");
            SafeDS ds_jzxxx = new SafeDS("dw_hddz_ysjjf_edit_cmd"); 
            ...
            try
            {
                ...
                if (sqdbh == null || sqdbh == "")
                {
                    if (ds_master.GetRowStatus(1, Sybase.DataWindow.DataBuffer.Primary) == Sybase.DataWindow.RowStatus.NewAndModified)
                    {
                        ...
                        SqlCommand cmd = this.DBHelp.GetCommand("select max(right(sqdbh,4)) from yw_hddz_ysjjf where substring(sqdbh,3,8) = '" + year.Substring(0, 8) + "'");
                ...
                if (ds_master.UpdateData() == 1)
                {
                    ...
                    else
                    {
                        ...
                        this.SetErrorInfo("其他应收款明细信息保存失败!\n\n详细错误信息：\n" + ds_jzxxx.DBError);
                    }
                }
                else
                {
        ...
        protected void ListSave()
        {
            ...
            SafeDS ds_list = new SafeDS("dw_hddz_ysjjf_list");
            try
            {
