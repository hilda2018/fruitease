...
namespace QsWebSoft.Service
{
    ...
    /// SalesOrder 的摘要说明
    /// </summary>
    public class Hdfyjydfyyfk : ServiceBase
    {
        //单据删除
        protected  void Delete()
        {
            ...
            if (successed)
            {
                Response.Write("检疫点费用预录入编号为<" + yfkdbh + ">,已被成功删除");
             
            }
            else
            {
                this.SetErrorInfo("检疫点费用预录入编号为<" + yfkdbh + ">,删除失败");
            }
        }
        ...
        protected  void Save()
        {
            ...
            SafeDS ds_master = new SafeDS("dw_hddz_hdfyjydfyyfk_edit");
            SafeDS ds_jzxxx = new SafeDS("dw_hddz_hdfyjydfyyfk_edit_cmd"); 
            ...
            try
            {
                ...
                if (ds_master.UpdateData() == 1)
                {
                    ...
                    else
                    {
                        ...
                        this.SetErrorInfo("检疫点费用预录入明细信息保存失败!\n\n详细错误信息：\n" + ds_jzxxx.DBError);
                    }
                }
                else
                {
                    this.DBHelp.Rollback();
                    this.SetErrorInfo("检疫点费用预录入保存失败!\n\n详细错误信息：\n" + ds_master.DBError + "  " + ds_master.LastError);
                }
            }
        ...
        protected void ListSave()
        {
            ...
            SafeDS ds_list = new SafeDS("dw_hddz_hdfyjydfyyfk_list");
            try
            {
                ...
                {
                    this.DBHelp.Rollback(); ;
                    this.SetErrorInfo("检疫点费用预录入信息保存失败!\n\n详细错误信息：\n" + ds_list.DBError);
                }
