...
namespace QsWebSoft.Service
{
    ...
    /// SalesOrder 的摘要说明
    /// </summary>
    public class Szhdfy_sffy : ServiceBase
    {
        //单据删除
        protected  void Delete()
        {
            ...
            if (successed)
            {
                Response.Write("检疫点费用编号为<" + sqdbh + ">,已被成功删除");
             
            }
            else
            {
                this.SetErrorInfo("检疫点费用编号为<" + sqdbh + ">,删除失败");
            }
        }
        ...
        protected  void Save()
        {
            ...
            SafeDS ds_master = new SafeDS("dw_szyw_sjfk_edit");
            SafeDS ds_jzxxx = new SafeDS("dw_szyw_sjfk_edit_cmd");
            try
            {
        ...
        //应付转实付
        protected void Szyw_Yfzsf_Save()
        {
            string userID = AppService.GetUserID();
            string sqdbh = Request.Form["sqdbh"].ToString();
            var operation = Request.Form["operation"].ToString();
            ...
            string dw_jzxxx = Request.Form["dw_jzxxx"].ToString();
            SafeDS ds_master = new SafeDS("dw_szyw_sjfk_edit");
            SafeDS ds_jzxxx = new SafeDS("dw_szyw_sjfk_yfzsf_cmd");
            try
            {
                ds_master.SetChanges(dw_master);
                ds_jzxxx.SetChanges(dw_jzxxx);
        ...
        protected void ListSave()
        {
            ...
            SafeDS ds_list = new SafeDS("dw_hddz_Szhdfy_sffy_list");
            try
            {
