...
namespace QsWebSoft.Service
{
    ...
    /// SalesOrder 的摘要说明
    /// </summary>
    public class Commodity :  ServiceBase
    {
        ...
        protected void Save()
        {
            ...
            SafeDS ds_master = new SafeDS("dw_ywy_commodity_edit");
            try
            {
                ...
                if (yw_spbm == null || yw_spbm == "")
                {
                    ...
                    {
                        SqlCommand cmd = this.DBHelp.GetCommand("select max(yw_spbm) from yw_commodity where sqr=@userid ");
   
                        cmd.Parameters.Add(new SqlParameter("@userid", userID)); 
                        object value = cmd.ExecuteScalar();
                        if (Convert.IsDBNull(value) || value == null)
                        {
                            yw_spbm = userID + "000001";
                        }
                        else
                        {
                            yw_spbm = (string)value;
                            yw_spbm = yw_spbm.Remove(0, userID.Length);
                            yw_spbm = userID + String.Format("{0:000000}", (long.Parse(yw_spbm) + 1));
                        }
