...
namespace QsWebSoft.Service
{
    ...
    /// SalesOrder 的摘要说明
    /// </summary>
    public class Hdfymtthf : ServiceBase
    {
        //单据删除
        protected  void Delete()
        {
            ...
            if (successed)
            {
                Response.Write("码头提货费编号为<" + sqdbh + ">,已被成功删除");
             
            }
            else
            {
                this.SetErrorInfo("码头提货费编号为<" + sqdbh + ">,删除失败");
            }
        }
        ...
        protected  void Save()
        {
            ...
            SafeDS ds_master = new SafeDS("dw_hddz_hdfymtthf_edit");
            SafeDS ds_jzxxx = new SafeDS("dw_hddz_hdfymtthf_edit_cmd"); 
            try
            {
                ...
                if (ds_master.UpdateData() == 1)
                {
                    ...
                    else
                    {
                        ...
                        this.SetErrorInfo("码头提货费明细信息保存失败!\n\n详细错误信息：\n" + ds_jzxxx.DBError);
                    }
                }
                else
                {
                    this.DBHelp.Rollback();
                    this.SetErrorInfo("码头提货费保存失败!\n\n详细错误信息：\n" + ds_master.DBError + "  " + ds_master.LastError);
                }
            }
        ...
        protected void ListSave()
        {
            ...
            SafeDS ds_list = new SafeDS("dw_hddz_hdfymtthf_list");
            try
            {
