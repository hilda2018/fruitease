...
namespace QsWebSoft.Service
{
    ...
    /// SalesOrder 的摘要说明
    /// </summary>
    public class Wlgzsjfk : ServiceBase
    {
        //单据删除
        protected  void Delete()
        {
            ...
            if (successed)
            {
                Response.Write("实际付款单编号为<" + sqdbh + ">,已被成功删除");
             
            }
            else
            {
                this.SetErrorInfo("实际付款单编号为<" + sqdbh + ">,删除失败");
            }
        }

        //单据删除--深圳
        protected void Delete_szyw()
        {
            bool successed = false;
        ...
        protected  void Save()
        {
            ...
            SafeDS ds_master = new SafeDS("dw_wlgz_sjfk_edit");
            SafeDS ds_jzxxx = new SafeDS("dw_wlgz_sjfk_edit_cmd"); 
            try
            {
                ...
                if (ds_master.UpdateData() == 1)
                {
                    ...
                    else
                    {
                        ...
                        this.SetErrorInfo("实际付款明细信息保存失败!\n\n详细错误信息：\n" + ds_jzxxx.DBError);
                    }
                }
                else
                {
                    this.DBHelp.Rollback();
                    this.SetErrorInfo("实际付款保存失败!\n\n详细错误信息：\n" + ds_master.DBError + "  " + ds_master.LastError);
                }
            }
        ...
        protected void ListSave()
        {
            ...
            SafeDS ds_list = new SafeDS("dw_wlgz_sjfk_list");
            try
            {
