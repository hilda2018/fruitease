...
namespace QsWebSoft.Service
{
    ...
    /// SalesOrder 的摘要说明
    /// </summary>
    public class Hdfyyshdfygj : ServiceBase
    {
        //单据删除
        protected void Delete()
        {
            ...
            if (master.ExecuteNonQuery() > 0)
            {
                ...
                {
                    if (update_yszyf.ExecuteNonQuery() >= 0)
                    {
                        if (update_hdfyys.ExecuteNonQuery() >= 0)
                        {
                            if (update_cqfys.ExecuteNonQuery() >= 0)
                            {
                                if (update_qtys.ExecuteNonQuery() >= 0)
                                {
            ...
            if (successed)
            {
                ...
                //string strErr;
                //Interfaces.GeneralPortal.DelHddz("yw_hddz_yshdfygj", yshdfygjbh, out strErr);             
        ...
        protected void Tbsxg()
        {
            string yshdfygjbh = Request.Form["yshdfygjbh"].ToString();
            string strErr;
            Interfaces.GeneralPortal.DataToFreshPort("yw_hddz_yshdfygj_cmd", null, yshdfygjbh, out strErr); 
