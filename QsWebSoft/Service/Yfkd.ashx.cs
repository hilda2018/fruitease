    ...
    public class Yfkd :  ServiceBase
    {
       ...
       //单据审核
        ...
        protected  void Save()
        {
            ...
            SafeDS ds_m = new SafeDS("dw_yfkd_edit");
            SafeDS ds_d = new SafeDS("dw_yfkd_edit_mx_dl");
            ...
            try
            {
                ...
                if (ds_m.GetRowStatus(1, Sybase.DataWindow.DataBuffer.Primary) == Sybase.DataWindow.RowStatus.NewAndModified)
                {
                    ...
                    SqlCommand cmd = this.DBHelp.GetCommand("select max(substring(sqdbh,10,4)) from yw_yfkd where  substring(sqdbh,3,2)=@year and substring(sqdbh,5,5)=@userid ");
                    cmd.Parameters.Add(new SqlParameter("@userid", userID));
                    cmd.Parameters.Add(new SqlParameter("@year", year));
                    object value = cmd.ExecuteScalar();
                    if (Convert.IsDBNull(value) || value == null)
                    {
                        sqdbh = "FK"+year+userID+"0001";
                    }
                    else
                    {
                        sqdbh = "FK" + year + userID + String.Format("{0:0000}", (long.Parse((string)value) + 1));
                    }
