...
namespace QsWebSoft.Service
{
    ...
    /// SalesOrder 的摘要说明
    /// </summary>
    public class Hdfyjydfyyfzsf : ServiceBase
    {
        //单据删除
        protected  void Delete()
        {
            ...
            SqlCommand cmd = DBHelp.GetCommand("update yw_hddz_fksqd_cmd set sqdbh = yfkdbh from yw_hddz_fksqd_cmd Where sqdbh=@sqdbh");
            ...
            if (successed)
            {
                Response.Write("应付转实付费用编号为<" + sqdbh + ">,已被成功删除");
             
            }
            else
            {
                this.SetErrorInfo("应付转实付费用编号为<" + sqdbh + ">,删除失败");
            }
        }
        ...
        protected  void Save()
        {
            ...
            SafeDS ds_master = new SafeDS("dw_hddz_hdfyjydfyyfzsf_edit");
            SafeDS ds_jzxxx = new SafeDS("dw_hddz_hdfyjydfyyfzsf_edit_cmd"); 
            try
            {
                ...
                if (ds_master.UpdateData() == 1)
                {
                    ...
                    else
                    {
                        ...
                        this.SetErrorInfo("应付转实付费用明细信息保存失败!\n\n详细错误信息：\n" + ds_jzxxx.DBError);
                    }
                }
                else
                {
                    this.DBHelp.Rollback();
                    this.SetErrorInfo("应付转实付费用保存失败!\n\n详细错误信息：\n" + ds_master.DBError + "  " + ds_master.LastError);
                }
            }
        ...
        protected void ListSave()
        {
            ...
            SafeDS ds_list = new SafeDS("dw_hddz_hdfyjydfyyfzsf_list");
            try
            {
                ...
                {
                    this.DBHelp.Rollback(); ;
                    this.SetErrorInfo("应付转实付费用信息保存失败!\n\n详细错误信息：\n" + ds_list.DBError);
                }
