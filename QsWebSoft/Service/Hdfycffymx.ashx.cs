...
namespace QsWebSoft.Service
{
    ...
    /// SalesOrder 的摘要说明
    /// </summary>
    public class Hdfycffymx : ServiceBase
    {
        //单据删除
        protected  void Delete()
        {
            ...
            SqlCommand master = DBHelp.GetCommand("delete from yw_hddz_fksqd_jzxxx Where sqdbh =@sqdbh"); 
            ...
            if (successed)
            {
                Response.Write("费用明细编号为<" + sqdbh + ">,已被成功删除");
             
            }
            else
            {
                this.SetErrorInfo("费用明细编号为<" + sqdbh + ">,删除失败");
            }
        }
        ...
        protected void ListSave()
        {
            ...
            SafeDS ds_list = new SafeDS("dw_hddz_hdfycffymx_list");
            try
            {
                ...
                {
                    this.DBHelp.Rollback(); ;
                    this.SetErrorInfo("费用拆分明细信息保存失败!\n\n详细错误信息：\n" + ds_list.DBError);
                }
