...
namespace QsWebSoft.Service
{
    ...
    /// SalesOrder 的摘要说明
    /// </summary>
    public class Wlgzfywfy : ServiceBase
    {
        //单据删除
        protected  void Delete()
        {
            ...
            SqlCommand master = DBHelp.GetCommand("delete from yw_wlgz_fksqd Where sqdbh =@sqdbh");
            SqlCommand cmd = DBHelp.GetCommand("delete from yw_wlgz_fksqd_cmd Where sqdbh=@sqdbh");
            ...
            if (successed)
            {
                Response.Write("海运非业务费用申请单编号为<" + sqdbh + ">,已被成功删除");
             
            }
            else
            {
                this.SetErrorInfo("海运非业务费用申请单编号为<" + sqdbh + ">,删除失败");
            }
        }
        ...
        protected  void Save()
        {
            ...
            SafeDS ds_master = new SafeDS("dw_wlgz_fywfy_edit");
            SafeDS ds_jzxxx = new SafeDS("dw_wlgz_fywfy_edit_cmd"); 
            try
            {
                ...
                if (sqdbh == null || sqdbh == "")
                {
                    if (ds_master.GetRowStatus(1, Sybase.DataWindow.DataBuffer.Primary) == Sybase.DataWindow.RowStatus.NewAndModified)
                    {
                        ...
                        SqlCommand cmd = this.DBHelp.GetCommand("select max(right(sqdbh,4)) from yw_wlgz_fksqd where substring(sqdbh,1,8) = '" + year.Substring(0, 8) + "' ");
                ...
                if (ds_master.UpdateData() == 1)
                {
                    ...
                    else
                    {
                        ...
                        this.SetErrorInfo("海运非业务费用申请单明细信息保存失败!\n\n详细错误信息：\n" + ds_jzxxx.DBError);
                    }
                }
                else
                {
                    this.DBHelp.Rollback();
                    this.SetErrorInfo("海运非业务费用申请单保存失败!\n\n详细错误信息：\n" + ds_master.DBError + "  " + ds_master.LastError);
                }
            }
        ...
        protected void ListSave()
        {
            ...
            SafeDS ds_list = new SafeDS("dw_wlgz_fywfy_list");
            try
            {
                ...
                {
                    this.DBHelp.Rollback(); ;
                    this.SetErrorInfo("海运非业务费用申请单信息保存失败!\n\n详细错误信息：\n" + ds_list.DBError);
                }
