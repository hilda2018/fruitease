...
function W_TszxList() {
    ...
    var ddlb_fl = new DropDownListBox(requestor);
    var ddlb_lcfl = new DropDownListBox(requestor);
    ...
    this.bind = function (win) {
        ...
        if (ddlb_fl == null) ddlb_fl = new DropDownListBox(requestor);
        ddlb_fl.Attach(win.Control("ddlb_fl"));
        if (ddlb_lcfl == null) ddlb_lcfl = new DropDownListBox(requestor);
        ddlb_lcfl.Attach(win.Control("ddlb_lcfl"));
        ...
        if (typeof (self.sle_SelectionChanged) == "function")
            win.AttachEvent("ddlb_fl", "SelectionChanged", self.sle_SelectionChanged);

        if (typeof (self.sle_SelectionChanged) == "function")
            win.AttachEvent("ddlb_lcfl", "SelectionChanged", self.sle_SelectionChanged);
    ...
    this.unbind = function () {
        ...
        ddlb_fl.Detach(); ddlb_fl = null;
        ddlb_lcfl.Detach(); ddlb_lcfl = null;
    ...
    this.Retrieve = function () {
        ...
        var qbxs = requestor.GetParm("qbxs");
 
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), userid, qbxs);
    ...
    this.OpenWindow = function () {
        ...
        if (dw_list.RowCount() <= 0)
            return;
        var zxbh = dw_list.GetItemString(dw_list.GetRow(), 'zxbh')
...
//        if (state == "提交") {
//            requestor.MessageBox("提示", "数据已经提交,不能被删除!", 1);
//            return;
//        }

        var zxbh = dw_list.GetItemString(row, "zxbh");
        if (requestor.MessageBox("提示", "删除投诉咨询编号为<" + zxbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;

        //执行删除的WebService
        webReq.SetFunctionID(requestor.GetFunctionID());
        webReq.Invoke("Delete", "zxbh=" + zxbh);
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(zxbh) like '%" + text + "%')  or ( Lower(zxbh) like '%" + text + "%')");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.sle_SelectionChanged = function () {
        dw_list.SetRedraw(false);
        var fl = ddlb_fl.GetText();
        var lcfl = ddlb_lcfl.GetText();
        var ls_filter = "";
        if (fl != "全部") {
            ls_filter = "fl = '" + fl + "'";
        }
        ...
        if (lcfl != "全部") {             
            if (ls_filter == "") {
                ...
                ls_filter = ls_filter + " and lcfl = '" + lcfl + "'";
            }
        }
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Nbgl.W_TszxList", "dw_tszx_list");
        self.Retrieve();
    }
    //#endregion
