...
function W_TszxEdit() {
    ...
    this.Save = function () {
        ...
        var zxbh = dw_master.GetItemString(row, "zxbh");

        var fl = dw_master.GetItemString(row, "fl");
        if (fl == null || fl == "") {
            requestor.MessageBox("提示", "分类必须输入！");
            dw_master.SetColumn("fl");
            return;
        }

        var lcfl = dw_master.GetItemString(row, "lcfl");
        if (lcfl == null || lcfl == "") {
            requestor.MessageBox("提示", "流程分类必须输入！");
            dw_master.SetColumn("lcfl");
            return;
        }
        ...
        webReq.Invoke("Save", "dw_master=" + dw_master_data + "&zxbh=" + zxbh + "&operation=" + editMode);

        if (webReq.StatusCode() == 200) {
            //服务器成功处理，返回单号
            zxbh = webReq.ResponseText();
    ...
    this.Print = function () {
        ...
        var zxbh = dw_master.GetItemString(1, "zxbh");
