...
function W_Ky_QdcxList() {

    var self = this;
    var requestor = new PBWindow();
    var dw_clxx = new DataWindow(requestor);
    ...
    this.bind = function (win) {
        ...
        if (dw_clxx == null) dw_clxx = new DataWindow(requestor);
        dw_clxx.Attach(win.Control("dw_clxx"));
        ...
        if (typeof (self.btnUpdate) == "function")
            win.AttachEvent("btn_update", "Clicked", self.btnUpdate);
    ...
    webReq.SetWebServiceUrl("/Service/Kyqdcx.ashx");   
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(endDate.getVarDate(), Dlwtf, userid, jdrjc);
        dw_clxx.Retrieve(endDate.getVarDate(), Dlwtf);
    ...
    this.btnUpdate = function () {
        ...
        if (dw_clxx.AcceptText() != 1)
            return;

        var row1 = dw_list.GetRow();
        var row2 = dw_clxx.GetRow();

        var cdphbm = dw_list.GetItemString(row1, "cdphbm");
        var PortVehicleID = dw_clxx.GetItemString(row2, "PortVehicleID");
        var qddate = dw_clxx.GetItemDate(row2, "signdate");

        dw_list.SetItem(row1, "PortVehicleID", PortVehicleID);
        dw_list.SetItem(row1, "signdate", qddate);
        dw_clxx.SetItem(row2, "cdphbm", cdphbm);
    ...
    this.Save = function () {
        ...
        if ((dw_list.ModifiedCount() + dw_list.DeletedCount()) <= 0 && (dw_clxx.ModifiedCount() + dw_clxx.DeletedCount()) <= 0) {
            alert("没有数据需要保存!");
            return;
        }
        ...
        var dw_d_data2 = dw_clxx.GetChanges();
        var dw_d_data2 = dw_clxx.GetChanges();
        if (dw_d_data2 == "")
            return;
        ...
        webReq.Invoke("Save", "dw_list=" + dw_d_data + "&dw_list2=" + dw_d_data2);
    ...
    this.Recover = function () {
        ...
        QsWebSoft.PubMethod.DelColRecord("QsWebSoft.Wlxx.W_Ky_QdcxList", "dw_ky_qdcx_list");
        self.Retrieve();
    }
    //#endregion
    ...
    //#region 获得字段焦点
    this.Help = function () {
        javascript: window.open('\签到管理.txt')
    }
    //#endregion
