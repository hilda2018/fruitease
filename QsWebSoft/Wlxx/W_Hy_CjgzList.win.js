...
function W_Hy_CjgzList() {
	 ...
	 var sle_cd= new SingleLineEdit( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(sle_cd == null ) sle_cd= new SingleLineEdit( requestor );
		 sle_cd.Attach( win.Control( "sle_cd") ); 
		 ...
		 if(typeof(self.slecd_EditChanged)=="function") 
			 win.AttachEvent("sle_cd","EditChanged",self.slecd_EditChanged); 
    ...
    //#region 打开编辑窗口
    this.New = function () {
        var iw_Hy_Wlgz_Hcj = null;

        if (iw_Hy_Wlgz_Hcj == null)
            iw_Hy_Wlgz_Hcj = new W_Hy_Wlgz_Hcj();

        var Dlwtf = requestor.GetParm("Dlwtf");
        var row = dw_list.GetRow();
        var cjmc = escape(dw_list.GetItemString(row, "cjmc")); 
        var rwbh = ""; 
        iw_Hy_Wlgz_Hcj.SetDataWindow(dw_list);         
        iw_Hy_Wlgz_Hcj.SetRwlx("换车架");
        var rwmc = "hcj";
        requestor.Open("/LoadWindow.ashx?WinName=Wlxx.W_Hy_Wlgz_Hcj&Rwbh=" + rwbh + '&Rwmc=' + rwmc + '&Cjmc=' + cjmc, iw_Hy_Wlgz_Hcj);
    ...
    this.OpenWindow = function () {
        var iw_Hy_WlgzEdit = null;

        if (iw_Hy_WlgzEdit == null)
            iw_Hy_WlgzEdit = new W_Hy_WlgzEdit();

        var Dlwtf = requestor.GetParm("Dlwtf");
        var row = dw_mx.GetRow();
        var rwbh = dw_mx.GetItemString(row, "rwbh");
        var ywbh = dw_mx.GetItemString(row, "ywbh");
        var hddz_cxh = dw_mx.GetItemNumber(row, "hddz_cxh");
        iw_Hy_WlgzEdit.SetDataWindow(dw_list);
        iw_Hy_WlgzEdit.SetData(ywbh);
        iw_Hy_WlgzEdit.SetRow(hddz_cxh);
        iw_Hy_WlgzEdit.SetRwlx("物流跟踪");
        var rwmc = "wlgz";

        requestor.Open("/LoadWindow.ashx?WinName=Wlxx.W_Hy_WlgzEdit&Rwbh=" + rwbh + '&Ywbh=' + ywbh + '&Hddz_cxh=' + hddz_cxh.toString() + '&Rwmc=' + rwmc, iw_Hy_WlgzEdit);
    ...
    this.dw_List_DoubleClicked = function (xPos, yPos, Row, dwoName) {
        var cjmc = dw_list.GetItemString(Row, "cjmc");
        var beginDate = new Date(dp_begin.GetValue());
        var endDate = new Date(dp_end.GetValue());
        ...
        dw_mx.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), cjmc)
...
//            }
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(cjmc) like Upper('%" + text + "%')) or ( Upper(cjh) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    //#region 过滤客户
    this.slecd_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(cdbm) like Upper('%" + text + "%')) or  ( Upper(cdjc) like Upper('%" + text + "%')) or ( Upper(cdpym) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Save = function () {
        ...
        webReq.Invoke("ScListSave", "dw_list=" + dw_d_data);
