...
function W_Ky_JyztList() {
    ...
    var ddlb_cyd = new DropDownListBox(requestor);
    ...
    var dw_jyzt_print = new DataWindow(requestor);
    ...
    this.bind = function (win) {
        ...
        if (ddlb_cyd == null) ddlb_cyd = new DropDownListBox(requestor);
        ddlb_cyd.Attach(win.Control("ddlb_cyd"));
        ...
        if (dw_jyzt_print == null) dw_jyzt_print = new DataWindow(requestor);
        dw_jyzt_print.Attach(win.Control("dw_jyzt_print"));
        ...
        if (typeof (self.sletgdh_EditChanged) == "function")
            win.AttachEvent("sle_2", "EditChanged", self.sletgdh_EditChanged);

        if (typeof (self.slebjh_EditChanged) == "function")
            win.AttachEvent("sle_bjh", "EditChanged", self.slebjh_EditChanged);

        if (typeof (self.ddlbcyd_SelectChanged) == "function")
            win.AttachEvent("ddlb_cyd", "SelectionChanged", self.ddlbcyd_SelectChanged);
        ...
        if (typeof (self.SqdPrint) == "function")
            win.AttachEvent("btn_dysq_print", "Clicked", self.SqdPrint);
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), jydzt, sfssys, userid, jdrjc);
        self.ddlbcyd_SelectChanged(cyd);
    }
    //#endregion
    ...
    this.dw_List_ItemChanged = function (row, dwoName, data) {
        if (dwoName == "jzxxx_jycydzt") {
            ...
            if (data == "扣货") {
                ...
                var date = new Date();
                dw_list.SetItem(row, "kgsj_lrsj", date.getVarDate())
                dw_list.SetItem(row, "kgsj_czry", ywy)
            } else {
                ...
                dw_list.SetItem(row, "kgsj_lrsj", date.getVarDate())
                dw_list.SetItem(row, "kgsj_czry", '')
                dw_list.SetItem(row, "kghclsj_lrsj", date.getVarDate())
                dw_list.SetItem(row, "kghclsj_czry", '')
            }
        }

        if (dwoName == "jzxxx_kgsj") {
            dw_list.AcceptText();
            var ywy = requestor.GetParm("userid");
            var date = new Date();
            dw_list.SetItem(row, "kgsj_lrsj", date.getVarDate())
        ...
        if (dwoName == "jzxxx_kghclsj") {
            dw_list.AcceptText();
            var ywy = requestor.GetParm("userid");
            var date = new Date();
            dw_list.SetItem(row, "kghclsj_lrsj", date.getVarDate())
            dw_list.SetItem(row, "kghclsj_czry", ywy)
        }
        ...
        if (dwoName == "jzxxx_kghcl") {
            ...
            if (data != null) {
                ...
                dw_list.SetItem(1, "jzxxx_kghclsj", "0000/00/00 00:00")
                //                dw_list.SetItem(row, "jzxxx_kghclsj", date.getVarDate());
            }

        }

        if (dwoName == "jzxxx_tgjysj") {
            dw_list.AcceptText();

            var ywy = requestor.GetParm("userid");
            var date = new Date();
            dw_list.SetItem(row, "tgjysj_lrsj", date.getVarDate())
            dw_list.SetItem(row, "tgjysj_czry", ywy)
            ...
            if (zjfscwld_bz == "Y" || sc_bz == "Y" || dzdd == "Y") {
                ...
                var jzxxx_tgjysj = new Date(dw_list.GetItemDate(Row, "jzxxx_tgjysj"));

                var num = QsWebSoft.PubMethod.UpdateQdsjywbh(ywbh, jzxxx_tgjysj).value;

            }

        }

        if (dwoName == "jzxxx_cjydsj") {
            dw_list.AcceptText();
            var ywy = requestor.GetParm("userid");
            var date = new Date();
            dw_list.SetItem(row, "cjydsj_lrsj", date.getVarDate())
            dw_list.SetItem(row, "cjydsj_czry", ywy)
        }
    ...
    //#region 打印
    this.SqdPrint = function () {
        var row = dw_list.GetRow();
        if (row <= 0)
            return;
        dw_print.SetDataObject("dw_ky_jyzt_print");
        dw_print.Retrieve(dw_list.GetItemString(dw_list.GetRow(), "ywbh"));
        dw_print.PrintPreview();
    }
    //#endregion
    ...
    //#region 过滤报检号
    this.ddlbcyd_SelectChanged = function () {
        var cyd = ddlb_cyd.GetText();
        dw_list.SetRedraw(false);
        if (cyd == "")
            dw_list.SetFilter("");
        else if (cyd == "郑州") {
            dw_list.SetFilter("( Upper(jzxxx_jyd_jc) like Upper('%郑州%'))");
        } else if (cyd == "非郑州") {
            dw_list.SetFilter("( Upper(jzxxx_jyd_jc) not like Upper('%郑州%'))");
        } else {
            dw_list.SetFilter("");
        }
    ...
    this.Save = function () {
        ...
        for (var i = 1; i <= dw_list.RowCount(); i++) {
            ...
            if (dw_list.GetItemStatus(i, 0, DWBUFFER.Primary) == 1) {
                ...
                dw_log.SetItem(row, "changecols", "jycydzt,tgjysj,kghcl,ssys,cjydsj");
        ...
        webReq.Invoke("JyztListSave", "dw_list=" + dw_d_data + "&dw_log=" + dw_log_data);
