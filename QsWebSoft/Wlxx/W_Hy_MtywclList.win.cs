...
{
    public partial class W_Hy_MtywclList
    {
        public W_Hy_MtywclList()
        {
        }
        public override void OnLoad()
        {
            ...
            //dw_log.Retrieve(userid,"mtyw");

            //派单员
            this.ds_pdy.DataWindowObject = "dw_pdy_list";
            this.ds_pdy.Retrieve();
            this.ddlb_pdy.Items.Add("全部");
            for (int row = 1; row <= ds_pdy.RowCount; row++)
            {
                this.ddlb_pdy.Items.Add(ds_pdy.GetItemString(row, "pdy"));
            }

            // 数据检索
            this.dw_list.Retrieve(DateTime.Parse(this.dp_begin.Value.ToString()), DateTime.Parse(this.dp_end.Value.ToString()), "全部", "全部", "全部", "全部", "全部", userid, "全部", "全部", "全部", "全部");
            //注册相关的js文件
            this.RegisterClientScriptInclude("ExtPB_Demo", "/Beta3/ExtPB_Demo.js");
            this.RegisterClientScriptInclude("W_Hy_MtywclList", "/Wlxx/W_Hy_MtywclList.win.js");
