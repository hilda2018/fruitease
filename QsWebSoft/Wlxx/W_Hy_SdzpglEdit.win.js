...
function W_Hy_SdzpglEdit() {
    ...
    //#region 变量定义
    var editMode = "";
    var iw_Hddz_Sdzpxx_Select = null;
    var dwc = new DataWindowChild();
    var iw_Wldw_Select = null;
    var iw_Sj_Select = null;
    //实例化明细单的WebService请求的JS类对象
    var webReq = new WebRequestor();
    webReq.SetWebServiceUrl("/Service/Hy_Sdzpgl.ashx");
    ...
    this.Save = function () {
        ...
        var kpje = dw_master.GetItemNumber(1, "kpje")
        var kpje_sum = 0
        if (dw_jzxxx.RowCount() > 0) {
            kpje_sum = dw_jzxxx.GetItemNumber(1, "kpje_sum")
        };

        if (kpje != kpje_sum) {
            alert("拆分金额和发票金额不相等不能保存!");
            return;
        };

        var row = dw_master.GetRow();
        var fph = dw_master.GetItemString(row, "fph");

        dw_master.SetItem(row, "fph", fph);
        ...
        for (row = dw_jzxxx.RowCount(); row > 0; row--) {
            dw_jzxxx.SetItem(row, "fph", fph);
            dw_jzxxx.SetItem(row, "cxh", row);
            if (row == dw_jzxxx.RowCount()) {
                hz_hth = dw_jzxxx.GetItemString(row, "hth")
                hz_ywbh = dw_jzxxx.GetItemString(row, "ywbh")
                hz_ztdh = dw_jzxxx.GetItemString(row, "ztdh")
                hz_ftdh = dw_jzxxx.GetItemString(row, "ftdh")
            }
            else {
                hz_hth = hz_hth + ", " + dw_jzxxx.GetItemString(row, "hth")
                hz_ywbh = hz_ywbh + ", " + dw_jzxxx.GetItemString(row, "ywbh")
                hz_ztdh = hz_ztdh + ", " + dw_jzxxx.GetItemString(row, "ztdh")
                hz_ftdh = hz_ftdh + ", " + dw_jzxxx.GetItemString(row, "ftdh")
            };
        ...
        dw_master.SetItem(1, "hz_ftdh", hz_ftdh)

        dw_master.SetItem(1, "jdrbm", dw_jzxxx.GetItemString(1, "jdrbm"))
        dw_master.SetItem(1, "jdrmc", dw_jzxxx.GetItemString(1, "jdrmc"))
        dw_master.SetItem(1, "jdrjc", dw_jzxxx.GetItemString(1, "jdrjc"))
        dw_master.SetItem(1, "jdrpym", dw_jzxxx.GetItemString(1, "jdrpym"))
        dw_master.SetItem(1, "hdrbm", dw_jzxxx.GetItemString(1, "hdrbm"))
        dw_master.SetItem(1, "hdrmc", dw_jzxxx.GetItemString(1, "hdrmc"))
        dw_master.SetItem(1, "hdrjc", dw_jzxxx.GetItemString(1, "hdrjc"))
        dw_master.SetItem(1, "hdrpym", dw_jzxxx.GetItemString(1, "hdrpym"))
        ...
        webReq.Invoke("Save", "dw_master=" + dw_master_data + "&dw_jzxxx=" + dw_jzxxx_data + "&dw_log=" + dw_log_data + "&fph=" + fph + "&operation=" + editMode);

        if (webReq.StatusCode() == 200) {
            //服务器成功处理，返回单号
            cdphbm = webReq.ResponseText();
            dw_master.SetItem(1, "fph", fph);
            ...
            dw_master.Modify("fph.Tabsequence=0");
    ...
    this.InsertRow = function () {
        if (iw_Hddz_Sdzpxx_Select == null)
            iw_Hddz_Sdzpxx_Select = new W_Hddz_Sdzpxx_Select();
        var ShareMode = requestor.GetParm("ShareMode");
        var ywy = requestor.GetParm("userid");
        var Dlwtf = requestor.GetParm("Dlwtf");
        row = dw_jzxxx.InsertRow(dw_jzxxx.GetRow());
        var dwo = dw_jzxxx.GetDataObject();
        iw_Hddz_Sdzpxx_Select.SetRow(row);
        iw_Hddz_Sdzpxx_Select.SetDwo(dwo);
        iw_Hddz_Sdzpxx_Select.SetDetailDW(dw_jzxxx);
        requestor.Open("/LoadWindow.ashx?WinName=Xt_Popwin.W_Hddz_Sdzpxx_Select&ShareMode=" + ShareMode + '&ywy=' + ywy + '&Dlwtf=' + Dlwtf + '&hdrbm=' + hdrbm + '&jdrbm=' + jdrbm, iw_Hddz_Sdzpxx_Select);
...
//        dw_jzxxx.InsertRow(0);
    ...
    this.dw_master_Clicked = function (xPos, yPos, Row, dwoName) {
        ...
        if (dwoName == "kprjc_t") {
            ...
            iw_Wldw_Select.SetData("sdzpgl_kpr");
