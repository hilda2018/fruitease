...
function W_Hy_SdzpglList() {
    ...
    var sle_kpr = new SingleLineEdit(requestor);
    ...
    this.bind = function (win) {
        ...
        if (sle_kpr == null) sle_kpr = new SingleLineEdit(requestor);
        sle_kpr.Attach(win.Control("sle_kpr"));
        ...
        if (typeof (self.slekpr_EditChanged) == "function")
            win.AttachEvent("sle_kpr", "EditChanged", self.slekpr_EditChanged);
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve(beginDate.getVarDate(), endDate.getVarDate(), userid, jdrjc);
    ...
    this.OpenWindow = function () {
        ...
        if (dw_list.RowCount() <= 0)
            return;
        var fph = dw_list.GetItemString(dw_list.GetRow(), 'fph') 
        ...
        if (openStyle == "2")
            ...
            ts_1.Open(node, label, "", "/OpenWindow.aspx?FunID=" + node + '&fph=' + fph + '&operation=' + operation.toString(), true);
    ...
    this.dw_List_DoubleClicked = function (xPos, yPos, Row, dwoName) {
        if (Row > 0) {
            if (dwoName == "fj") {
                ...
                var ShareMode = requestor.GetParm("ShareMode");
                var Dlwtf = requestor.GetParm("Dlwtf");
                var fph = dw_list.GetItemString(Row, "fph");
                var lx = "gjyffpgl"
                requestor.Open("/LoadWindow.ashx?WinName=Hddz.W_Fj_Edit&ShareMode=" + ShareMode + "&ywy=" + ywy + "&Dlwtf=" + Dlwtf + "&ywbh=" + fph + "&lx=" + lx, iw_Fj_Edit);
                dw_list.SetFocus();
...
//        }
        ...
        var fph = dw_list.GetItemString(row, "fph");
        if (requestor.MessageBox("提示", "删除发票号为<" + fph + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;

        //执行删除的WebService
        webReq.SetFunctionID(requestor.GetFunctionID());
        webReq.Invoke("Delete", "fph=" + fph);
    ...
    this.sle_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(fph) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    //#region 过滤客户
    this.slekpr_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(kprjc) like Upper('%" + text + "%')) or ( Upper(kprpym) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.slehth_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(hz_hth) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.sleywbh_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(hz_ywbh) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.sleftdh_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(hz_ftdh) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Save = function () {
        ...
        for (var i = 1; i < dw_list.RowCount(); i++) {
            if (dw_list.GetItemStatus(i, 0, DWBUFFER.Primary) == 1) {
                var fph = dw_list.GetItemString(i, "fph");
