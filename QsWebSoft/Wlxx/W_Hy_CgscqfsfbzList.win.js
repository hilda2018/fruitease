    ...
    this.dw_List_DoubleClicked = function (xPos, yPos, Row, dwoName) {
        if (Row > 0) {
            if (dwoName == "cgsjc") {               
                ...
                iw_Wldw_Select.SetData("hy_cgscqfsfbz");
   ...
   //#region dw_List  ItemChanged
    ...
    this.slecgs_EditChanged = function (text) {
        ...
        if (text == "")
            ...
            dw_list.SetFilter("( Upper(cgsmc) like Upper('%" + text + "%')) or ( Upper(cgsjc) like Upper('%" + text + "%')) or ( Upper(cgspym) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
    ...
    this.Save = function () {
        ...
        webReq.Invoke("CgscqfsfbzListSave", "dw_list=" + dw_d_data);
