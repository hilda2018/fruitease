...
function W_Hy_MtsjrwList() {
	 ...
	 var sle_clsscd= new SingleLineEdit( requestor );
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(sle_clsscd == null ) sle_clsscd= new SingleLineEdit( requestor );
		 sle_clsscd.Attach( win.Control( "sle_clsscd") ); 
		 ...
		 if(typeof(self.sleClsscd_EditChanged)=="function") 
			 win.AttachEvent("sle_clsscd","EditChanged",self.sleClsscd_EditChanged); 
		 ...
		 if(typeof(self.OpenWlgz)=="function") 
			 win.AttachEvent("btn_open_rw","Clicked",self.OpenWlgz); 
	 ...
	 if(typeof(baseUnBind)=="function") baseUnBind(); 
		 ...
		 sle_clsscd.Detach(); sle_clsscd=null; 
    ...
    this.Retrieve = function () {
        ...
        dw_list.Retrieve( beginDate.getVarDate(), endDate.getVarDate(), mtywcl, dhzt, tgzt, userid, jdrjc, sc, pdy );
...
//            }
    ...
    //#region 过滤车辆所属车队
    this.sleClsscd_EditChanged = function (text) {
        dw_list.SetRedraw(false);
        if (text == "")
            dw_list.SetFilter("");
        else
            dw_list.SetFilter("( Upper(jzxxx_clsscd) like Upper('%" + text + "%'))");
        dw_list.Filter();
        dw_list.Sort();
        dw_list.SetRedraw(true);
    }
    ...
    this.Save = function () {
        ...
        webReq.Invoke("MtsjrwListSave", "dw_list=" + dw_d_data);
