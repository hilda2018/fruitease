...
function W_SzywFkglList() {
	 ...
	 if(typeof( baseBind )=="function") baseBind(win);
		 ...
		 if(typeof(self.YfZSf)=="function") 
			 win.AttachEvent("btn_new","Clicked",self.YfZSf); 
...
//        var status = dw_list.GetItemString(dw_list.GetRow(), 'state')
        ...
        if (state == "提交" || state == "待批" || zbr != userid) {
            operation = "show";
        }
...
//    }
    ...
    this.Delete = function () {
        ...
        var sqdbh = dw_list.GetItemString(row, "sqdbh");
        if (requestor.MessageBox("提示", "删除海运实际付款申请单号为<" + sqdbh + ">,是否确定?", ICON.Question, BUTTON.YesNo) == 2)
            return;
